...
Всего 400 сущностей: 338 локальных определений, 26 внешних и 36 динамических границ. Связей 977; из них 924 добавлены в .003. Начальные 35 сущностей/53 связи сохранены отдельными частями; добавлены 365 сущностей. Два начальных процесса сохранены без изменения. **Полные процессы пилота и IQ-07 ожидают .004.**
...
| IQ-07 — контроль остатка | P13 | Процессы Actor ещё not_indexed |

Пустой результат P12 не является утверждением об отсутствии читателей во всей системе. P13 намеренно сохраняет not_indexed до следующей задачи. Все непустые ответы имеют partial, подтверждённую свежесть и адреса для перехода к исходникам.

## Адреса для TASK-0006.004

Это перечень уже созданных узлов, а не готовые записи новых процессов. Порядок, ветви, await и выходы будут оформлены в .004; начальные proc-000001/proc-000002 остаются самостоятельными примерами.
