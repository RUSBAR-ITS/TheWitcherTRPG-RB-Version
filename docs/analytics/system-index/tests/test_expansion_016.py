...
class DefenseExpansion(unittest.TestCase):
    ...
    def test_graph_addresses_reverse_links_facets_and_processes(self):
        ...
        self.assertEqual([len(d.sources),len(d.entities),len(d.relations),len(d.processes)],[615,4120,9766,211])
        self.assertEqual(len(d.manifest['scope']['selected_sources']),103)
        self.assertEqual(collections.Counter(s['coverage']['definitions']['state'] for s in d.sources.values()),{'complete':2,'partial':222,'not_indexed':391})
