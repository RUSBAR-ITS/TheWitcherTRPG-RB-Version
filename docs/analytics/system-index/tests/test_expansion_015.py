...
class WeaponAttackExpansion(unittest.TestCase):
    ...
    def test_graph_addresses_reverse_links_facets_and_processes(self):
        ...
        self.assertEqual([len(d.sources),len(d.entities),len(d.relations),len(d.processes)],[615,4022,9427,189])
        self.assertEqual(len(d.manifest['scope']['selected_sources']),94)
        self.assertEqual(collections.Counter(s['coverage']['definitions']['state'] for s in d.sources.values()),{'complete':2,'partial':219,'not_indexed':394})
