...
class ArmorExpansion(unittest.TestCase):
    ...
    def test_selection_and_layer_branches(self):
        a=self.source(10)
        for at,t in [(3,'-this.system.lifepathModifiers.ignoredArmorEncumbrance'),(4,"this.items.filter(item => item.type == 'armor' && item.system.equipped && !item.system.isStored)"),(9,'Math.max(encumbranceModifier, 0)'),(13,"this.getList('armor').filter(a => a.system.equipped)"),(59,'this.getArmors([])'),(66,'properties.bypassesNaturalArmor'),(73,'this.getArmorSp(armorSet, location.name, properties).displaySP'),(74,'this.getArmorSp(armorSet, location.name, properties).totalSP'),(110,'lightCount > 1 || mediumCount > 1 || heavyCount > 1'),(112,'return;'),(125,"armorSet['lightArmor']?.system[location].modifiedStoppingPower"),(137,'!properties.bypassesWornArmor'),(138,'if (heavyArmorSP)'),(145,'this.getArmorDiffBonus(heavyArmorSP, mediumArmorSP)'),(156,'this.getArmorDiffBonus(mediumArmorSP, lightArmorSP)'),(160,'this.getArmorDiffBonus(heavyArmorSP, lightArmorSP)'),(170,'naturalArmorSP && !properties.bypassesNaturalArmor'),(184,'underArmor <= 0 || overArmor <= 0')]:self.assertIn(t,a[at-1])
        ...
        actor='\n'.join(self.source(47));self.assertIn('!i.system.isStored',actor);self.assertIn('this.getArmorEcumbrance()',actor)
        self.assertIn(self.q['CommonItemData.isStored'],{r['to'] for r in self.edges('actor.armorMixin.getArmorEcumbrance','reads')})
