...
class SkillUIExpansion(unittest.TestCase):
    ...
    def test_render_listener_dispatch_and_old_dom_contract(self):
        ...
        self.assertIn('await super._onRender','\n'.join(a));self.assertIn('this.activateListeners(this.element)','\n'.join(a))
        self.assertIn('super.activateListeners(html)','\n'.join(c))
        self.assertIn('this.skillListener(html)','\n'.join(a));self.assertIn('this.customSkillListener(html)','\n'.join(a))
        self.assertIn('this.skillListener(html[0])','\n'.join(v));self.assertIn('this.customSkillListener(html[0])','\n'.join(v))
        sm=self.source('src-000045');cm=self.source('src-000040')
        self.assertIn('const $html = $(html)',sm[26]);self.assertIn('html = $(html)',cm[2])
        self.assertNotIn('jQuery =', '\n'.join(sm))
        self.assertEqual(self.data.entities['ent-000379']['boundary']['expression'], '$(html)')
        self.assertNotIn('ent-000379', {r['to'] for r in self.edges('ent-000308','writes')})
        self.assertIn('ent-000379', {r['to'] for r in self.edges('ent-000308','calls')})
        ...
        self.assertIn("html.find('#custom-rollable, [data-action=rollCustomSkill]').on('click', thisActor.rollCustomSkillCheck.bind(thisActor))",cm[5])
