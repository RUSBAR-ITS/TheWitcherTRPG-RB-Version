...
Отсутствующие li/owner/effect не везде защищены. Запросы create/update/delete и косвенные _preCreate/_preUpdate проходят через ядро. Возвращаемый Promise события не означает ожидания со стороны DOM dispatch. Браузер, права и запись в БД здесь не исполнялись. См. [issue-00055](../../issues/potential/issue-00055.md), [issue-00056](../../issues/potential/issue-00056.md).
...
Категории/actions Item-конфигурации и targets-обёртки имеют определения и связи, но не отдельные полные процессы. Фрагменты соседних контекстов и вызовов не означают покрытия всех их ветвей. Дублирование травм в tab — представление, не два документа или два выполнения лечения: [issue-00054](../../issues/potential/issue-00054.md#дополнительная-сверка-task-0003047).
