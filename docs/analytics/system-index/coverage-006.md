...
Для ready разделены три состояния: отсутствующий pack, отклонённый getIndex и успешно полученный пустой индекс. Первые два не достигают hotbar/font/socket; пустой fulfilled индекс допускает продолжение, а отказ позднего applyCritWound относится к другой границе ([issue-00289](../../issues/closed/issue-00289.md)). Петля index означает ожидание одного Promise и не является повтором запроса.
