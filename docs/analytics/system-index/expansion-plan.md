...
**Текущее состояние:** .001–.019 выполнены; во [второй очереди](#second-wave) следующая — [TASK-0006.020](../../tasks/task-0006.020.md). B01–B20 служат учёту остатка. Исторические таблицы и срез постановки .012–.023 сохранены; текущие роли и фактическое покрытие приведены отдельно.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). После .019: определения есть в 239/615 файлах: два словаря complete по строковым ключам, 237 файлов partial; роли 128 основных/111 смежных, 376 без определений. Остались непокрытые области частичных файлов и определения 376 файлов. [Границы .019](coverage-019.md) и sources.jsonl уточняют каждый аспект.

[Точный перечень](expansion-inventory.json) содержит каждую пару source ID/путь ровно один раз. pilot_role и исходные boundaries сохраняют срез .005; current_index и current_role отражают .019; три аспекта покрытия хранятся в sources.jsonl. Приложение сопоставлено с реестром и sources.jsonl, в manifest не подключено и формат графа не меняет.
