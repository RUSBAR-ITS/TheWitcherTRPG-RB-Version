...
**Первая очередь:** .006–.011 выполнены 2026-09-15. Остальные области и их порядок остаются предложением; новая очередь ещё не поставлена. B01–B20 служат учёту остатка, а не являются двадцатью задачами. Исторические таблицы .005 сохранены; текущее покрытие указано отдельно.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). После .011 определения есть в 154 файлах: два словаря complete по строковым ключам, 152 файла partial; роли 60/94/461. Остались определения 461 файла и непокрытые области частичных файлов. Строковые ссылки config не означают полный разбор его правил. [Границы .011](coverage-011.md) и sources.jsonl уточняют каждый аспект.

[Точный перечень](expansion-inventory.json) содержит каждую пару source ID/путь ровно один раз. pilot_role и исходные boundaries сохраняют срез .005; current_index и current_role отражают .011; три аспекта покрытия хранятся в sources.jsonl. Приложение сопоставлено с реестром и sources.jsonl, в manifest не подключено и формат графа не меняет.
...
Остаток: алгоритмы боя/урона, магии, жизненного цикла Item и форм, валюты/IP/наград, ремесла, генератора/экспортов и другие частичные области. Это перечень для формирования следующей очереди, без нового приоритета или TASK. Два словаря завершены по строковым определениям; их произвольные читатели и внешние runtime overrides остаются границей.
...
Историческая проверка .005: перечень 615/615, роли 23/13/579 и 62 границы соответствовали пилоту. После .011 те же 615/615 путей, роли 60/94/461 и 214 границ; раздельные сведения в inventory. .006–.011 выполнены, остальные области требуют дальнейшей постановки. TASK-0006 остаётся in-progress.
