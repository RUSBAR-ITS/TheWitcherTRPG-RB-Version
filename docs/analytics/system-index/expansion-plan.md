...
**Первая очередь оформлена 2026-09-15 по поручению пользователя:** TASK-0006.006–TASK-0006.011, все planned. Остальные области и их порядок остаются предложением. B01–B20 служат учёту остатка, а не являются двадцатью задачами; крупные области потребуют нескольких порций. Состав ниже сохраняет срез приёмки .005 — наполнение ещё не началось.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). Остаток здесь относится к наполнению компактного индекса: 23 основных и 13 смежных файлов имеют частичный охват; 579 — только каталог. Внутри 36 файлов тоже остаются существенные области. Число 579 нельзя использовать как число «оставшихся задач» или считать остальные 36 полностью законченными.

[Точный перечень](expansion-inventory.json) содержит каждую из 615 пар source ID/путь ровно один раз, с ролью primary/neighbor/catalog_only и основной областью. Перечень сопоставлен с реестром и sources.jsonl. Это приложение к предложению; оно не подключено к manifest и не меняет формат графа. При расширении рабочего набора его роли и состав нужно пересмотреть по новому срезу.
...
## Остаток внутри 23 основных файлов
...
| --- | --- | ---: | --- |
| 1 | [TASK-0006.006 — Подключение документов и интерфейсов](../../tasks/task-0006.006.md) | 7 | planned |
| 2 | [TASK-0006.007 — Редактор ActiveEffect и мастер изменений](../../tasks/task-0006.007.md) | 6 | planned |
| 3 | [TASK-0006.008 — Категории, перенос и статусы эффектов](../../tasks/task-0006.008.md) | 8 | planned |
| 4 | [TASK-0006.009 — Текущие и старые входы навыка](../../tasks/task-0006.009.md) | 9 | planned |
| 5 | [TASK-0006.010 — Редактирование характеристик и навыков](../../tasks/task-0006.010.md) | 8 | planned |
...
Перечень 615/615, без пропусков и повторов; роли 23/13/579 совпадают с индексом. Все 62 граничных ID существуют. Первые шесть подзадач .006–.011 оформлены по поручению пользователя 2026-09-15; остальные требуют дальнейшей постановки. TASK-0006 остаётся in-progress.
