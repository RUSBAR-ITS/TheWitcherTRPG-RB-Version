...
**Текущее состояние:** .001–.020 выполнены; во [второй очереди](#second-wave) следующая — [TASK-0006.021](../../tasks/task-0006.021.md). B01–B20 служат учёту остатка. Исторические таблицы и срез постановки .012–.023 сохранены; текущие роли и фактическое покрытие приведены отдельно.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). После .020: Определения есть в 243/615 файлах: два словаря complete по строковым ключам, 241 файл partial; роли 136 основных/107 смежных, 372 без определений. Остались непокрытые области частичных файлов и определения 372 файлов. [Границы .020](coverage-020.md) и sources.jsonl уточняют каждый аспект.

[Точный перечень](expansion-inventory.json) содержит каждую пару source ID/путь ровно один раз. pilot_role и исходные boundaries сохраняют срез .005; current_index и current_role отражают .020; три аспекта покрытия хранятся в sources.jsonl. Приложение сопоставлено с реестром и sources.jsonl, в manifest не подключено и формат графа не меняет.
