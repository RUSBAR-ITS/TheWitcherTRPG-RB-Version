...
**Текущее состояние:** .001–.016 выполнены; во [второй очереди](#second-wave) следующая — [TASK-0006.017](../../tasks/task-0006.017.md). B01–B20 служат учёту остатка. Исторические таблицы и срез постановки .012–.023 сохранены; текущие роли и фактическое покрытие приведены отдельно.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). После .016: Определения есть в 224/615 файлах: два словаря complete по строковым ключам, 222 файла partial; роли 103 основных/121 смежный, 391 без определений. Остались непокрытые области частичных файлов и определения 391 файла. [Границы .016](coverage-016.md) и sources.jsonl уточняют каждый аспект.

[Точный перечень](expansion-inventory.json) содержит каждую пару source ID/путь ровно один раз. pilot_role и исходные boundaries сохраняют срез .005; current_index и current_role отражают .016; три аспекта покрытия хранятся в sources.jsonl. Приложение сопоставлено с реестром и sources.jsonl, в manifest не подключено и формат графа не меняет.
...
| [TASK-0006.017](../../tasks/task-0006.017.md) | Бросок урона и контракты боевых сообщений | 8 | .016 | `planned` |
