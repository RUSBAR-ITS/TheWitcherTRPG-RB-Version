...
**Это предложение для согласования.** Следующие TASK не созданы, приоритеты и размеры порций не утверждены. Области B01–B20 служат учёту остатка, а не являются двадцатью задачами. В крупных областях потребуется несколько порций.
...
## Предлагаемые первые порции

Номера ниже — порядок обсуждения, не TASK ID. Размер относится к основным адресам; уже индексированные модели/методы используются как смежные источники без дублирования ID.
...
Перечень 615/615, без пропусков и повторов; роли 23/13/579 совпадают с индексом. Все 62 граничных ID существуют. Следующие подзадачи будут сформированы после согласования этого предложения. TASK-0006 остаётся in-progress.
