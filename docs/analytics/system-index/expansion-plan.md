...
**Текущее состояние:** .001–.017 выполнены; во [второй очереди](#second-wave) следующая — [TASK-0006.018](../../tasks/task-0006.018.md). B01–B20 служат учёту остатка. Исторические таблицы и срез постановки .012–.023 сохранены; текущие роли и фактическое покрытие приведены отдельно.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). После .017: Определения есть в 227/615 файлах: два словаря complete по строковым ключам, 225 файлов partial; роли 111 основных/116 смежных, 388 без определений. Остались непокрытые области частичных файлов и определения 388 файлов. [Границы .017](coverage-017.md) и sources.jsonl уточняют каждый аспект.

[Точный перечень](expansion-inventory.json) содержит каждую пару source ID/путь ровно один раз. pilot_role и исходные boundaries сохраняют срез .005; current_index и current_role отражают .017; три аспекта покрытия хранятся в sources.jsonl. Приложение сопоставлено с реестром и sources.jsonl, в manifest не подключено и формат графа не меняет.
...
| [TASK-0006.018](../../tasks/task-0006.018.md) | Броня, сопротивления и износ SP | 8 | .017 | `planned` |
