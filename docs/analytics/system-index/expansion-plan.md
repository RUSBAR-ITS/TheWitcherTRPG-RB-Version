...
**Текущее состояние:** .006–.012 выполнены; во [второй очереди](#second-wave) следующая — .013. B01–B20 служат учёту остатка. Исторические таблицы .005 и срез постановки .012–.023 сохранены; фактическое покрытие приведено отдельно.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). После .012: Определения есть в 179/615 файлах: два словаря complete по строковым ключам, 177 файлов partial; роли 65 основных/114 смежных, 436 без определений. Остались непокрытые области частичных файлов и определения 436 файлов. [Границы .012](coverage-012.md) и sources.jsonl уточняют каждый аспект.

[Точный перечень](expansion-inventory.json) содержит каждую пару source ID/путь ровно один раз. pilot_role и исходные boundaries сохраняют срез .005; current_index и current_role отражают .012; три аспекта покрытия хранятся в sources.jsonl. Приложение сопоставлено с реестром и sources.jsonl, в manifest не подключено и формат графа не меняет.
...
| [TASK-0006.013](../../tasks/task-0006.013.md) | Инвентарь, количество и использование расходников | 10 | .012 | `planned` |
