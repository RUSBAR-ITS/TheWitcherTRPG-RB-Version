...
**Текущее состояние:** .006–.013 выполнены; во [второй очереди](#second-wave) следующая — .014. B01–B20 служат учёту остатка. Исторические таблицы и срез постановки .012–.023 сохранены; фактическое покрытие приведено отдельно.

## Что осталось

Все 615 файлов уже описаны в [аудите](../code-audit/registry.md). После .013: Определения есть в 197/615 файлах: два словаря complete по строковым ключам, 195 файлов partial; роли 74 основных/123 смежных, 418 без определений. Остались непокрытые области частичных файлов и определения 418 файлов. [Границы .013](coverage-013.md) и sources.jsonl уточняют каждый аспект.

[Точный перечень](expansion-inventory.json) содержит каждую пару source ID/путь ровно один раз. pilot_role и исходные boundaries сохраняют срез .005; current_index и current_role отражают .013; три аспекта покрытия хранятся в sources.jsonl. Приложение сопоставлено с реестром и sources.jsonl, в manifest не подключено и формат графа не меняет.
...
| [TASK-0006.016](../../tasks/task-0006.016.md) | Защита, критический результат и выбор локации | 10 | .015 | `planned` |
