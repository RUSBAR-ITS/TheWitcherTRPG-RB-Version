...
| Тип файла | JSON: экспорт RollTable; 3 TableResult (0 text, 3 document) |
| Статус анализа | Проверено |
| Дата проверки | 2026-09-16: актуализация технических обращений по issue-00001 |
| Ветка и коммит | `dev`, база `d8e0e1ad1159cb71a8769b0353f812de6f1ed4d8` + незакоммиченное исправление issue-00001 |
| Изменения относительно коммита | issue-00001; текущие технические обращения актуализированы. Прежние опыты ниже относятся к своим датам. |
| Задача и порция | [TASK-0003.056](../../../../../tasks/task-0003.056.md); 13 файлов / 2435 строк; этот файл — 102 строк |
| Запись перекрёстной сверки | [TASK-0003.056](../../../review-log.md#task-0003056) |
| SHA-256 файла | 6d1f15ea1c8c74932bebf3db7106ad86b1c400b08cce5cba50f25d4043235e11 |
...
Выбирает удачное, неудачное или романтическое событие жизни. Имя документа — `Life Event Generator`. Результат работы — текстовые TableResult и сообщение; создание персонажа как Actor, его вещей или ActiveEffect этим файлом не реализовано.

## Условия использования

[system.json](../../../../../../system.json):32–38,58–63 регистрирует Character-gen как RollTable в Character Generation, путь packs/character-generator.db. UUID данного документа — `Compendium.TheWitcherTRPG-RB-Version.Character-gen.RollTable.4Y8IpS3ArbbP2gGc`. JSON является экспортом данных, не браузерным модулем.
...
| --- | --- | --- |
| name / _id / _key | `Life Event Generator`; `4Y8IpS3ArbbP2gGc`; `!tables!4Y8IpS3ArbbP2gGc` | Название, ID и ключ хранения RollTable |
| results | 3 записей, полный перечень ниже | Локальные ID уникальны у родителя; совпадение с ID другого родителя допустимо |
| formula / replacement / displayRoll | `1d10` / true / false | Выбор по диапазонам, повторение результатов и видимость основного броска в сообщении этой таблицы |
| img | `icons/sundries/gaming/dice-runed-brown.webp` | Корневая иконка; путь найден в public установленного Foundry |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.gUOTqBlrFsBrxz1G`; остальные четыре поля null | История происхождения; roll следует documentUuid, не compendiumSource |
...
| Result _id / _key | Перечень ID ниже; `!tables.results!4Y8IpS3ArbbP2gGc.<id>` | Ключ экспорта; _key не является полем модели Foundry |
...
Описание корня: `<p>Roll once for every full 10 years you've been alive.</p>`
...
| --- | --- | --- | --- | --- | --- |
| 1 | `fEC34SOBkzdDBUtx` | [1,4] | document | `Fortune` | 10–33 |
| 2 | `nzF8eW0BGLhTGpdM` | [5,7] | document | `Misfortune` | 34–57 |
| 3 | `fnoSJmCrZPrgneXp` | [8,10] | document | `Romance` | 58–81 |

На 1–4 вызывается Fortune (40%), на 5–7 Misfortune (30%), на 8–10 Romance (30%). Fortune or Misfortune и Allies and Enemies не вызываются: это отдельные таблицы lifepath. Description просит повторять за каждые полные 10 лет жизни, но возраст не читается и цикла нет. Встроенных [[…]] у этого JSON нет, однако итоговые descriptions Fortune/Misfortune содержат такие вставки и вычисляются при показе чата. При Romantic Tragedy/Problematic Love вложенный Romance добавляет второй текст; остальные пути дают один.
...
| --- | --- | --- |
| 1–4 | 4/10 | `fEC34SOBkzdDBUtx` |
| 5–7 | 3/10 | `nzF8eW0BGLhTGpdM` |
| 8–10 | 3/10 | `fnoSJmCrZPrgneXp` |
...
| RollTable.normalize({save:false}) | Единичные weight → клон с 1d3 и одиночными диапазонами | Может разрушить составные выдачи и исходные вероятности; при заданной formula автоматически не вызывается |
...
Статический граф этого входа без ограничения ядра: 1–2 конечных text, глубина до 2. Все ветви данного прямого входа укладываются в предел _depth≤5; дополнительные внешние уровни нужно считать отдельно. Эти границы получены обходом диапазонов и ссылок, не утверждением о переборе всех случайных сочетаний во время исполнения.
...
| --- | --- | --- | --- |
| `fEC34SOBkzdDBUtx`; [1,4] | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.Z0eeWQI3R8v4YNLd` | [packsJson/lifepath/Fortune_Z0eeWQI3R8v4YNLd.json](../../../../../../packsJson/lifepath/Fortune_Z0eeWQI3R8v4YNLd.json); [карточка](../lifepath/Fortune_Z0eeWQI3R8v4YNLd.json.md) | documentUuid:30; имя/ID/тип сверены |
| `nzF8eW0BGLhTGpdM`; [5,7] | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.JNbvihde5EGIFPdB` | [packsJson/lifepath/Misfortune_JNbvihde5EGIFPdB.json](../../../../../../packsJson/lifepath/Misfortune_JNbvihde5EGIFPdB.json); [карточка](../lifepath/Misfortune_JNbvihde5EGIFPdB.json.md) | documentUuid:54; имя/ID/тип сверены |
| `fnoSJmCrZPrgneXp`; [8,10] | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.CDgdx129wZvINn16` | [packsJson/lifepath/Romance_CDgdx129wZvINn16.json](../../../../../../packsJson/lifepath/Romance_CDgdx129wZvINn16.json); [карточка](../lifepath/Romance_CDgdx129wZvINn16.json.md) | documentUuid:78; имя/ID/тип сверены |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Life Event Generator` | Общий поиск при экспорте добычи ожидает Item; этот граф выдаёт текст |
...
| --- | --- | --- | --- |
| Полный файл и схема | Строгий Python JSON, реальные модели ядра | 3 results, все поля и resource paths; имя/ID/UUID согласованы | Экспорт не сравнивался с DB |
...
[issue-00319](../../../../../issues/closed/issue-00319.md) — неверное значение x10/x100 в денежных inline-вставках нижележащих таблиц; в этом файле таких формул нет.
