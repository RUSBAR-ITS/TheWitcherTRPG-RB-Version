# packsJson/character-generator/Witcher_Lifepath__Non_Neutral_Decade_obLsbuoNixBUbeAy.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator/Witcher_Lifepath__Non_Neutral_Decade_obLsbuoNixBUbeAy.json](../../../../../../packsJson/character-generator/Witcher_Lifepath__Non_Neutral_Decade_obLsbuoNixBUbeAy.json) |
...
| SHA-256 файла | 9979f01401cba503014f360d2974d7527a441aefb40f6997fd99aae7ecdb1c26 |
...
Выбирает опасность десятилетия с отказом от нейтралитета и всегда добавляет его исход. Имя документа — `Witcher Lifepath: Non-Neutral Decade`. Результат работы — текстовые TableResult и сообщение; создание персонажа как Actor, его вещей или ActiveEffect этим файлом не реализовано.

## Условия использования

[system.json](../../../../../../system.json):32–38,58–63 регистрирует Character-gen как RollTable в Character Generation, путь packs/character-generator.db. UUID данного документа — `Compendium.TheWitcherTRPG-RB-Version.Character-gen.RollTable.obLsbuoNixBUbeAy`. JSON является экспортом данных, не браузерным модулем.
...
| --- | --- | --- |
| name / _id / _key | `Witcher Lifepath: Non-Neutral Decade`; `obLsbuoNixBUbeAy`; `!tables!obLsbuoNixBUbeAy` | Название, ID и ключ хранения RollTable |
...
| img | `icons/environment/settlement/city-night.webp` | Корневая иконка; путь найден в public установленного Foundry |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.8FIAJdS7J8B9Xtil`; остальные четыре поля null | История происхождения; roll следует documentUuid, не compendiumSource |
...
| Result _id / _key | Перечень ID ниже; `!tables.results!obLsbuoNixBUbeAy.<id>` | Ключ экспорта; _key не является полем модели Foundry |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `UdHUplxRtBUmcUsy` | [1,50] | text | `No danger occurred in this decade.` | 10–32 |
| 2 | `5k0jxtIvzmH6UNl5` | [1,50] | document | `Witcher Lifepath: Non-Neutral Outcome` | 33–56 |
| 3 | `gnrDCDMdILjGyQdl` | [51,100] | text | `During this decade, a danger occurred:` | 57–79 |
| 4 | `YVCSNqm0kMcDxKF3` | [51,100] | document | `Witcher Lifepath: Dangers` | 80–103 |
| 5 | `OAW3DJizEEzA919s` | [51,100] | document | `Witcher Lifepath: Non-Neutral Outcome` | 104–127 |

На 1–50 выводится No danger и Non-Neutral Outcome; на 51–100 — вводный текст опасности, Dangers и Non-Neutral Outcome. Вероятность опасности 50%; Outcome не заменяется опасностью. Проверены в том числе 50/51 и оба предела. Нейтралитет Actor не читается и не записывается; имя таблицы обозначает выбор пользователя.
...
| --- | --- | --- |
| 1–50 | 50/100 | `UdHUplxRtBUmcUsy`, `5k0jxtIvzmH6UNl5` |
| 51–100 | 50/100 | `gnrDCDMdILjGyQdl`, `YVCSNqm0kMcDxKF3`, `OAW3DJizEEzA919s` |
...
| --- | --- | --- | --- |
| `5k0jxtIvzmH6UNl5`; [1,50] | `Compendium.TheWitcherTRPG-RB-Version.Witcher_Lifepath_and_BG_Sub-tables.RollTable.sARR2vzegIiAh3uU` | [packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json); [карточка](../witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json.md) | documentUuid:53; имя/ID/тип сверены |
| `YVCSNqm0kMcDxKF3`; [51,100] | `Compendium.TheWitcherTRPG-RB-Version.Witcher_Lifepath_and_BG_Sub-tables.RollTable.QDAhRAIL7LNz9gME` | [packsJson/witcher-lifepath/Witcher_Lifepath__Dangers_QDAhRAIL7LNz9gME.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Dangers_QDAhRAIL7LNz9gME.json); [карточка](../witcher-lifepath/Witcher_Lifepath__Dangers_QDAhRAIL7LNz9gME.json.md) | documentUuid:100; имя/ID/тип сверены |
| `OAW3DJizEEzA919s`; [51,100] | `Compendium.TheWitcherTRPG-RB-Version.Witcher_Lifepath_and_BG_Sub-tables.RollTable.sARR2vzegIiAh3uU` | [packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json); [карточка](../witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json.md) | documentUuid:124; имя/ID/тип сверены |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Lifepath: Non-Neutral Decade` | Общий поиск при экспорте добычи ожидает Item; этот граф выдаёт текст |
...
Witcher Lifepath: Non-Neutral Decade: 1d100, возможные totals 1…100; 5 результатов (2 text / 3 document), replacement=true, displayRoll=true. Рекурсивные переходы: Witcher Lifepath: Non-Neutral Outcome; Witcher Lifepath: Dangers. По разрешимому графу от этого прямого входа 2–18 конечных текстов, максимальная глубина 4; входящих файлов 0. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json](../witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json.md), [packsJson/witcher-lifepath/Witcher_Lifepath__Dangers_QDAhRAIL7LNz9gME.json](../witcher-lifepath/Witcher_Lifepath__Dangers_QDAhRAIL7LNz9gME.json.md).
