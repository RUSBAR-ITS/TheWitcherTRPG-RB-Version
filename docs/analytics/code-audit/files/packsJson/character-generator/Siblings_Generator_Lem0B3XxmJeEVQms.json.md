# packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json
...
Собирает описание одного брата или сестры из четырёх подтаблиц. Имя документа — `Siblings Generator`. Результат работы — текстовые TableResult и сообщение; создание персонажа как Actor, его вещей или ActiveEffect этим файлом не реализовано.
...
| --- | --- | --- |
| name / _id / _key | `Siblings Generator`; `Lem0B3XxmJeEVQms`; `!tables!Lem0B3XxmJeEVQms` | Название, ID и ключ хранения RollTable |
| results | 4 записей, полный перечень ниже | Локальные ID уникальны у родителя; совпадение с ID другого родителя допустимо |
...
| img | `icons/environment/people/group.webp` | Корневая иконка; путь найден в public установленного Foundry |
| description | Полный текст ниже | Отдельное описание таблицы, не дополнительный TableResult |
| flags | `{"better-rolltables":{"table-type":"none"},"core":{}}` | Поля сохранены моделью; параметры better-rolltables не исполняются стандартным RollTable.roll |
| ownership / folder / sort | default=0; 2w1nWBDVMXqVbciv=3; null; 0 | Сохранённые права и расположение |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.Rgx1vfQgy82DVGXx`; остальные четыре поля null | История происхождения; roll следует documentUuid, не compendiumSource |
...
| Result _id / _key | Перечень ID ниже; `!tables.results!Lem0B3XxmJeEVQms.<id>` | Ключ экспорта; _key не является полем модели Foundry |
...
Описание корня: `<p>You have a sibling that is:</p>`
...
| --- | --- | --- | --- | --- | --- |
| 1 | `lHDeQbKOzEyvSCTK` | [1,1] | document | `Siblings: Gender` | 12–35 |
| 2 | `6wmO1a81DMV7foRw` | [1,1] | document | `Siblings: Age` | 36–59 |
| 3 | `x8daSlTDdqBl39P0` | [1,1] | document | `Siblings: Feelings About You` | 60–83 |
| 4 | `ZHa5PBGJnRAbdzZr` | [1,1] | document | `Siblings: Personality` | 84–107 |

Все четыре [1,1] при formula=1d1 выбираются вместе и в порядке Gender → Age → Feelings About You → Personality. Получается описание одного родственника из четырёх текстов. Корневой description «You have a sibling that is:» показан дополнительно и не является пятой записью results. Таблицы количества братьев/сестёр выдают текстовые числа и не содержат обратной ссылки на этот генератор; вызовы нужное число раз требуют внешнего действия.
...
| --- | --- | --- |
| 1–1 | 1/1 | `lHDeQbKOzEyvSCTK`, `6wmO1a81DMV7foRw`, `x8daSlTDdqBl39P0`, `ZHa5PBGJnRAbdzZr` |
...
| RollTable.normalize({save:false}) | Единичные weight → клон с 1d4 и одиночными диапазонами | Может разрушить составные выдачи и исходные вероятности; при заданной formula автоматически не вызывается |
...
Статический граф этого входа без ограничения ядра: 4–4 конечных text, глубина до 1. Все ветви данного прямого входа укладываются в предел _depth≤5; дополнительные внешние уровни нужно считать отдельно. Эти границы получены обходом диапазонов и ссылок, не утверждением о переборе всех случайных сочетаний во время исполнения.
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Siblings Generator` | Общий поиск при экспорте добычи ожидает Item; этот граф выдаёт текст |
...
| --- | --- | --- | --- |
| Полный файл и схема | Строгий Python JSON, реальные модели ядра | 4 results, все поля и resource paths; имя/ID/UUID согласованы | Экспорт не сравнивался с DB |
...
Siblings Generator: 1d1, возможные totals 1…1; 4 результатов (0 text / 4 document), replacement=true, displayRoll=false. Рекурсивные переходы: Siblings: Gender; Siblings: Age; Siblings: Feelings About You; Siblings: Personality. По разрешимому графу от этого прямого входа 4–4 конечных текстов, максимальная глубина 1; входящих файлов 0. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator-sub-tables/Siblings__Gender_QbkrG0W11fREICK8.json](../character-generator-sub-tables/Siblings__Gender_QbkrG0W11fREICK8.json.md), [packsJson/character-generator-sub-tables/Siblings__Age_0PcW1kFO5g40cELc.json](../character-generator-sub-tables/Siblings__Age_0PcW1kFO5g40cELc.json.md), [packsJson/character-generator-sub-tables/Siblings__Feelings_About_You_ACf1JfCfVtJWw5DG.json](../character-generator-sub-tables/Siblings__Feelings_About_You_ACf1JfCfVtJWw5DG.json.md), [packsJson/character-generator-sub-tables/Siblings__Personality_zNQCbwyK1biYFszn.json](../character-generator-sub-tables/Siblings__Personality_zNQCbwyK1biYFszn.json.md), [module/data/item/homelandData.js](../../module/data/item/homelandData.js.md), [module/data/item/professionData.js](../../module/data/item/professionData.js.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).
