# packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Closeness_TLO7kA1RyxfxP2D8.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Closeness_TLO7kA1RyxfxP2D8.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Closeness_TLO7kA1RyxfxP2D8.json) |
...
| Задача и порция | [TASK-0003.055](../../../../../tasks/task-0003.055.md); 41 файл / 8708 строк; этот файл — 99 строк |
| Запись перекрёстной сверки | [TASK-0003.055](../../../review-log.md#task-0003055) |
| SHA-256 файла | 8645e3fc900004779840b6d568f64aa671d987ee9ae17dea0ff59fddf2d40bc7 |

## Назначение файла

Выбирает степень близости с союзником. Имя документа — `Witcher Lifepath: Allies - Closeness`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):36,71–76 регистрирует RollTable-пакет Witcher_Lifepath_and_BG_Sub-tables в папке Character Generation с путём packs/witcher-lifepath.db. Адрес документа — `Compendium.TheWitcherTRPG.Witcher_Lifepath_and_BG_Sub-tables.RollTable.TLO7kA1RyxfxP2D8`.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Lifepath: Allies - Closeness`; `TLO7kA1RyxfxP2D8`; `!tables!TLO7kA1RyxfxP2D8` | Имя, ID и ключ хранения корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.2X2WBXiU7SelRfuK`; systemId/systemVersion/duplicateSource/exportSource=null | Метаданные происхождения; не ссылка для рекурсивного выбора |
...
| Result _id/_key | ID в перечне; `!tables.results!TLO7kA1RyxfxP2D8.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,6] | text | `Closeness To Your Ally: Acquaintances` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [7,9] | text | `Closeness To Your Ally: Friends` | 33–55 |
| 3 | `9kVdEtFeX4a3nAoe` | [10,10] | text | `Closeness To Your Ally: Bound By Bond` | 56–78 |

Вероятности Acquaintances/Friends/Bound By Bond — 60/30/10%; отдельного числового параметра отношений не создаётся.
...
| --- | --- | --- |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Generator_47kWQhpq3yeAG74c.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Generator_47kWQhpq3yeAG74c.json) | `KgpE0Q8gz4x0tE9A`; documentUuid:102; range=[1,1] | `Witcher Lifepath: Allies - Generator`: при выборе диапазона и recursive:true вызывает этот документ |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Lifepath: Allies - Closeness` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
...
Witcher Lifepath: Allies - Closeness: 1d10, возможные totals 1…10; 3 результатов (3 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
