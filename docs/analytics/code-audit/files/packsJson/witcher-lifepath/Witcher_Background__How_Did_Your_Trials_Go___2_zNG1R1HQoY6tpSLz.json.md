# packsJson/witcher-lifepath/Witcher_Background__How_Did_Your_Trials_Go___2_zNG1R1HQoY6tpSLz.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/witcher-lifepath/Witcher_Background__How_Did_Your_Trials_Go___2_zNG1R1HQoY6tpSLz.json](../../../../../../packsJson/witcher-lifepath/Witcher_Background__How_Did_Your_Trials_Go___2_zNG1R1HQoY6tpSLz.json) |
...
| SHA-256 файла | 4d0ccf06db2b94ad82e18dd07d945cf4af3a943a1c52bff2abc0cfa56c5ba4ff |
...
[system.json](../../../../../../system.json):36,71–76 регистрирует RollTable-пакет Witcher_Lifepath_and_BG_Sub-tables в папке Character Generation с путём packs/witcher-lifepath.db. Адрес документа — `Compendium.TheWitcherTRPG-RB-Version.Witcher_Lifepath_and_BG_Sub-tables.RollTable.zNG1R1HQoY6tpSLz`.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Background: How Did Your Trials Go? +2`; `zNG1R1HQoY6tpSLz`; `!tables!zNG1R1HQoY6tpSLz` | Имя, ID и ключ хранения корневого RollTable |
...
| Result _id/_key | ID в перечне; `!tables.results!zNG1R1HQoY6tpSLz.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| 2 | `g8M6NpFm7tM6Vxit` | [-3,1] | document | `Witcher Background: What Was Your Most Important Event` | 33–56 |
...
| 4 | `PetWGxxqNcQ9CTY8` | [2,3] | document | `Witcher Background: What Was Your Most Important Event` | 80–103 |
...
| 6 | `pBJUmRRDkPudGBWz` | [4,9] | document | `Witcher Background: What Was Your Most Important Event` | 127–150 |
...
| 8 | `ErK1v2gFZ5OEch8Z` | [10,14] | document | `Witcher Background: What Was Your Most Important Event` | 174–197 |
...
Реальная formula даёт значения 3…12. Для каждого диапазона указана доля граней основного кубика; строки с одинаковым диапазоном выбираются вместе, поэтому это вероятность группы, а не деление между её записями.

| range | Одновременно выбранных записей | Доля граней | Результат |
| --- | --- | --- | --- |
| [-3,1] | 2 | 0/10 | Не выбирается штатной formula |
| [2,3] | 2 | 1/10 | text + document |
| [4,9] | 2 | 6/10 | text + document |
| [10,14] | 2 | 3/10 | text + document |
...
| Диапазоны и формулы | Настоящие Roll, RollTable.roll/getResultsForRoll | Все 10 граней этой formula; ожидаемые ID независимо выбраны по raw range; минимум 3, максимум 12 | Не перебраны все комбинации вложенных случайных чисел |
...
Witcher Background: How Did Your Trials Go? +2: 1d10+2, возможные totals 3…12; 8 результатов (4 text / 4 document), replacement=true, displayRoll=false. Рекурсивные переходы: Witcher Background: What Was Your Most Important Event. По разрешимому графу от этого прямого входа 3–3 конечных текстов, максимальная глубина 2; входящих файлов 1. Inline-выражений в результатах нет.  Штатной формулой недостижимы results[0, 1]; это общие диапазоны сдвинутых Trials, не 16 переходов из 00321.
