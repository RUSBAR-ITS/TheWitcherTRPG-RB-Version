# packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__Are_They_Alive__eywzppppagpC4HYO.json
...
Определяет, жив ли враг; при смерти добавляет причину и встроенный бросок прошедших десятилетий. Имя документа — `Witcher Lifepath: Danger - Enemies: Are They Alive?`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Lifepath: Danger - Enemies: Are They Alive?`; `eywzppppagpC4HYO`; `!tables!eywzppppagpC4HYO` | Имя, ID и ключ хранения корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.5SMk1LldZEW7ymdp`; systemId/systemVersion/duplicateSource/exportSource=null | Метаданные происхождения; не ссылка для рекурсивного выбора |
...
| Result _id/_key | ID в перечне; `!tables.results!eywzppppagpC4HYO.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,30] | text | `Enemy - Are They Alive? Your enemy died:` | 10–32 |
| 2 | `kQX7ZeIIWKy5yTPF` | [1,30] | document | `Witcher Lifepath: Danger - Enemies: How Did They Die?` | 33–56 |
| 3 | `oKZeEBIq2FLMNk2e` | [31,100] | text | `Enemy - Are They Alive? Your enemy is still alive.` | 57–79 |

При 1–30 выбираются два результата, включая How Did They Die?; при 31–100 один. Число десятилетий из описания вложенной причины генерируется при подготовке HTML, не при roll(). Возраст/календарь не учитываются.
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Lifepath: Danger - Enemies: Are They Alive?` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
...
Witcher Lifepath: Danger - Enemies: Are They Alive?: 1d100, возможные totals 1…100; 3 результатов (2 text / 1 document), replacement=true, displayRoll=false. Рекурсивные переходы: Witcher Lifepath: Danger - Enemies: How Did They Die?. По разрешимому графу от этого прямого входа 1–2 конечных текстов, максимальная глубина 1; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies_Generator_iwGZ1Sj0v9iBqvSk.json](Witcher_Lifepath__Danger___Enemies_Generator_iwGZ1Sj0v9iBqvSk.json.md), [packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__How_Did_They_Die__SwPXh5SRHrwfXyWP.json](Witcher_Lifepath__Danger___Enemies__How_Did_They_Die__SwPXh5SRHrwfXyWP.json.md).
