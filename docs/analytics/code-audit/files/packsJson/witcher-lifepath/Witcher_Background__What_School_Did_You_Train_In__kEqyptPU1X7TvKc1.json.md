# packsJson/witcher-lifepath/Witcher_Background__What_School_Did_You_Train_In__kEqyptPU1X7TvKc1.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/witcher-lifepath/Witcher_Background__What_School_Did_You_Train_In__kEqyptPU1X7TvKc1.json](../../../../../../packsJson/witcher-lifepath/Witcher_Background__What_School_Did_You_Train_In__kEqyptPU1X7TvKc1.json) |
...
| Задача и порция | [TASK-0003.055](../../../../../tasks/task-0003.055.md); 41 файл / 8708 строк; этот файл — 145 строк |
| Запись перекрёстной сверки | [TASK-0003.055](../../../review-log.md#task-0003055) |
| SHA-256 файла | 0a64e5eb95e43e05cb676da9ef3fc8bbfeebb86bf66fd78403ec61300035d217 |

## Назначение файла

Выбирает текст школы ведьмака из пяти вариантов с описаниями особенностей. Имя документа — `Witcher Background: What School Did You Train In?`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):36,71–76 регистрирует RollTable-пакет Witcher_Lifepath_and_BG_Sub-tables в папке Character Generation с путём packs/witcher-lifepath.db. Адрес документа — `Compendium.TheWitcherTRPG.Witcher_Lifepath_and_BG_Sub-tables.RollTable.kEqyptPU1X7TvKc1`.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Background: What School Did You Train In?`; `kEqyptPU1X7TvKc1`; `!tables!kEqyptPU1X7TvKc1` | Имя, ID и ключ хранения корневого RollTable |
| results | 5 вложенных документов | Локальные ID уникальны у этого родителя; совпадение ID в другой таблице допустимо |
...
| Result _id/_key | ID в перечне; `!tables.results!kEqyptPU1X7TvKc1.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,2] | text | `What School Did You Train In? - The Wolf School (No Penalty For Strong Strikes) You trained at Kaer Morhen in the heights of the Blue Mountains. Your training was tough and structured, focusing on a very rounded approach to the Witcher profession. You were taught to strike hard and fast to end hunts quickly.` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [3,4] | text | `What School Did You Train In? - The Gryphon School (+2 Vigor Threshold) You were trained at Kaer Y Seren along the coastal side of the Dragon Mountains. Your training was heavily focused on fighting any number of opponents and using your limited magical power to its greatest potential.` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [5,6] | text | `What School Did You Train In? - The Cat School (Immune to Non-Magical Charm Attempts) You trained in the Dyn Marv Caravan, a traveling troop of witchers who sold their skills to anyone who could pay, for any job. Their mutations and training flayed your emotions, and you struggle against violent, cruel impulses.` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [7,8] | text | `What School Did You Train In? - The Viper School (No Penalties for Dual Wielding) You trained at Gorthwr Gvaed in the deep chasms of the Tir Tochair Mountains. Unlike other witchers, you were trained on twin blades and an assassination-based approach to killing monsters.` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [9,10] | text | `What School Did You Train In? - The Bear School (-2 to Overall Armor Penalty) You trained in the snowy heights of the Amell Mountains at Haern Cadvch. You conditioned your body to endure all manner of punishment and move quickly and efficiently in heavy steel armor.` | 102–124 |

Пять диапазонов шириной 2 дают по 20%: Wolf, Gryphon, Cat, Viper, Bear. Текстовые «No Penalty», +2 Vigor и −2 Armor Penalty не записываются в Actor.lifepathModifiers или ActiveEffect. В Witcher Background Generator эта таблица вызывается отдельной ветвью при любом из трёх возрастных исходов; школа не выбирает вариант Trials.
...
| RollTable.normalize({save:false}) | Создаёт клон с 1d5 и одиночными диапазонами по 5 единичным весам | Может изменить исходные вероятности и одновременный выбор; у этого экспорта formula задана, автоматическая нормализация в roll не требуется |
...
| --- | --- | --- |
| [packsJson/character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json](../../../../../../packsJson/character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json) | `qajkRaccfhCw3FmJ`; documentUuid:53; range=[1,2] | `Witcher Background Generator`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json](../../../../../../packsJson/character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json) | `I2WgdMaqvCOO86Cd`; documentUuid:124; range=[3,8] | `Witcher Background Generator`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json](../../../../../../packsJson/character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json) | `q8LCtgshNjOjac5P`; documentUuid:195; range=[9,10] | `Witcher Background Generator`: при выборе диапазона и recursive:true вызывает этот документ |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Background: What School Did You Train In?` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
...
| --- | --- | --- | --- |
| Все поля и ключи | Python stdin, строгий JSON без повторных ключей; отдельная сверка 41 файла | 321 result, 362 уникальных ключа хранения; этот файл: 5 results | Совпадение с установленной БД не проверено |
...
| --- | --- | --- |
| [Witcher Background Generator](../character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json.md) | `qajkRaccfhCw3FmJ` | 53; [1,2] |
| [Witcher Background Generator](../character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json.md) | `I2WgdMaqvCOO86Cd` | 124; [3,8] |
| [Witcher Background Generator](../character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json.md) | `q8LCtgshNjOjac5P` | 195; [9,10] |
...
Witcher Background: What School Did You Train In?: 1d10, возможные totals 1…10; 5 результатов (5 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json](../character-generator/Witcher_Background_Generator_C5d6zkIMvS9gDWEN.json.md), [module/data/actor/templates/character/general/backgroundData.js](../../module/data/actor/templates/character/general/backgroundData.js.md), [module/data/item/professionData.js](../../module/data/item/professionData.js.md).

[Протокол и границы](../../../review-log.md#task-0004017) — TASK-0004.017; процессы [R017-01](../../../cross-check-0002.md#r017-01), [R017-02](../../../cross-check-0002.md#r017-02), [R017-03](../../../cross-check-0002.md#r017-03), [R017-04](../../../cross-check-0002.md#r017-04), [R017-16](../../../cross-check-0002.md#r017-16). В этой порции выполнена статическая сверка; прежние опыты сохраняют свои даты и фасады. Новых поведенческих запусков нет; браузер, мир, сеть и запись в БД не запускались.
