# packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Position_zG6aQ2srMV79PDHY.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Position_zG6aQ2srMV79PDHY.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Position_zG6aQ2srMV79PDHY.json) |
...
| SHA-256 файла | 249641272fb153efd5ddc9744928bbe4371b9c970a2dab4a3108e24add24edb8 |

## Назначение файла

Выбирает положение или занятие союзника. Имя документа — `Witcher Lifepath: Allies - Position`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):36,71–76 регистрирует RollTable-пакет Witcher_Lifepath_and_BG_Sub-tables в папке Character Generation с путём packs/witcher-lifepath.db. Адрес документа — `Compendium.TheWitcherTRPG.Witcher_Lifepath_and_BG_Sub-tables.RollTable.zG6aQ2srMV79PDHY`.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Lifepath: Allies - Position`; `zG6aQ2srMV79PDHY`; `!tables!zG6aQ2srMV79PDHY` | Имя, ID и ключ хранения корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.hcMvZiWNjz8cQDc4`; systemId/systemVersion/duplicateSource/exportSource=null | Метаданные происхождения; не ссылка для рекурсивного выбора |
...
| Result _id/_key | ID в перечне; `!tables.results!zG6aQ2srMV79PDHY.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,1] | text | `Ally Position: A Bounty Hunter` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [2,2] | text | `Ally Position: A Mage` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [3,3] | text | `Ally Position: A Mentor or Teacher` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [4,4] | text | `Ally Position: A Childhood Friend` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [5,5] | text | `Ally Position: A Craftsman` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | [6,6] | text | `Ally Position: An Old Enemy` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | [7,7] | text | `Ally Position: A Duke/Duchess` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | [8,8] | text | `Ally Position: A Priest/Priestess` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | [9,9] | text | `Ally Position: A Soldier` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | [10,10] | text | `Ally Position: A Bard` | 217–239 |

Десять вариантов по 10%; имена занятий не являются ссылками на профессию Item.
...
| --- | --- | --- |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Generator_47kWQhpq3yeAG74c.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Generator_47kWQhpq3yeAG74c.json) | `mp0F0yfLeczgHYKB`; documentUuid:54; range=[1,1] | `Witcher Lifepath: Allies - Generator`: при выборе диапазона и recursive:true вызывает этот документ |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Lifepath: Allies - Position` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
...
Witcher Lifepath: Allies - Position: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
