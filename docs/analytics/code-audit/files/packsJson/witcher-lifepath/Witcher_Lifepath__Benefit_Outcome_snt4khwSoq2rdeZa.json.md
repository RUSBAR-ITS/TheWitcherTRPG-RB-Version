...
| SHA-256 файла | 2f3dec5b69dbfbd78c684cb4a21f50d23875b3692c120d7f0f46c2d205f2c10c |

## Назначение файла

Выбирает благоприятный исход десятилетия; часть текстов содержит вычисляемые вставки. Имя документа — `Witcher Lifepath: Benefit Outcome`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):36,71–76 регистрирует RollTable-пакет Witcher_Lifepath_and_BG_Sub-tables в папке Character Generation с путём packs/witcher-lifepath.db. Адрес документа — `Compendium.TheWitcherTRPG.Witcher_Lifepath_and_BG_Sub-tables.RollTable.snt4khwSoq2rdeZa`.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Lifepath: Benefit Outcome`; `snt4khwSoq2rdeZa`; `!tables!snt4khwSoq2rdeZa` | Имя, ID и ключ хранения корневого RollTable |
...
| Result _id/_key | ID в перечне; `!tables.results!snt4khwSoq2rdeZa.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,1] | text | `Law of Surprises - You invoked the Law of Surprises during that decade. Roll 1d10 [[1d10]] to see what you got in return. 1: a baby, 2: a dog, 3: a horse, 4: a new plow, 5: a cat, 6: a barrel of ale, 7: a piece of jewelery worth 1d6x10 [[1d6x10]] crowns, 8: a weapon worth up to 500 crowns, 9: an ox, 10: a mule.` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [2,2] | text | `Romance - You found a lover who saw past your mutations and desensitization. Somehow you managed to make a meaningful connection with a person. Roll 1d10 [[1d10]]. 1-6: it lasted a few weeks, 7-8: it lasted a few months, 9-10: it’s still going, on and off.` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [3,3] | text | `Windfall - You raked in a suprisingly large amount of coin that decade. You managed to not only pay for alchemy ingredients and repairs to your gear, but also put some coin aside for a legitimate savings. You gain 1d10x100 [[1d10x100]] crowns.` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [4,4] | text | `A Noble Owes You - You performed a task for a noble. It may have been legal, it may have been illegal—either way, the noble you helped out owes you big and knows you’ll come to collect someday. You can invoke this favor at any time but it must be reasonable (GM’s discretion).` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [5,5] | text | `Witcher Secrets Passed Down - Along your journeys you met with and traveled with another witcher. This witcher taught you and shared some long-lost knowledge. You gain a witcher diagram: a potion, oil, or decoction of your choice.` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | [6,6] | text | `Knighted For Valor - At some point that decade, you fought bravely to defend a country. You may have gone to protect someone or you may just have been in the right place at the right time. For this great deed, you were knighted by a king/queen. You gain +1 Reputation in one country of your choice.` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | [7,7] | text | `Fell in with Bandits - You fell in with a group of bandits or scoia’tael while on a hunt. You may not have agreed with their methods, but they didn’t bother you and you didn’t bother them. You even shared some drinks. You can ask them for a favor once a month as long as it’s reasonable (GM’s discretion).` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | [8,8] | text | `Explored a Ruin - You had to hunt a monster through a large and complex ruin. Along the way you found something useful. Roll 1d10 [[1d10]]. 1-2: elven enhancement, 3-4: elven messer, 5-6: dwarven enhancement, 7-8: gnomish hand crossbow, 9-10: dwarven cloak.` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | [9,9] | text | `A Mage Owes You - During this decade you did a favor for a mage. You may have gathered monster parts for their experiments, let them study you, or even captured a monster alive for them. Either way, the mage now owes you one favor in return as long as it’s reasonable (GM’s discretion).` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | [10,10] | text | `Found a Teacher - You studied under a mentor. You spent many weeks learning, practicing, and looking to your mentor for guidance. It was a strange experience. You may gain +1 in any INT skill or start a new INT skill at +2.` | 217–239 |

Десять текстов по 10%; 5 inline-вставок в 4 строках. Law of Surprises содержит сразу [[1d10]] и [[1d6x10]]: обе исполняются при выводе, даже если первый бросок не дал 7. Windfall содержит [[1d10x100]]. В реальном ядре x означает explode: на максимальных гранях денежные вставки дали 6 и 10, контрольные 1d6*10 и 1d10*100 — 60 и 1000. Это дополнительные места issue-00319. Получение предметов, репутации, навыков и услуг остаётся ручной интерпретацией текста.
...
Inline-вставки: `q0DM4FW7XCnFdKZu`: `[[1d10]]`; `q0DM4FW7XCnFdKZu`: `[[1d6x10]]`; `oKZeEBIq2FLMNk2e`: `[[1d10]]`; `kQX7ZeIIWKy5yTPF`: `[[1d10x100]]`; `tfUjaxwfTwqW98vG`: `[[1d10]]`. Всего 5; формулы валидны для настоящего парсера. При выводе исполняются все вставки выбранного description; roll и draw({displayChat:false}) их не вычисляют. Текстовые @UUID/@Compendium, внешние URL и исполняемые макросы в этом файле не найдены. Упоминания людей, предметов, монстров, правил и страниц сами по себе не являются зависимостями на документы.

## Известные потребители

| Файл / компонент | Запись / используемые данные | Условия |
| --- | --- | --- |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Cautious_Outcome_jhPNDSApv5lQlUk3.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Cautious_Outcome_jhPNDSApv5lQlUk3.json) | `26OatJZ2irFtE5Ly`; documentUuid:53; range=[1,1] | `Witcher Lifepath: Cautious Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json) | `26OatJZ2irFtE5Ly`; documentUuid:53; range=[1,2] | `Witcher Lifepath: Non-Neutral Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Normal_Outcome_Lu49KrUT3wDY1bJr.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Normal_Outcome_Lu49KrUT3wDY1bJr.json) | `26OatJZ2irFtE5Ly`; documentUuid:53; range=[1,1] | `Witcher Lifepath: Normal Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Risky_Outcome_R6BzlpXysvPx6O5c.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Risky_Outcome_R6BzlpXysvPx6O5c.json) | `26OatJZ2irFtE5Ly`; documentUuid:53; range=[1,5] | `Witcher Lifepath: Risky Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Lifepath: Benefit Outcome` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
