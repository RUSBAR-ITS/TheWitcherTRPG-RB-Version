# packsJson/witcher-lifepath/Witcher_Lifepath__Allies___Generator_47kWQhpq3yeAG74c.json
...
Собирает шесть аспектов союзника последовательными вложенными бросками. Имя документа — `Witcher Lifepath: Allies - Generator`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Lifepath: Allies - Generator`; `47kWQhpq3yeAG74c`; `!tables!47kWQhpq3yeAG74c` | Имя, ID и ключ хранения корневого RollTable |
| results | 6 вложенных документов | Локальные ID уникальны у этого родителя; совпадение ID в другой таблице допустимо |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.OmaVsbOv4sSJuRLI`; systemId/systemVersion/duplicateSource/exportSource=null | Метаданные происхождения; не ссылка для рекурсивного выбора |
...
| Result _id/_key | ID в перечне; `!tables.results!47kWQhpq3yeAG74c.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,1] | document | `Witcher Lifepath: Allies - Gender` | 10–33 |
| 2 | `mp0F0yfLeczgHYKB` | [1,1] | document | `Witcher Lifepath: Allies - Position` | 34–57 |
| 3 | `a90EHPhkX2hszHH8` | [1,1] | document | `Witcher Lifepath: Allies - How You Met` | 58–81 |
| 4 | `KgpE0Q8gz4x0tE9A` | [1,1] | document | `Witcher Lifepath: Allies - Closeness` | 82–105 |
| 5 | `ph4JxbZMB7MniZZ5` | [1,1] | document | `Witcher Lifepath: Allies - Where Are They?` | 106–129 |
| 6 | `CO3QQZcFp1D2YDGe` | [1,1] | document | `Witcher Lifepath: Allies - Are They Alive?` | 130–153 |

Шесть диапазонов [1,1] при 1d1 исполняются все: Gender → Position → How You Met → Closeness → Where Are They? → Are They Alive?. При живом союзнике получается 6 текстов, при умершем — 7, включая причину смерти. Слово Optional в Where Are They? не выключает её безусловную ссылку.
...
| --- | --- | --- | --- |
| [1,1] | 6 | 1/1 | document + document + document + document + document + document |
...
| RollTable.normalize({save:false}) | Создаёт клон с 1d6 и одиночными диапазонами по 6 единичным весам | Может изменить исходные вероятности и одновременный выбор; у этого экспорта formula задана, автоматическая нормализация в roll не требуется |
...
При прямом запуске с _depth=0 граф допускает 6–7 конечных text; максимальная внутренняя глубина — 2. Эти границы получены отдельным обходом всех диапазонов и ссылок; проверки исполнения перебирали каждый основной результат и выбранные вложенные сочетания, не все комбинации графа. Ядро отклоняет _depth>5. Witcher Background Generator даёт максимальную глубину 4, его ветка RandomCharacter — 5; эти входы проверены отдельно.
...
| --- | --- | --- |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Cautious_Outcome_jhPNDSApv5lQlUk3.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Cautious_Outcome_jhPNDSApv5lQlUk3.json) | `RVpgllc0boqo3hTN`; documentUuid:100; range=[2,2] | `Witcher Lifepath: Cautious Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Non_Neutral_Outcome_sARR2vzegIiAh3uU.json) | `RVpgllc0boqo3hTN`; documentUuid:100; range=[3,7] | `Witcher Lifepath: Non-Neutral Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Normal_Outcome_Lu49KrUT3wDY1bJr.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Normal_Outcome_Lu49KrUT3wDY1bJr.json) | `RVpgllc0boqo3hTN`; documentUuid:100; range=[2,2] | `Witcher Lifepath: Normal Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Risky_Outcome_R6BzlpXysvPx6O5c.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Risky_Outcome_R6BzlpXysvPx6O5c.json) | `RVpgllc0boqo3hTN`; documentUuid:100; range=[6,7] | `Witcher Lifepath: Risky Outcome`: при выборе диапазона и recursive:true вызывает этот документ |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Lifepath: Allies - Generator` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
...
| --- | --- | --- | --- |
| Все поля и ключи | Python stdin, строгий JSON без повторных ключей; отдельная сверка 41 файла | 321 result, 362 уникальных ключа хранения; этот файл: 6 results | Совпадение с установленной БД не проверено |
