# packsJson/witcher-lifepath/Witcher_Background__How_Did_Your_Trials_Go___2_CDX4hqsxcxxovYz3.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/witcher-lifepath/Witcher_Background__How_Did_Your_Trials_Go___2_CDX4hqsxcxxovYz3.json](../../../../../../packsJson/witcher-lifepath/Witcher_Background__How_Did_Your_Trials_Go___2_CDX4hqsxcxxovYz3.json) |
...
| SHA-256 файла | 0f8563fa11aaa8208e84599e1ad8a41b7e978a88b9a6be331d7843220bc40e0f |
...
Испытания ведьмака с формулой 1d10−2; выводит результат мутаций и продолжает важным событием. Имя документа — `Witcher Background: How Did Your Trials Go? -2`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):36,71–76 регистрирует RollTable-пакет Witcher_Lifepath_and_BG_Sub-tables в папке Character Generation с путём packs/witcher-lifepath.db. Адрес документа — `Compendium.TheWitcherTRPG-RB-Version.Witcher_Lifepath_and_BG_Sub-tables.RollTable.CDX4hqsxcxxovYz3`.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Background: How Did Your Trials Go? -2`; `CDX4hqsxcxxovYz3`; `!tables!CDX4hqsxcxxovYz3` | Имя, ID и ключ хранения корневого RollTable |
| results | 8 вложенных документов | Локальные ID уникальны у этого родителя; совпадение ID в другой таблице допустимо |
| formula, replacement, displayRoll | `1d10-2`, true, false | Формула выбора, повторное использование результатов, скрытый основной бросок в собственном сообщении |
...
| Result _id/_key | ID в перечне; `!tables.results!CDX4hqsxcxxovYz3.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| 2 | `bYxTODjNqz7aUYAb` | [-3,1] | document | `Witcher Background: What Was Your Most Important Event` | 33–56 |
...
| 4 | `QcVDT06FJBeWG9iX` | [2,3] | document | `Witcher Background: What Was Your Most Important Event` | 80–103 |
...
| 6 | `4USYOsTytUepe5tq` | [4,9] | document | `Witcher Background: What Was Your Most Important Event` | 127–150 |
...
| 8 | `MGrLuHzN2tBHy1tD` | [10,14] | document | `Witcher Background: What Was Your Most Important Event` | 174–197 |
...
Реальная formula даёт значения -1…8. Для каждого диапазона указана доля граней основного кубика; строки с одинаковым диапазоном выбираются вместе, поэтому это вероятность группы, а не деление между её записями.

| range | Одновременно выбранных записей | Доля граней | Результат |
| --- | --- | --- | --- |
| [-3,1] | 2 | 3/10 | text + document |
| [2,3] | 2 | 2/10 | text + document |
| [4,9] | 2 | 5/10 | text + document |
| [10,14] | 2 | 0/10 | Не выбирается штатной formula |
...
| RollTable.roll({roll, recursive}) | По умолчанию `1d10-2`; recursive=true раскрывает documentUuid через fromUuid и innerTable.roll | Родительские модификаторы и исходный roll во вложенный вызов не передаются; _depth увеличивается на 1 |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Background: How Did Your Trials Go? -2` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
...
| Диапазоны и формулы | Настоящие Roll, RollTable.roll/getResultsForRoll | Все 10 граней этой formula; ожидаемые ID независимо выбраны по raw range; минимум -1, максимум 8 | Не перебраны все комбинации вложенных случайных чисел |
...
Witcher Background: How Did Your Trials Go? -2: 1d10-2, возможные totals -1…8; 8 результатов (4 text / 4 document), replacement=true, displayRoll=false. Рекурсивные переходы: Witcher Background: What Was Your Most Important Event. По разрешимому графу от этого прямого входа 3–3 конечных текстов, максимальная глубина 2; входящих файлов 1. Inline-выражений в результатах нет.  Штатной формулой недостижимы results[6, 7]; это общие диапазоны сдвинутых Trials, не 16 переходов из 00321.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/witcher-lifepath/Witcher_Background__How_Did_Early_Training_Go__u2n9HR4RhSt1QV3l.json](Witcher_Background__How_Did_Early_Training_Go__u2n9HR4RhSt1QV3l.json.md), [packsJson/witcher-lifepath/Witcher_Background__What_Was_Your_Most_Important_Event_34nU6OswdgAyMC6p.json](Witcher_Background__What_Was_Your_Most_Important_Event_34nU6OswdgAyMC6p.json.md).
