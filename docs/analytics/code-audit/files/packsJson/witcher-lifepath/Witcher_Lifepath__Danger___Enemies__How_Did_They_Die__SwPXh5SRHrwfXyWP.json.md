# packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__How_Did_They_Die__SwPXh5SRHrwfXyWP.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__How_Did_They_Die__SwPXh5SRHrwfXyWP.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__How_Did_They_Die__SwPXh5SRHrwfXyWP.json) |
...
| SHA-256 файла | 3232ae2b566cd1ef1cdf67fdc8bd66f6341b9e2e91a49feccbce4b5d04c6a129 |

## Назначение файла

Выбирает обстоятельства смерти врага; вычисляет число прошедших десятилетий при показе текста. Имя документа — `Witcher Lifepath: Danger - Enemies: How Did They Die?`. Это данные для выборки и вывода описаний, включая явно заданные ссылки и формулы; самостоятельной программы изменения Actor/Item в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):36,71–76 регистрирует RollTable-пакет Witcher_Lifepath_and_BG_Sub-tables в папке Character Generation с путём packs/witcher-lifepath.db. Адрес документа — `Compendium.TheWitcherTRPG.Witcher_Lifepath_and_BG_Sub-tables.RollTable.SwPXh5SRHrwfXyWP`.
...
| --- | --- | --- |
| name, _id, _key | `Witcher Lifepath: Danger - Enemies: How Did They Die?`; `SwPXh5SRHrwfXyWP`; `!tables!SwPXh5SRHrwfXyWP` | Имя, ID и ключ хранения корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.bXuX31bdaX2lfVkb`; systemId/systemVersion/duplicateSource/exportSource=null | Метаданные происхождения; не ссылка для рекурсивного выбора |
...
| Result _id/_key | ID в перечне; `!tables.results!SwPXh5SRHrwfXyWP.<resultId>` | Ключи хранения; _key удалён строгой моделью из toObject |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,3] | text | `As a casualty of war [[1d10]] decades later.` | 10–32 |
| 2 | `tfUjaxwfTwqW98vG` | [4,6] | text | `You killed them [[1d10]] decades later.` | 33–55 |
| 3 | `XFy2l49tSxFMwvUu` | [7,9] | text | `From a monster attack [[1d10]] decades later.` | 56–78 |
| 4 | `NlfhnGUfAnLtImRU` | [10,10] | text | `Peacefully and unrelated to any major events [[1d10]] decades later.` | 79–101 |

Четыре причины имеют вероятности 30/30/30/10%. Каждая содержит [[1d10]]; при getHTML вычисляется 1–10 десятилетий. Полный выбранный текст выводится, но дата смерти не сравнивается с возрастом или другими событиями.
...
Inline-вставки: `q0DM4FW7XCnFdKZu`: `[[1d10]]`; `tfUjaxwfTwqW98vG`: `[[1d10]]`; `XFy2l49tSxFMwvUu`: `[[1d10]]`; `NlfhnGUfAnLtImRU`: `[[1d10]]`. Всего 4; формулы валидны для настоящего парсера. При выводе исполняются все вставки выбранного description; roll и draw({displayChat:false}) их не вычисляют. Текстовые @UUID/@Compendium, внешние URL и исполняемые макросы в этом файле не найдены. Упоминания людей, предметов, монстров, правил и страниц сами по себе не являются зависимостями на документы.

## Известные потребители

| Файл / компонент | Запись / используемые данные | Условия |
| --- | --- | --- |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__Are_They_Alive__eywzppppagpC4HYO.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__Are_They_Alive__eywzppppagpC4HYO.json) | `kQX7ZeIIWKy5yTPF`; documentUuid:53; range=[1,30] | `Witcher Lifepath: Danger - Enemies: Are They Alive?`: при выборе диапазона и recursive:true вызывает этот документ |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Возможное совпадение Item.name с `Witcher Lifepath: Danger - Enemies: How Did They Die?` | Общий поиск таблицы при экспорте добычи ожидает Item, а этот граф возвращает text |
...
Witcher Lifepath: Danger - Enemies: How Did They Die?: 1d10, возможные totals 1…10; 4 результатов (4 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. В description 4 inline: 1d10; вычисление относится к getHTML, не к roll.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies__Are_They_Alive__eywzppppagpC4HYO.json](Witcher_Lifepath__Danger___Enemies__Are_They_Alive__eywzppppagpC4HYO.json.md).

[Протокол и границы](../../../review-log.md#task-0004017) — TASK-0004.017; процессы [R017-01](../../../cross-check-0002.md#r017-01), [R017-02](../../../cross-check-0002.md#r017-02), [R017-03](../../../cross-check-0002.md#r017-03), [R017-04](../../../cross-check-0002.md#r017-04), [R017-17](../../../cross-check-0002.md#r017-17), [R017-05](../../../cross-check-0002.md#r017-05). В этой порции выполнена статическая сверка; прежние опыты сохраняют свои даты и фасады. Новых поведенческих запусков нет; браузер, мир, сеть и запись в БД не запускались.
