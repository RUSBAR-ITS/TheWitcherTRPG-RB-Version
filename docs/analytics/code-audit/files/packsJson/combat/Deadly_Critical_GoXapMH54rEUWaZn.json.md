# packsJson/combat/Deadly_Critical_GoXapMH54rEUWaZn.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/combat/Deadly_Critical_GoXapMH54rEUWaZn.json](../../../../../../packsJson/combat/Deadly_Critical_GoXapMH54rEUWaZn.json) |
...
| SHA-256 файла | 9b0597c412f603ee2e10e393c74e4089312ac0f4164eac54db9764e2978482e4 |

## Назначение файла

Текстовая таблица смертельных критических результатов по сумме 2d6. Имя документа — `Deadly Critical`. Собственных функций, классов JS или обработчиков Actor в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):24–29,45–50 регистрирует Combat как RollTable, путь packs/combat.db, папка Witcher TRPG System. UUID — `Compendium.TheWitcherTRPG.Combat.RollTable.GoXapMH54rEUWaZn`. Соседний criticalWounds — отдельный пакет типа Item.
...
| --- | --- | --- |
| name / _id / _key | `Deadly Critical` / `GoXapMH54rEUWaZn` / `!tables!GoXapMH54rEUWaZn` | Название, ID и служебный ключ таблицы |
...
| img | `https://assets.forge-vtt.com/bazaar/core/icons/commodities/bones/skull-hollow-brown-red.webp` | Иконка таблицы |
| flags | `{"better-rolltables":{},"core":{},"scene-packer":{"sourceId":"Compendium.wtrpg-compendium.combat.RollTable.dR0fghweDP4uIt6c","hash":"15f6798eac5783bc35ecd1ea199d13e3a46d315a"}}` | Данные сторонних пространств; sourceId/hash не являются целью рекурсивного roll |
...
| Result _id / _key | ID ниже; `!tables.results!GoXapMH54rEUWaZn.<id>` | ID имеет смысл вместе с родителем; повтор между таблицами допустим |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `md13gU5GXWqdSo3b` | [2,3] | text | `Dismembered Leg - The blow tears your leg from your body or damages it beyond repair. Quarter your SPD, Dodge/Escape, and Athletics. This wound begins bleeding. \| Stabilized: You quarter your SPD, Dodge/Escape, and Athletics. \| Treated: The leg can be replaced with a prosthetic.` | 14–36 |
| 2 | `Kg6cLOw7bcrhf9zi` | [4,5] | text | `Dismembered Arm - The blow rends your arm from your body or damages it beyond repair. The arm cannot be used and you start bleeding. \| Stabilized: That arm is useless. \| Treated: The arm can be replaced with a prosthetic.` | 37–59 |
| 3 | `Aupjx9gqdL7IdDsG` | [6,8] | text | `Septic Shock - The blow damages your intestines, letting waste enter your blood stream. Quarter your Stamina, take a -3 to INT, WILL, REF, and DEX. You are poisoned. \| Stabilized: Your Stamina is halved and you take a -1 to INT, WILL, REF, and DEX. \| Treated: You take a -5 to Stamina permanently.` | 60–82 |
| 4 | `dcTrmwGr6QqbwArw` | [9,10] | text | `Heart Damage - The blow damages your heart. Make an immediate Death save. If you survive, the wound is bleeding and you must quarter your Stamina, SPD, and BODY. \| Stabilized: You halve your Stamina, SPD, and BODY. \| Treated: You take +2 damage per round from bleeding damage permanently` | 83–105 |
| 5 | `8BDa9D8PpydHDt9F` | [11,11] | text | `Damaged Eye - The blow cuts into or indents your eyeball. You take a -5 to sight-based Awareness and -4 to DEX. This wound begins bleeding. \| Stabilized: You take a -3 to sight-based Awareness and -2 to DEX \| Treated: Permanent -1 to sight-based Awareness and DEX.` | 106–128 |
| 6 | `H0XA02QcwUBIRRWa` | [12,12] | text | `Separated Spine/Decapitated - The blow either snaps your neck or separates your head from your shoulders. You die immediately. \| Stabilized: This wound cannot be stabilized. \| Treated: This wound cannot be treated.` | 129–151 |

Шесть description содержат Dismembered Leg, Dismembered Arm, Septic Shock, Heart Damage, Damaged Eye и Separated Spine/Decapitated. Смерть, кровотечение, протезирование и изменение характеристик здесь только описаны. JSON не содержит Item/ActiveEffect/changes или UUID травмы. Формулировки о возможности лечения не являются bool-полями.
...
Полностью описан соответствующий отдельный Item-каталог [Deadly](../criticalWounds/Deadly_uofXQEP6HBtekOAO/_Folder.json.md): 22 Item и Folder, 21 эффект/43 changes, 14 переходов. Шесть текстовых результатов Combat не содержат ссылки на Item; applyCritWound читает другой пакет. Decapitation Item имеет terminal none/followUp=null и status dead; текст таблицы не запрещает вызов его treat программно. Все 128 RollTable/995 результатов повторно проверены по настоящим моделям, броскам и HTML, без записи.
