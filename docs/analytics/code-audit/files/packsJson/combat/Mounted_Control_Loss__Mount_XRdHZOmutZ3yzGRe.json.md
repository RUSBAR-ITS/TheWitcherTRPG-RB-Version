# packsJson/combat/Mounted_Control_Loss__Mount_XRdHZOmutZ3yzGRe.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/combat/Mounted_Control_Loss__Mount_XRdHZOmutZ3yzGRe.json](../../../../../../packsJson/combat/Mounted_Control_Loss__Mount_XRdHZOmutZ3yzGRe.json) |
...
| SHA-256 файла | 81ebd92fee315f3bd9fb6f10d50bb37fe292bf2466919a8d2b6c3a3f754e1de8 |

## Назначение файла

Текстовые последствия потери управления для ездового животного. Имя документа — `Mounted Control Loss: Mount`. Собственных функций, классов JS или обработчиков Actor в JSON нет.

## Условия использования

[system.json](../../../../../../system.json):24–29,45–50 регистрирует Combat как RollTable, путь packs/combat.db, папка Witcher TRPG System. UUID — `Compendium.TheWitcherTRPG.Combat.RollTable.XRdHZOmutZ3yzGRe`. Соседний criticalWounds — отдельный пакет типа Item.
...
| --- | --- | --- |
| name / _id / _key | `Mounted Control Loss: Mount` / `XRdHZOmutZ3yzGRe` / `!tables!XRdHZOmutZ3yzGRe` | Название, ID и служебный ключ таблицы |
...
| flags | `{"better-rolltables":{},"core":{},"scene-packer":{"sourceId":"Compendium.wtrpg-compendium.combat.RollTable.XlxN7Dytx84BxN6X","hash":"a1bec211d9710a5278db6effcb71bdb38523f6d5"}}` | Данные сторонних пространств; sourceId/hash не являются целью рекурсивного roll |
...
| Result _id / _key | ID ниже; `!tables.results!XRdHZOmutZ3yzGRe.<id>` | ID имеет смысл вместе с родителем; повтор между таблицами допустим |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `2tyZUbhqjxmHQEds` | [1,3] | text | `Mounted Control Loss Mount Effect - Refusal: Your mount simply doesn’t to do what you want it to do. You must make another Control check next round.` | 14–36 |
| 2 | `i4n3taLZOfbhwLaS` | [4,4] | text | `Mounted Control Loss Mount Effect - Spooked: Your mount startles and rears. You must make an Athletics check (DC:16) to stay on its back and an Animal Handling check (DC:18) to calm it down before you can keep riding.` | 37–59 |
| 3 | `t464bMNA1oIhHyfM` | [5,5] | text | `Mounted Control Loss Mount Effect - Stumble: Your mount stumbles and must make a DC:14 Athletics check to stay standing.` | 60–82 |
| 4 | `g0lEhtumnXpkHYbX` | [6,6] | text | `Mounted Control Loss Mount Effect - Stumble: Your mount stumbles and must make a DC:18 Athletics check to stay standing.` | 83–105 |
| 5 | `PzpRoZqfOfF3Q4Cy` | [7,7] | text | `Mounted Control Loss Mount Effect - Trip: Your mount trips and must make a DC:15 Athletics check or fall and take [[1d10]] points of damage to one randomly rolled leg. Roll 1d10 = [[1d10]]. 1-3: Front Left, 4-6: Front Right, 7-8: Back Left, 9-10: Back Right.` | 106–128 |
| 6 | `h04HqTPf5v1DPHzG` | [8,8] | text | `Mounted Control Loss Mount Effect - Trip: Your mount trips and must make a DC:20 Athletics check or fall and take [[2d10]] points of damage to one randomly rolled leg. Roll 1d10 = [[1d10]]. 1-3: Front Left, 4-6: Front Right, 7-8: Back Left, 9-10: Back Right.` | 129–151 |
| 7 | `Ul4o2Z6V39ujXoFQ` | [9,9] | text | `Mounted Control Loss Mount Effect - Fall: Your mount falls over. If you are on it or within a meter of it, make a DC:18 Athletics check to avoid being landed on. If you fail, your mount falls on you. Roll a random location to see what part of you falls under the horse, then take [[2d10]] damage to that location. Your horse takes this as torso damage. After this you are trapped under your mount. You can attempt a Control check the next turn to get it off of you.` | 152–174 |
| 8 | `YKhMEggwa22NX9sA` | [10,10] | text | `Mounted Control Loss Mount Effect - Faint: Your mount faints from the sheer stress of the situation. If you are on it or within a meter of it, make a DC:18 Athletics check to avoid being landed on. If you fail, your mount falls on you. Roll a random location to see what part of you falls under the horse and take [[2d10]] damage to that location. Your horse takes this as torso damage. After this you are trapped under your mount. Your mount must make a Stun save each round to come to.` | 175–197 |

При 1–3 выбирается Refusal; 4 — Spooked; 5/6 — Stumble с разными DC; 7/8 — Trip; 9 — Fall; 10 — Faint. В Trip две независимые вставки: повреждение и число для выбора ноги. В Fall/Faint по одной вставке повреждения. Условия Athletics/Control/Stun — текст, поэтому inline-урон вычисляется при показе результата до каких-либо ручных проверок. Прямое открытие этой таблицы работает в изолированном сценарии, а общий Mounted Control Loss хранит другой UUID и её не вызывает.
...
| --- | --- | --- |
| `PzpRoZqfOfF3Q4Cy` / 125 | `1d10` | 10 |
| `PzpRoZqfOfF3Q4Cy` / 125 | `1d10` | 10 |
| `h04HqTPf5v1DPHzG` / 148 | `2d10` | 20 |
| `h04HqTPf5v1DPHzG` / 148 | `1d10` | 10 |
| `Ul4o2Z6V39ujXoFQ` / 171 | `2d10` | 20 |
| `YKhMEggwa22NX9sA` / 194 | `2d10` | 20 |
...
Связанные карточки: [Mounted Control Loss](Mounted_Control_Loss_VVb2zLR4NLdMLVQQ.json.md), [Mounted Control Loss: Personal](Mounted_Control_Loss__Personal_KWLoKiOHKXXnq5E4.json.md). Сходство имён проверено отдельно от точных UUID: действующих входящих ссылок из остальных экспортов у этих трёх файлов нет.
...
[issue-00323](../../../../../issues/closed/issue-00323.md): общий Mounted Control Loss хранит два отсутствующих UUID; дочерние документы доступны самостоятельно.

Общие риски extract/compile описаны в [issue-00313](../../../../../issues/closed/issue-00313.md), [issue-00314](../../../../../issues/closed/issue-00314.md), [issue-00315](../../../../../issues/closed/issue-00315.md); повторно не воспроизводились, конкретный JSON ими не объявлен повреждённым. Регистрация наблюдений не означает подтверждения или разрешения исправления.
...
Mounted Control Loss: Mount: 1d10, возможные totals 1…10; 8 результатов (8 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 0. В description 6 inline: 1d10, 2d10; вычисление относится к getHTML, не к roll.
...
[Протокол и границы](../../../review-log.md#task-0004017) — TASK-0004.017; процессы [R017-01](../../../cross-check-0002.md#r017-01), [R017-02](../../../cross-check-0002.md#r017-02), [R017-03](../../../cross-check-0002.md#r017-03), [R017-04](../../../cross-check-0002.md#r017-04), [R017-22](../../../cross-check-0002.md#r017-22), [R017-23](../../../cross-check-0002.md#r017-23), [R017-05](../../../cross-check-0002.md#r017-05). В этой порции выполнена статическая сверка; прежние опыты сохраняют свои даты и фасады. Новых поведенческих запусков нет; браузер, мир, сеть и запись в БД не запускались.
