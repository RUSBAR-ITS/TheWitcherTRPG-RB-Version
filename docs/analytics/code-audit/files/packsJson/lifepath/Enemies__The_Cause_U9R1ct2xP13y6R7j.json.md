# packsJson/lifepath/Enemies__The_Cause_U9R1ct2xP13y6R7j.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Enemies__The_Cause_U9R1ct2xP13y6R7j.json](../../../../../../packsJson/lifepath/Enemies__The_Cause_U9R1ct2xP13y6R7j.json) |
...
| SHA-256 файла | d4b897a12a8973974322df9258708986d14e12068d1495c88120210bbf794c7e |

## Назначение файла

Задаёт десять текстовых причин конфликта с врагом. Имя RollTable — `Enemies: The Cause`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.U9R1ct2xP13y6R7j`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Enemies: The Cause`; `U9R1ct2xP13y6R7j`; `!tables!U9R1ct2xP13y6R7j` | Имя документа, идентификатор и ключ хранилища |
...
| Result _key | `!tables.results!U9R1ct2xP13y6R7j.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.AFlxvOkbTAVvKpEU`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,1] | text | `Why Does Your Enemy Dislike You: Assaulted the Offended Party` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [2,2] | text | `Why Does Your Enemy Dislike You: Caused the Loss of a Loved One` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [3,3] | text | `Why Does Your Enemy Dislike You: Caused Major Humiliation` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [4,4] | text | `Why Does Your Enemy Dislike You: Caused a Curse` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [5,5] | text | `Why Does Your Enemy Dislike You: Accused of Illegal Witchcraft` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | [6,6] | text | `Why Does Your Enemy Dislike You: Turned Down Romantically` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | [7,7] | text | `Why Does Your Enemy Dislike You: Caused a Terrible Wound` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | [8,8] | text | `Why Does Your Enemy Dislike You: Blackmail` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | [9,9] | text | `Why Does Your Enemy Dislike You: Foiled Plans` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | [10,10] | text | `Why Does Your Enemy Dislike You: Caused a Monster Attack` | 217–239 |

Каждая запись выбирается на одной грани 1d10 с вероятностью 1/10. normalize по weight=1 сохраняет формулу и одиночные диапазоны.
...
| --- | --- | --- |
| [packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json](../../../../../../packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json) | `a90EHPhkX2hszHH8`, documentUuid:78; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Enemies: The Cause` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Enemies: The Cause: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json](Enemies__Generator_7AXmeCSRkK3ktJ9Y.json.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).
