# packsJson/lifepath/Allies__Closeness_IswiqefPmaHECa5X.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Allies__Closeness_IswiqefPmaHECa5X.json](../../../../../../packsJson/lifepath/Allies__Closeness_IswiqefPmaHECa5X.json) |
...
| SHA-256 файла | d2d3c360298426b6876e7e443dfab0d5cbb3427804ad532b859e268ed37ddf92 |

## Назначение файла

Описывает степень близости с союзником: пять текстовых вариантов с разной шириной диапазонов. Имя RollTable — `Allies: Closeness`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.IswiqefPmaHECa5X`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Allies: Closeness`; `IswiqefPmaHECa5X`; `!tables!IswiqefPmaHECa5X` | Имя документа, идентификатор и ключ хранилища |
...
| Result _key | `!tables.results!IswiqefPmaHECa5X.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.ROPXY35Dl6mJbLGS`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,4] | text | `Closeness To Your Ally: Acquaintances` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [5,6] | text | `Closeness To Your Ally: Friends` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [7,8] | text | `Closeness To Your Ally: Close Friends` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [9,9] | text | `Closeness To Your Ally: Inseparable` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [10,10] | text | `Closeness To Your Ally: Bound By Bond` | 102–124 |

Вероятности пяти записей при равномерном d10: 40%, 20%, 20%, 10%, 10%. Все weight=1, поэтому явный normalize построит 1d5 с равными одиночными диапазонами и изменит распределение.
...
| --- | --- | --- |
| [packsJson/lifepath/Allies__Generator_Va7NF10ETcMvndFo.json](../../../../../../packsJson/lifepath/Allies__Generator_Va7NF10ETcMvndFo.json) | `KgpE0Q8gz4x0tE9A`, documentUuid:102; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Allies: Closeness` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Allies: Closeness: 1d10, возможные totals 1…10; 5 результатов (5 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
