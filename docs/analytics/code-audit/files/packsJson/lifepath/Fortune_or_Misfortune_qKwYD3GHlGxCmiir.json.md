# packsJson/lifepath/Fortune_or_Misfortune_qKwYD3GHlGxCmiir.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Fortune_or_Misfortune_qKwYD3GHlGxCmiir.json](../../../../../../packsJson/lifepath/Fortune_or_Misfortune_qKwYD3GHlGxCmiir.json) |
...
| SHA-256 файла | 1acd02e4214f1440d2a3e97ae1b3a556126c6c367b27e2d7f78a8be40ce2af2a |

## Назначение файла

Выбирает Misfortune при 1 или Fortune при 2 и раскрывает один текстовый результат выбранной таблицы. Имя RollTable — `Fortune or Misfortune`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.qKwYD3GHlGxCmiir`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Fortune or Misfortune`; `qKwYD3GHlGxCmiir`; `!tables!qKwYD3GHlGxCmiir` | Имя документа, идентификатор и ключ хранилища |
...
| Result _key | `!tables.results!qKwYD3GHlGxCmiir.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.eYZyRmHnDVd9Lrty`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `JQmqgYHhLo9rqERr` | [1,1] | document | `Misfortune` | 10–33 |
| 2 | `cUNRbBflCqmrkzp7` | [2,2] | document | `Fortune` | 34–57 |

При recursive:true d2 возвращает один текст Misfortune или Fortune. Селектор не является промежуточной ступенью Life Event Generator: тот ссылается непосредственно на обе листовые таблицы. Входящих documentUuid на этот селектор в экспортном наборе нет.
...
| `JQmqgYHhLo9rqERr`: `Misfortune` | [Misfortune_JNbvihde5EGIFPdB.json](Misfortune_JNbvihde5EGIFPdB.json.md) | `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.JNbvihde5EGIFPdB`; recursive:true раскрывает RollTable, false сохраняет ссылку |
| `cUNRbBflCqmrkzp7`: `Fortune` | [Fortune_Z0eeWQI3R8v4YNLd.json](Fortune_Z0eeWQI3R8v4YNLd.json.md) | `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.Z0eeWQI3R8v4YNLd`; recursive:true раскрывает RollTable, false сохраняет ссылку |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Fortune or Misfortune` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
[issue-00319](../../../../../issues/potential/issue-00319.md): x100 в Jackpot/Debt не умножает d10 на 100 (достижимо через этот селектор). [issue-00039](../../../../../issues/potential/issue-00039.md) касается предположений общего потребителя добычи о results[0]; эта таблица не обещает Item-результаты. [issue-00313](../../../../../issues/potential/issue-00313.md) относится к удалению корневых JSON до успешного извлечения; исходник здесь не изменялся.
...
Fortune or Misfortune: 1d2, возможные totals 1…2; 2 результатов (0 text / 2 document), replacement=true, displayRoll=false. Рекурсивные переходы: Misfortune; Fortune. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 1; входящих файлов 0. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/lifepath/Misfortune_JNbvihde5EGIFPdB.json](Misfortune_JNbvihde5EGIFPdB.json.md), [packsJson/lifepath/Fortune_Z0eeWQI3R8v4YNLd.json](Fortune_Z0eeWQI3R8v4YNLd.json.md), [module/data/actor/templates/character/general/lifeEventData.js](../../module/data/actor/templates/character/general/lifeEventData.js.md), [module/data/actor/templates/character/general/lifeEventsData.js](../../module/data/actor/templates/character/general/lifeEventsData.js.md).
