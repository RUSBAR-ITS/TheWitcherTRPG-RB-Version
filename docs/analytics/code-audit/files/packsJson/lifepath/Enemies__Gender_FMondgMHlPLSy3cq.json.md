# packsJson/lifepath/Enemies__Gender_FMondgMHlPLSy3cq.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Enemies__Gender_FMondgMHlPLSy3cq.json](../../../../../../packsJson/lifepath/Enemies__Gender_FMondgMHlPLSy3cq.json) |
...
| Задача и порция | [TASK-0003.053](../../../../../tasks/task-0003.053.md); 21 файл / 3845 строк, этот файл — 99 строк |
| Запись перекрёстной сверки | [TASK-0003.053](../../../review-log.md#task-0003053) |
| SHA-256 файла | c4b67fccb5798dc48a7e0a8286450189250a15ef03651fc11a6012e8775d18f8 |

## Назначение файла

Описывает гендер врага; используется также генератором врагов ведьмачьего жизненного пути. Имя RollTable — `Enemies: Gender`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.FMondgMHlPLSy3cq`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Enemies: Gender`; `FMondgMHlPLSy3cq`; `!tables!FMondgMHlPLSy3cq` | Имя документа, идентификатор и ключ хранилища |
| results | 3 вложенных TableResult; полный список ниже | Собственные ID уникальны внутри родителя; порядок сохраняется при выборе |
...
| Result _key | `!tables.results!FMondgMHlPLSy3cq.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.CeDEF0i8HITBbBlt`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `ciSfEh9nn9LIoeuR` | [1,4] | text | `Enemy Gender: Male` | 10–32 |
| 2 | `Gl1XVUlHbuQVyANO` | [5,8] | text | `Enemy Gender: Female` | 33–55 |
| 3 | `WeN79l7ug6yDM7Qf` | [9,10] | text | `Enemy Gender: Gender Diverse` | 56–78 |

Вероятности записей: 40%, 40%, 20%. normalize построит 1d3. Внешний ведьмачий генератор использует именно эту таблицу, а не отдельную копию гендера.
...
| RollTable.normalize({save:false}) | Строит 1d3 и последовательные диапазоны по 3 единичным весам | Возвращает клон без записи; save=true обновил бы документ |
...
| --- | --- | --- |
| [packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json](../../../../../../packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json) | `q0DM4FW7XCnFdKZu`, documentUuid:30; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
| [packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies_Generator_iwGZ1Sj0v9iBqvSk.json](../../../../../../packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies_Generator_iwGZ1Sj0v9iBqvSk.json) | `q0DM4FW7XCnFdKZu`, documentUuid:30; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Enemies: Gender` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Завершён разбор прежнего внешнего потребителя Witcher_Lifepath__Danger___Enemies_Generator_iwGZ1Sj0v9iBqvSk.json. Его result q0DM4FW7XCnFdKZu, documentUuid:30, [1,1] вызывает именно этот Life_Event_Sub-tables.RollTable.FMondgMHlPLSy3cq; это единственная исходящая межпакетная ссылка 41 ведьмачьей таблицы. Реальный рекурсивный roll с lookup в памяти возвращает пол первым из семи аспектов; при живом враге всего семь текстов, при умершем восемь. Дополнительные два сценария mortality и четыре опасных Decade подтвердили эту связь; исходная карточка .053 сохраняется.
...
Enemies: Gender: 1d10, возможные totals 1…10; 3 результатов (3 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 2. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json](Enemies__Generator_7AXmeCSRkK3ktJ9Y.json.md), [packsJson/witcher-lifepath/Witcher_Lifepath__Danger___Enemies_Generator_iwGZ1Sj0v9iBqvSk.json](../witcher-lifepath/Witcher_Lifepath__Danger___Enemies_Generator_iwGZ1Sj0v9iBqvSk.json.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).

[Протокол и границы](../../../review-log.md#task-0004017) — TASK-0004.017; процессы [R017-01](../../../cross-check-0002.md#r017-01), [R017-02](../../../cross-check-0002.md#r017-02), [R017-03](../../../cross-check-0002.md#r017-03), [R017-04](../../../cross-check-0002.md#r017-04), [R017-08](../../../cross-check-0002.md#r017-08), [R017-17](../../../cross-check-0002.md#r017-17). В этой порции выполнена статическая сверка; прежние опыты сохраняют свои даты и фасады. Новых поведенческих запусков нет; браузер, мир, сеть и запись в БД не запускались.
