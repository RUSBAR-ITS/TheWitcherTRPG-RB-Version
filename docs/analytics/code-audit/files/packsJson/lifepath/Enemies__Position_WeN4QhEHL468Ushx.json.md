# packsJson/lifepath/Enemies__Position_WeN4QhEHL468Ushx.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Enemies__Position_WeN4QhEHL468Ushx.json](../../../../../../packsJson/lifepath/Enemies__Position_WeN4QhEHL468Ushx.json) |
...
| SHA-256 файла | a4a0afec2c7fb3e0414888715a818d6ad73b90754b54bdefd23123e74dcb0842 |

## Назначение файла

Задаёт десять текстовых вариантов положения или роли врага. Имя RollTable — `Enemies: Position`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.WeN4QhEHL468Ushx`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Enemies: Position`; `WeN4QhEHL468Ushx`; `!tables!WeN4QhEHL468Ushx` | Имя документа, идентификатор и ключ хранилища |
...
| Result _key | `!tables.results!WeN4QhEHL468Ushx.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.vrZgyP6QW0QFtvQa`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,1] | text | `Enemy Position: Ex-Friend` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [2,2] | text | `Enemy Position: Ex-Lover` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [3,3] | text | `Enemy Position: Relative` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [4,4] | text | `Enemy Position: Childhood Enemy` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [5,5] | text | `Enemy Position: A Cultist` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | [6,6] | text | `Enemy Position: A Bard` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | [7,7] | text | `Enemy Position: A Soldier` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | [8,8] | text | `Enemy Position: A Bandit` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | [9,9] | text | `Enemy Position: A Duke/Duchess` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | [10,10] | text | `Enemy Position: A Mage` | 217–239 |
...
| --- | --- | --- |
| [packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json](../../../../../../packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json) | `mp0F0yfLeczgHYKB`, documentUuid:54; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Enemies: Position` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Enemies: Position: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
