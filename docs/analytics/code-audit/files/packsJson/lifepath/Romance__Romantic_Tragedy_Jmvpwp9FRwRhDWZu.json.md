# packsJson/lifepath/Romance__Romantic_Tragedy_Jmvpwp9FRwRhDWZu.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Romance__Romantic_Tragedy_Jmvpwp9FRwRhDWZu.json](../../../../../../packsJson/lifepath/Romance__Romantic_Tragedy_Jmvpwp9FRwRhDWZu.json) |
...
| SHA-256 файла | 380e2bacfcda020c7c75926750c60ca5649d5b2b7a26682d203176d48643d218 |

## Назначение файла

Задаёт десять текстовых исходов романтической трагедии; подтаблица Romance. Имя RollTable — `Romance: Romantic Tragedy`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.Jmvpwp9FRwRhDWZu`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Romance: Romantic Tragedy`; `Jmvpwp9FRwRhDWZu`; `!tables!Jmvpwp9FRwRhDWZu` | Имя документа, идентификатор и ключ хранилища |
...
| Result _key | `!tables.results!Jmvpwp9FRwRhDWZu.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.RWyNtIGCLA9pKhEe`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,1] | text | `Your Lover was captured by bandits some time ago and is still their captive.` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [2,2] | text | `Your Lover mysteriously vanished one day and you don’t know where they went.` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [3,3] | text | `Your Lover was imprisoned or exiled for crimes they may not have commited.` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [4,4] | text | `Your Lover was taken from you by a powerful curse.` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [5,5] | text | `Something got between you and your Lover and you were forced to kill them.` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | [6,6] | text | `Your Lover comitted suicide. You may not know why they did it.` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | [7,7] | text | `Your Lover was kidnapped by a noble and made into a concubine.` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | [8,8] | text | `A rival cut you out of the action and stole your Lover’s affection.` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | [9,9] | text | `Your Lover was killed by monsters. It may have been an accident or planned.` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | [10,10] | text | `Your Lover is a mage, a witcher, or a sentient monster, dooming the romance.` | 217–239 |

Прямой draw учитывает собственный displayRoll=true. При раскрытии из Romance сообщение формирует родитель с displayRoll=false, поэтому HTML броска не появляется. Описания проклятий, смерти и других последствий остаются текстом.
...
| --- | --- | --- |
| [packsJson/lifepath/Romance_CDgdx129wZvINn16.json](../../../../../../packsJson/lifepath/Romance_CDgdx129wZvINn16.json) | `diG3ThmxwJLO2CDQ`, documentUuid:76; диапазон [2,4] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Romance: Romantic Tragedy` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Romance: Romantic Tragedy: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
