# packsJson/lifepath/Romance__Problematic_Love_l7k0hSL3iRzjJcYs.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Romance__Problematic_Love_l7k0hSL3iRzjJcYs.json](../../../../../../packsJson/lifepath/Romance__Problematic_Love_l7k0hSL3iRzjJcYs.json) |
...
| SHA-256 файла | b869887818250ef06f8b97c20064df4be1d202d568fe75e757d87cbc438f1852 |

## Назначение файла

Задаёт десять текстовых обстоятельств проблемных отношений; подтаблица Romance. Имя RollTable — `Romance: Problematic Love`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.l7k0hSL3iRzjJcYs`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Romance: Problematic Love`; `l7k0hSL3iRzjJcYs`; `!tables!l7k0hSL3iRzjJcYs` | Имя документа, идентификатор и ключ хранилища |
...
| formula, replacement, displayRoll | `1d10`, true, true | Формула выбора; повторное получение разрешено; настройка HTML основного броска в собственном сообщении |
...
| Result _key | `!tables.results!l7k0hSL3iRzjJcYs.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.mtcwd7JxTTleCslr`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,1] | text | `Your Lover’s family or friends hate you and do not condone your romance.` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [2,2] | text | `Your Lover works as a whore for a living and refuses to give up their job.` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [3,3] | text | `Your Lover is under a minor curse such as paranoia or horrible nightmares.` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | [4,4] | text | `Your Lover slept around and refused to stop when you found out.` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | [5,5] | text | `Your Lover is insanely jealous and can’t stand you being around any possible rival.` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | [6,6] | text | `You fight constantly and nothing can stop it for long. You always descend into screaming.` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | [7,7] | text | `You’re professional rivals of some sort. You steal customers from each other often.` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | [8,8] | text | `One of you is human and the other is non-human, making your life difficult.` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | [9,9] | text | `Your Lover is already married. They may or may not be willing to leave their spouse.` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | [10,10] | text | `Your friends or family hate your Lover and do not condone your romance.` | 217–239 |

Прямой draw учитывает собственный displayRoll=true. При раскрытии из Romance сообщение формирует родитель с displayRoll=false, поэтому HTML броска не появляется. Само описание выбранной записи сохраняется.
...
| TableResult.getHTML / RollTable.toMessage | Готовят описание/ссылку и HTML результата, затем ChatMessage | displayRoll=true действует у таблицы, создающей сообщение; у вложенной таблицы свой чат не создаётся |
...
| --- | --- | --- |
| [packsJson/lifepath/Romance_CDgdx129wZvINn16.json](../../../../../../packsJson/lifepath/Romance_CDgdx129wZvINn16.json) | `OE6SRABJiTVvYX8Q`, documentUuid:123; диапазон [5,6] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Romance: Problematic Love` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Romance: Problematic Love: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/lifepath/Romance_CDgdx129wZvINn16.json](Romance_CDgdx129wZvINn16.json.md), [module/data/actor/templates/character/general/lifeEventData.js](../../module/data/actor/templates/character/general/lifeEventData.js.md), [module/data/actor/templates/character/general/lifeEventsData.js](../../module/data/actor/templates/character/general/lifeEventsData.js.md).
