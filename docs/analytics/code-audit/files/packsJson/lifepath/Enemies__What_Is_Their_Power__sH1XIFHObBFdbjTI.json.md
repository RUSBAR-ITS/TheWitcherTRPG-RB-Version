# packsJson/lifepath/Enemies__What_Is_Their_Power__sH1XIFHObBFdbjTI.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Enemies__What_Is_Their_Power__sH1XIFHObBFdbjTI.json](../../../../../../packsJson/lifepath/Enemies__What_Is_Their_Power__sH1XIFHObBFdbjTI.json) |
...
| SHA-256 файла | ea8f23168e15645d3ff97676cbdea2931b054a04b8e2a00fa22e66a27127ffc1 |

## Назначение файла

Описывает источник силы врага: общественное влияние, знания, физическая сила, слуги или магия. Имя RollTable — `Enemies: What Is Their Power?`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.sH1XIFHObBFdbjTI`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Enemies: What Is Their Power?`; `sH1XIFHObBFdbjTI`; `!tables!sH1XIFHObBFdbjTI` | Имя документа, идентификатор и ключ хранилища |
...
| Result _key | `!tables.results!sH1XIFHObBFdbjTI.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.lxGO1xfI2nYEdd2w`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,2] | text | `Enemy - Power Source: Social Power` | 10–32 |
| 2 | `tfUjaxwfTwqW98vG` | [3,4] | text | `Enemy - Power Source: Knowledge` | 33–55 |
| 3 | `XFy2l49tSxFMwvUu` | [5,6] | text | `Enemy - Power Source: Physical` | 56–78 |
| 4 | `NlfhnGUfAnLtImRU` | [7,8] | text | `Enemy - Power Source: Minions` | 79–101 |
| 5 | `9r21On8F1Le0wu4M` | [9,10] | text | `Enemy - Power Source: Magic` | 102–124 |
...
| --- | --- | --- |
| [packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json](../../../../../../packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json) | `jfwK7Kaom9UaZb0J`, documentUuid:174; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Enemies: What Is Their Power?` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Enemies: What Is Their Power?: 1d10, возможные totals 1…10; 5 результатов (5 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
