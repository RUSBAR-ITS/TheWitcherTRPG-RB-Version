# packsJson/lifepath/Allies__Where_Are_They__W19e7rtl3ycrMhQU.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Allies__Where_Are_They__W19e7rtl3ycrMhQU.json](../../../../../../packsJson/lifepath/Allies__Where_Are_They__W19e7rtl3ycrMhQU.json) |
...
| Задача и порция | [TASK-0003.053](../../../../../tasks/task-0003.053.md); 21 файл / 3845 строк, этот файл — 122 строк |
| Запись перекрёстной сверки | [TASK-0003.053](../../../review-log.md#task-0003053) |
| SHA-256 файла | 0d31edf7cd3c05192e665d67013512810798e207930aff108864ad4898fbfc48 |

## Назначение файла

Задаёт четыре текстовых региона, где находится союзник. Имя RollTable — `Allies: Where Are They?`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG.Life_Event_Sub-tables.RollTable.W19e7rtl3ycrMhQU`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Allies: Where Are They?`; `W19e7rtl3ycrMhQU`; `!tables!W19e7rtl3ycrMhQU` | Имя документа, идентификатор и ключ хранилища |
| results | 4 вложенных TableResult; полный список ниже | Собственные ID уникальны внутри родителя; порядок сохраняется при выборе |
...
| Result _key | `!tables.results!W19e7rtl3ycrMhQU.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.cKgCcCVB0YOw3b6e`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | [1,3] | text | `Where Is Your Ally: The Northern Kingdoms` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | [4,6] | text | `Where Is Your Ally: The Empire of Nilfgaard` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | [7,9] | text | `Where Is Your Ally: Elder Lands` | 56–78 |
| 4 | `AU7Vzk5bQT7sfuhA` | [10,10] | text | `Where Is Your Ally: Beyond the Boundaries` | 79–101 |

Вероятности записей: 30%, 30%, 30%, 10%. normalize построит 1d4 и изменит распределение.
...
| RollTable.normalize({save:false}) | Строит 1d4 и последовательные диапазоны по 4 единичным весам | Возвращает клон без записи; save=true обновил бы документ |
...
| --- | --- | --- |
| [packsJson/lifepath/Allies__Generator_Va7NF10ETcMvndFo.json](../../../../../../packsJson/lifepath/Allies__Generator_Va7NF10ETcMvndFo.json) | `ph4JxbZMB7MniZZ5`, documentUuid:126; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Allies: Where Are They?` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Allies: Where Are They?: 1d10, возможные totals 1…10; 4 результатов (4 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
