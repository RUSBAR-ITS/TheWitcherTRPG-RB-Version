# packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json](../../../../../../packsJson/lifepath/Enemies__Generator_7AXmeCSRkK3ktJ9Y.json) |
...
| Задача и порция | [TASK-0003.053](../../../../../tasks/task-0003.053.md); 21 файл / 3845 строк, этот файл — 198 строк |
| Запись перекрёстной сверки | [TASK-0003.053](../../../review-log.md#task-0003053) |
| SHA-256 файла | ab49a73cb7070459055a361e4c008e66f68931121bfc16c2d94567069e86593b |
...
Собирает описание врага из семи подтаблиц: гендер, положение, причина, пострадавшая сторона, уровень силы, эскалация и источник силы. Имя RollTable — `Enemies: Generator`. Последствия в описаниях служат информацией для пользователя; исполнительных changes, эффектов и обработчиков Actor в JSON нет.
...
Адрес по манифесту и ID: `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.7AXmeCSRkK3ktJ9Y`. Штатный draw доступен через лист/каталог таблиц; внешние потребители могут разрешить UUID или найти документ по имени. Экспорт не подтверждает совпадение с действующей БД и права конкретного игрока.

## Введённые сущности и действия с ними

| Сущность / поля | Значение и место | Действия |
| --- | --- | --- |
| name, _id, _key | `Enemies: Generator`; `7AXmeCSRkK3ktJ9Y`; `!tables!7AXmeCSRkK3ktJ9Y` | Имя документа, идентификатор и ключ хранилища |
| results | 7 вложенных TableResult; полный список ниже | Собственные ID уникальны внутри родителя; порядок сохраняется при выборе |
...
| documentUuid | 7 ссылок, перечислены ниже | Адрес цели есть только у document-результатов |
...
| Result _key | `!tables.results!7AXmeCSRkK3ktJ9Y.<resultId>` | Отдельный ключ каждого результата; входит в экспорт, отбрасывается toObject модели |
| flags | better-rolltables={}, core={} | Пустые пространства флагов, настроек автоматизации не содержат |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.kfXcEsNVGJZY8sT0`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; compendiumSource не используется для вложенного броска |
...
| 2 | `mp0F0yfLeczgHYKB` | [1,1] | document | `Enemies: Position` | 34–57 |
| 3 | `a90EHPhkX2hszHH8` | [1,1] | document | `Enemies: The Cause` | 58–81 |
| 4 | `KgpE0Q8gz4x0tE9A` | [1,1] | document | `Enemies: Who Was Wronged` | 82–105 |
| 5 | `ph4JxbZMB7MniZZ5` | [1,1] | document | `Enemies: Power` | 106–129 |
| 6 | `J0EWzUzeZpdtwFPt` | [1,1] | document | `Enemies: How Far Has It Escalated?` | 130–153 |
| 7 | `jfwK7Kaom9UaZb0J` | [1,1] | document | `Enemies: What Is Their Power?` | 154–177 |

Все семь результатов имеют диапазон [1,1] и выбираются одновременно. recursive:false возвращает семь document-результатов; recursive:true раскрывает их в семь текстов в порядке записей. normalize построит 1d7 с выбором одной подтаблицы и изменит смысл генератора.
...
| RollTable.normalize({save:false}) | Строит 1d7 и последовательные диапазоны по 7 единичным весам | Возвращает клон без записи; save=true обновил бы документ |
...
| `q0DM4FW7XCnFdKZu`: `Enemies: Gender` | [Enemies__Gender_FMondgMHlPLSy3cq.json](Enemies__Gender_FMondgMHlPLSy3cq.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.FMondgMHlPLSy3cq`; recursive:true раскрывает RollTable, false сохраняет ссылку |
| `mp0F0yfLeczgHYKB`: `Enemies: Position` | [Enemies__Position_WeN4QhEHL468Ushx.json](Enemies__Position_WeN4QhEHL468Ushx.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.WeN4QhEHL468Ushx`; recursive:true раскрывает RollTable, false сохраняет ссылку |
| `a90EHPhkX2hszHH8`: `Enemies: The Cause` | [Enemies__The_Cause_U9R1ct2xP13y6R7j.json](Enemies__The_Cause_U9R1ct2xP13y6R7j.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.U9R1ct2xP13y6R7j`; recursive:true раскрывает RollTable, false сохраняет ссылку |
| `KgpE0Q8gz4x0tE9A`: `Enemies: Who Was Wronged` | [Enemies__Who_Was_Wronged_cz5KlvgE7I7QV57H.json](Enemies__Who_Was_Wronged_cz5KlvgE7I7QV57H.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.cz5KlvgE7I7QV57H`; recursive:true раскрывает RollTable, false сохраняет ссылку |
| `ph4JxbZMB7MniZZ5`: `Enemies: Power` | [Enemies__Power_9mYMTKkCuU2ElJdx.json](Enemies__Power_9mYMTKkCuU2ElJdx.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.9mYMTKkCuU2ElJdx`; recursive:true раскрывает RollTable, false сохраняет ссылку |
| `J0EWzUzeZpdtwFPt`: `Enemies: How Far Has It Escalated?` | [Enemies__How_Far_Has_It_Escalated__BLiqJBssahtqPqVf.json](Enemies__How_Far_Has_It_Escalated__BLiqJBssahtqPqVf.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.BLiqJBssahtqPqVf`; recursive:true раскрывает RollTable, false сохраняет ссылку |
| `jfwK7Kaom9UaZb0J`: `Enemies: What Is Their Power?` | [Enemies__What_Is_Their_Power__sH1XIFHObBFdbjTI.json](Enemies__What_Is_Their_Power__sH1XIFHObBFdbjTI.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Life_Event_Sub-tables.RollTable.sH1XIFHObBFdbjTI`; recursive:true раскрывает RollTable, false сохраняет ссылку |
...
| --- | --- | --- |
| [packsJson/lifepath/Allies_and_Enemies_Lp42vhkw20Ys973y.json](../../../../../../packsJson/lifepath/Allies_and_Enemies_Lp42vhkw20Ys973y.json) | `JQmqgYHhLo9rqERr`, documentUuid:30; диапазон [1,1] | При рекурсивном roll обращается к этой таблице |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Enemies: Generator` | Условный поиск checkIfItemHasRollTable при экспорте добычи; берёт results[0], а этот набор после рекурсии возвращает текст и не создаёт Item |
...
Отдельной новой проблемы этого файла не зарегистрировано. [issue-00039](../../../../../issues/closed/issue-00039.md) касается предположений общего потребителя добычи о results[0]; эта таблица не обещает Item-результаты. [issue-00313](../../../../../issues/potential/issue-00313.md) относится к удалению корневых JSON до успешного извлечения; исходник здесь не изменялся.
