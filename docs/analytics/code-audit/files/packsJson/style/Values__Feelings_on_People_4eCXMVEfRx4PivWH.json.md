# packsJson/style/Values__Feelings_on_People_4eCXMVEfRx4PivWH.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/style/Values__Feelings_on_People_4eCXMVEfRx4PivWH.json](../../../../../../packsJson/style/Values__Feelings_on_People_4eCXMVEfRx4PivWH.json) |
...
| SHA-256 файла | 7cebedeb40643231dc3cbdc4e23ec68cfd699507b1661fc05af4983865e7e55a |

## Назначение файла

Отношение к людям: десять текстовых вариантов для описания персонажа. Файл задаёт таблицу `Values: Feelings on People`, а не Item и не ActiveEffect. Собственных обработчиков изменения Actor и игровых бонусов в нём нет.
...
| _stats | 245–252: coreVersion 13.341; compendiumSource `RollTable.02MmGzQObI5D24ck`; systemId/systemVersion/duplicateSource/exportSource null | История происхождения документа, не адрес вложенного броска |
...
Все результаты имеют _key вида `!tables.results!4eCXMVEfRx4PivWH.<ID результата>`. Повтор тех же десяти локальных ID в соседних таблицах допустим: родитель входит в _key и UUID; 70 вложенных UUID серии различаются. В каждой отдельной таблице ID не повторяются.

| Значение d10 / диапазон [n,n] | ID результата | Точное description из JSON | Строки записи |
| --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | `Feelings on People: People Are Tools to Be Used` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | `Feelings on People: Our Kind Are Fine but Plough the Rest` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | `Feelings on People: People Can Never Be Trusted` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | `Feelings on People: People Have to Prove Themselves` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | `Feelings on People: Neutral` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | `Feelings on People: Neutral` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | `Feelings on People: People Are Great` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | `Feelings on People: Everyone Deserves to Die` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | `Feelings on People: People Are Hedonistic Swine` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | `Feelings on People: All Life Is Valuable` | 217–239 |

Результаты 5 и 6 имеют одинаковое описание Feelings on People: Neutral, но разные ID и диапазоны. При 1d10 это два доступных исхода из десяти. Повтор текста не создаёт конфликта документов и сам по себе не является технической ошибкой; соответствие распределения рулбуку не оценивалось.
...
| --- | --- | --- |
| [packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json](../../../../../../packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json) | results[6], ID `HkPKK3KrmtmNyo1B`; documentUuid, строка 174 | Прямая ссылка на этот RollTable. При рекурсивном roll генератора получается один из его текстовых результатов |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Values: Feelings on People` | Условный потребитель checkIfItemHasRollTable при экспорте добычи. Подбирает таблицу по индексу/имени; текстовый результат не превращается в Item, выдаётся exportLootInvalidItemError |
...
Новых технических проблем данного JSON в выполненном объёме не обнаружено. [issue-00313](../../../../../issues/closed/issue-00313.md) относится к предварительному удалению таких корневых JSON утилитой извлечения; это прежняя проблема процесса, а не дефект этих результатов. [issue-00039](../../../../../issues/closed/issue-00039.md) описывает ограничения другого потребителя RollTable — генератора добычи; таблица стиля не объявляется генератором Item.
...
| --- | --- | --- |
| [Style and Values](../character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json.md) | `HkPKK3KrmtmNyo1B` | 174; [1,1] |
...
Values: Feelings on People: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
