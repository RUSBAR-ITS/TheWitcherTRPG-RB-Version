# packsJson/style/Style__Affectations_4RWDMDzNdnz2kgwU.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/style/Style__Affectations_4RWDMDzNdnz2kgwU.json](../../../../../../packsJson/style/Style__Affectations_4RWDMDzNdnz2kgwU.json) |
...
| SHA-256 файла | fe3fb9bb56009173dd350dc874a527157bee9c03be08a4da5e69e3d5787d5808 |

## Назначение файла

Дополняющие детали внешности: десять текстовых вариантов для описания персонажа. Файл задаёт таблицу `Style: Affectations`, а не Item и не ActiveEffect. Собственных обработчиков изменения Actor и игровых бонусов в нём нет.
...
| _stats | 245–252: coreVersion 13.341; compendiumSource `RollTable.MZIEW8pxcOEojTs0`; systemId/systemVersion/duplicateSource/exportSource null | История происхождения документа, не адрес вложенного броска |
...
Все результаты имеют _key вида `!tables.results!4RWDMDzNdnz2kgwU.<ID результата>`. Повтор тех же десяти локальных ID в соседних таблицах допустим: родитель входит в _key и UUID; 70 вложенных UUID серии различаются. В каждой отдельной таблице ID не повторяются.

| Значение d10 / диапазон [n,n] | ID результата | Точное description из JSON | Строки записи |
| --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | `Affectations: Trophies` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | `Affectations: Rings &amp; Jewelry` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | `Affectations: Trinkets` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | `Affectations: Tattoos` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | `Affectations: War Paint` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | `Affectations: Shadowy Cloak` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | `Affectations: Bright Bandanas` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | `Affectations: Eye Patch` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | `Affectations: Furs` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | `Affectations: Insignias &amp; Plaques` | 217–239 |

Все десять описаний различаются. В 2 результатах сохранена HTML-сущность &amp;amp; (в JSON — `&amp;`); штатный шаблон вставляет описание как HTML. В проверке шаблона дополнительного экранирования не возникло; декодирование браузером отдельно не запускалось.
...
| --- | --- | --- |
| [packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json](../../../../../../packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json) | results[3], ID `YqKoWFzgGZuSuzj0`; documentUuid, строка 102 | Прямая ссылка на этот RollTable. При рекурсивном roll генератора получается один из его текстовых результатов |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Style: Affectations` | Условный потребитель checkIfItemHasRollTable при экспорте добычи. Подбирает таблицу по индексу/имени; текстовый результат не превращается в Item, выдаётся exportLootInvalidItemError |
...
| --- | --- | --- |
| [Style and Values](../character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json.md) | `YqKoWFzgGZuSuzj0` | 102; [1,1] |
...
Style: Affectations: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
