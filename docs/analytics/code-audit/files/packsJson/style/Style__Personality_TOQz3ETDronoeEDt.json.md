# packsJson/style/Style__Personality_TOQz3ETDronoeEDt.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/style/Style__Personality_TOQz3ETDronoeEDt.json](../../../../../../packsJson/style/Style__Personality_TOQz3ETDronoeEDt.json) |
...
| SHA-256 файла | 0d57aa68f7b21e71a83dc624b8398afa72f22ccf4d849b5c82512d80818df8a2 |

## Назначение файла

Черты характера: десять текстовых вариантов для описания персонажа. Файл задаёт таблицу `Style: Personality`, а не Item и не ActiveEffect. Собственных обработчиков изменения Actor и игровых бонусов в нём нет.
...
| _stats | 245–252: coreVersion 13.341; compendiumSource `RollTable.ygsDW95JDZPPL8oA`; systemId/systemVersion/duplicateSource/exportSource null | История происхождения документа, не адрес вложенного броска |
...
Все результаты имеют _key вида `!tables.results!TOQz3ETDronoeEDt.<ID результата>`. Повтор тех же десяти локальных ID в соседних таблицах допустим: родитель входит в _key и UUID; 70 вложенных UUID серии различаются. В каждой отдельной таблице ID не повторяются.

| Значение d10 / диапазон [n,n] | ID результата | Точное description из JSON | Строки записи |
| --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | `Personality: Secretive` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | `Personality: Rebellious` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | `Personality: Violent` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | `Personality: Idealistic` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | `Personality: Contemplative` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | `Personality: Stern` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | `Personality: Deceptive` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | `Personality: Friendly` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | `Personality: Arrogant` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | `Personality: Nervous` | 217–239 |
...
| --- | --- | --- |
| [packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json](../../../../../../packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json) | results[1], ID `6eMBIJMQZmhaVyCr`; documentUuid, строка 54 | Прямая ссылка на этот RollTable. При рекурсивном roll генератора получается один из его текстовых результатов |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Style: Personality` | Условный потребитель checkIfItemHasRollTable при экспорте добычи. Подбирает таблицу по индексу/имени; текстовый результат не превращается в Item, выдаётся exportLootInvalidItemError |
...
| --- | --- | --- |
| [Style and Values](../character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json.md) | `6eMBIJMQZmhaVyCr` | 54; [1,1] |
...
Style: Personality: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
