# packsJson/style/Values__Valued_Person_y1WCi6n2Kpwqb27P.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/style/Values__Valued_Person_y1WCi6n2Kpwqb27P.json](../../../../../../packsJson/style/Values__Valued_Person_y1WCi6n2Kpwqb27P.json) |
...
| SHA-256 файла | 2e19da243bc10cecd030b65dc8c13a7141587228fa3d36380704669367cc4d94 |

## Назначение файла

Значимый человек: десять текстовых вариантов для описания персонажа. Файл задаёт таблицу `Values: Valued Person`, а не Item и не ActiveEffect. Собственных обработчиков изменения Actor и игровых бонусов в нём нет.
...
| _stats | 245–252: coreVersion 13.341; compendiumSource `RollTable.iBJANCiKuGMIhzMx`; systemId/systemVersion/duplicateSource/exportSource null | История происхождения документа, не адрес вложенного броска |
...
Все результаты имеют _key вида `!tables.results!y1WCi6n2Kpwqb27P.<ID результата>`. Повтор тех же десяти локальных ID в соседних таблицах допустим: родитель входит в _key и UUID; 70 вложенных UUID серии различаются. В каждой отдельной таблице ID не повторяются.

| Значение d10 / диапазон [n,n] | ID результата | Точное description из JSON | Строки записи |
| --- | --- | --- | --- |
| 1 | `q0DM4FW7XCnFdKZu` | `Valued Person: A Parent` | 10–32 |
| 2 | `oKZeEBIq2FLMNk2e` | `Valued Person: A Sibling` | 33–55 |
| 3 | `kQX7ZeIIWKy5yTPF` | `Valued Person: A Lover` | 56–78 |
| 4 | `36ROPw9QWjYWmpBV` | `Valued Person: A Friend` | 79–101 |
| 5 | `9kVdEtFeX4a3nAoe` | `Valued Person: Yourself` | 102–124 |
| 6 | `cQfRuoxqB1FI58LA` | `Valued Person: A Pet` | 125–147 |
| 7 | `th9s2aW3pwS3ZZX2` | `Valued Person: A Mentor` | 148–170 |
| 8 | `tfUjaxwfTwqW98vG` | `Valued Person: A Public Figure` | 171–193 |
| 9 | `XFy2l49tSxFMwvUu` | `Valued Person: A Personal Hero` | 194–216 |
| 10 | `NlfhnGUfAnLtImRU` | `Valued Person: No One` | 217–239 |
...
| --- | --- | --- |
| [packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json](../../../../../../packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json) | results[4], ID `7LFO8Ez5nQK98SWT`; documentUuid, строка 126 | Прямая ссылка на этот RollTable. При рекурсивном roll генератора получается один из его текстовых результатов |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Values: Valued Person` | Условный потребитель checkIfItemHasRollTable при экспорте добычи. Подбирает таблицу по индексу/имени; текстовый результат не превращается в Item, выдаётся exportLootInvalidItemError |
...
| --- | --- | --- |
| [Style and Values](../character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json.md) | `7LFO8Ez5nQK98SWT` | 126; [1,1] |
...
Values: Valued Person: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
