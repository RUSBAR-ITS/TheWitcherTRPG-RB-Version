# packsJson/character-generator-sub-tables/Siblings__Feelings_About_You_ACf1JfCfVtJWw5DG.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Siblings__Feelings_About_You_ACf1JfCfVtJWw5DG.json](../../../../../../packsJson/character-generator-sub-tables/Siblings__Feelings_About_You_ACf1JfCfVtJWw5DG.json) |
...
| SHA-256 файла | 0663584ed31915141c2159848d56b969bc0f87d2744aeba551cb42b5226adb0f |

## Назначение файла

Описывает отношение одного брата или сестры к персонажу. Имя документа — `Siblings: Feelings About You`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.ACf1JfCfVtJWw5DG`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Siblings: Feelings About You`; `ACf1JfCfVtJWw5DG`; `!tables!ACf1JfCfVtJWw5DG` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.s7HqHUdoybIKo9GX`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!ACf1JfCfVtJWw5DG.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `ciSfEh9nn9LIoeuR` | [1,1] | text | `Wants You Dead` | 10–32 |
| 2 | `yMqVK3tiav0L8fQk` | [2,2] | text | `Can’t Stand You` | 33–55 |
| 3 | `j2lDuepiTSG7Pwem` | [3,3] | text | `Jealous of You` | 56–78 |
| 4 | `YN5dkMg8cg9oF81P` | [4,4] | text | `No Feelings About You` | 79–101 |
| 5 | `d5F4mYU9fpQwiuB1` | [5,5] | text | `No Feelings About You` | 102–124 |
| 6 | `htXu6vKZbTPVjQtn` | [6,6] | text | `No Feelings About You` | 125–147 |
| 7 | `woyxU9KOvRD10vrT` | [7,7] | text | `No Feelings About You` | 148–170 |
| 8 | `7F8XPaw6iN19ZxYz` | [8,8] | text | `Likes You` | 171–193 |
| 9 | `K1FjnYrs9T6QF8lr` | [9,9] | text | `Looks Up To You` | 194–216 |
| 10 | `Lc1qzvjevzlL9Lvg` | [10,10] | text | `Possessive of You` | 217–239 |

Десять ID с одиночными диапазонами. No Feelings About You повторяется на 4–7 и получает совокупные 40%; это отдельные допустимые записи. Остальные шесть вариантов имеют по 10%.
...
| --- | --- | --- |
| [packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json](../../../../../../packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json) | `x8daSlTDdqBl39P0`; строка 80; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Siblings: Feelings About You` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Siblings Generator](../character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json.md) | `x8daSlTDdqBl39P0` | 80; [1,1] |
...
Siblings: Feelings About You: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json](../character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json.md), [module/data/item/homelandData.js](../../module/data/item/homelandData.js.md), [module/data/item/professionData.js](../../module/data/item/professionData.js.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).
