# packsJson/character-generator-sub-tables/Siblings__Age_0PcW1kFO5g40cELc.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Siblings__Age_0PcW1kFO5g40cELc.json](../../../../../../packsJson/character-generator-sub-tables/Siblings__Age_0PcW1kFO5g40cELc.json) |
...
| SHA-256 файла | 3be5dc2e6589a92f8730253df12efc4c5e5dcb07cc0fbe65ce249ee2e5f1ae05 |

## Назначение файла

Описывает возраст одного брата или сестры относительно персонажа. Имя документа — `Siblings: Age`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.0PcW1kFO5g40cELc`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Siblings: Age`; `0PcW1kFO5g40cELc`; `!tables!0PcW1kFO5g40cELc` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.uGmjGxzhFDSpuxI4`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!0PcW1kFO5g40cELc.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `ciSfEh9nn9LIoeuR` | [1,5] | text | `Younger` | 10–32 |
| 2 | `yMqVK3tiav0L8fQk` | [6,9] | text | `Older` | 33–55 |
| 3 | `j2lDuepiTSG7Pwem` | [10,10] | text | `Twin` | 56–78 |

Диапазоны Younger 1–5, Older 6–9, Twin 10 дают вероятности 50%, 40%, 10%. Возраст числом не рассчитывается. normalize по трём весам построит 1d3 и изменит распределение.
...
| --- | --- | --- |
| [packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json](../../../../../../packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json) | `6wmO1a81DMV7foRw`; строка 56; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Siblings: Age` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Siblings Generator](../character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json.md) | `6wmO1a81DMV7foRw` | 56; [1,1] |
...
Siblings: Age: 1d10, возможные totals 1…10; 3 результатов (3 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
