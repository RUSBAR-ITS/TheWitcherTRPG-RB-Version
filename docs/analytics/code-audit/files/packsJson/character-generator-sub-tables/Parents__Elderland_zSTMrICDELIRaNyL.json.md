# packsJson/character-generator-sub-tables/Parents__Elderland_zSTMrICDELIRaNyL.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Parents__Elderland_zSTMrICDELIRaNyL.json](../../../../../../packsJson/character-generator-sub-tables/Parents__Elderland_zSTMrICDELIRaNyL.json) |
| Тип файла | JSON: экспорт RollTable, 4 TableResult (2 text, 2 document) |
| Статус анализа | Проверено |
| Дата проверки | 2026-09-16: актуализация технических обращений по issue-00001 |
| Ветка и коммит | `dev`, база `d8e0e1ad1159cb71a8769b0353f812de6f1ed4d8` + незакоммиченное исправление issue-00001 |
| Изменения относительно коммита | issue-00001; текущие технические обращения актуализированы. Прежние опыты ниже относятся к своим датам. |
| Задача и порция | [TASK-0003.054](../../../../../tasks/task-0003.054.md); 35 файлов / 8319 строк; этот файл — 124 строк |
| Запись перекрёстной сверки | [TASK-0003.054](../../../review-log.md#task-0003054) |
| SHA-256 файла | 13ddbd07855f46c976944c458dc3453cc4ec58bde284c3b62aed0975cc920035 |
...
Выбирает состояние родителей старших рас и раскрывает семейное положение либо судьбу родителей. Имя документа — `Parents: Elderland`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.zSTMrICDELIRaNyL`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Parents: Elderland`; `zSTMrICDELIRaNyL`; `!tables!zSTMrICDELIRaNyL` | Имя, идентификатор и ключ корневого RollTable |
| results | 4 вложенных документов, полный перечень ниже | Локальные ID уникальны внутри родителя; порядок определяет порядок выбранных результатов |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.zAohHgsPBN7zHlSl`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!zSTMrICDELIRaNyL.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `L8reTnv3nbQNgi0e` | [1,1] | text | `Your Parents Are Alive` | 10–32 |
| 2 | `MWl2EoIWD33CT3oV` | [1,1] | document | `Family Status: Elderland` | 33–56 |
| 3 | `liefj3VOt3jFZnfH` | [2,2] | text | `Something Happened To Your Parents` | 57–79 |
| 4 | `a6YCIDdtwpoL5ytY` | [2,2] | document | `Parental Fate: Elderland` | 80–103 |

На 1 выбираются текст Your Parents Are Alive и Family Status: Elderland; на 2 — текст Something Happened To Your Parents и Parental Fate: Elderland. По 50% для каждой группы. Первая группа даёт три текста; состав второй зависит от подтаблицы судьбы.
...
| RollTable.normalize({save:false}) | По 4 единичным весам создаёт клон с 1d4 и одиночными диапазонами | Меняет формулу/диапазоны; для составных выдач разделяет совместно выбранные записи; save=true записал бы изменения |
...
| --- | --- | --- |
| `MWl2EoIWD33CT3oV`, строка 53 | [Family Status: Elderland](Family_Status__Elderland_GOp8mGE4MiGaqjk2.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.GOp8mGE4MiGaqjk2` |
| `a6YCIDdtwpoL5ytY`, строка 100 | [Parental Fate: Elderland](Parental_Fate__Elderland_jZVPaCIoQxFZiyRu.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.jZVPaCIoQxFZiyRu` |

## Известные потребители

| Файл / компонент | Запись / используемые данные | Условия |
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json](../../../../../../packsJson/character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json) | `VySuFOPm5I8jY8r9`; строка 76; range=[1,1] | При recursive:true вызывает эту RollTable |
| [packsJson/character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json](../../../../../../packsJson/character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json) | `VySuFOPm5I8jY8r9`; строка 76; range=[1,1] | При recursive:true вызывает эту RollTable |
| [packsJson/character-generator-sub-tables/Family_and_Parents__Elderland_d7NLtNEdvkagBLOP.json](../../../../../../packsJson/character-generator-sub-tables/Family_and_Parents__Elderland_d7NLtNEdvkagBLOP.json) | `VySuFOPm5I8jY8r9`; строка 53; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Parents: Elderland` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Background Generator: Dwarves](../character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json.md) | `VySuFOPm5I8jY8r9` | 76; [1,1] |
| [Background Generator: Elves](../character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json.md) | `VySuFOPm5I8jY8r9` | 76; [1,1] |
