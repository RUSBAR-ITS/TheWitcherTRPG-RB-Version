# packsJson/character-generator-sub-tables/Nilfgaard_Vassal_Origin_IEPMPHNGahrfyCWI.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Nilfgaard_Vassal_Origin_IEPMPHNGahrfyCWI.json](../../../../../../packsJson/character-generator-sub-tables/Nilfgaard_Vassal_Origin_IEPMPHNGahrfyCWI.json) |
...
| SHA-256 файла | 640effc9e61482117226434904a3f123c1aadf931a79f8be6f23943cb4e724fc |

## Назначение файла

Выбирает одно из десяти вассальных государств Нильфгаарда с текстовым указанием бонуса. Имя документа — `Nilfgaard Vassal Origin`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.IEPMPHNGahrfyCWI`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Nilfgaard Vassal Origin`; `IEPMPHNGahrfyCWI`; `!tables!IEPMPHNGahrfyCWI` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.RLTWo7SY0YCx4Oqo`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!IEPMPHNGahrfyCWI.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `8UG84FSvmBKRmwHI` | [1,1] | text | `Vicovaro (+1 Education)` | 10–32 |
| 2 | `YxO7ky9eSVJUOI3N` | [2,2] | text | `Angren (+1 Wilderness Survival)` | 33–55 |
| 3 | `Qch5z1vn6YQagvCO` | [3,3] | text | `Nazair (+1 Brawling)` | 56–78 |
| 4 | `2zCNPnCinG46jtWE` | [4,4] | text | `Mettina (+1 Ride)` | 79–101 |
| 5 | `zr9L8SkmKgKC3FJH` | [5,5] | text | `Mag Turga (+1 Endurance)` | 102–124 |
| 6 | `Fk8gK60UrUNEvhTk` | [6,6] | text | `Gheso (+1 Stealth)` | 125–147 |
| 7 | `M2HENBWyZLf6ss7y` | [7,7] | text | `Ebbing (+1 Deduction)` | 148–170 |
| 8 | `IpJb93nRxSWccCmT` | [8,8] | text | `Maecht (+1 Charisma)` | 171–193 |
| 9 | `jCsO4hX1EfeRlY8p` | [9,9] | text | `Gemmeria (+1 Intimidation)` | 194–216 |
| 10 | `BJSnWUWuLLZdzDQF` | [10,10] | text | `Etolia (+1 Courage)` | 217–239 |

Десять равновероятных записей; текст не имеет общего префикса Origin. Заголовок добавляет вызывающая Origin: Nilfgaard. Название земли и +1 к навыку не задают значения полей Actor/Item.
...
| --- | --- | --- |
| [packsJson/character-generator-sub-tables/Origin__Nilfgaard_DAfZ8BGKmclyyFYc.json](../../../../../../packsJson/character-generator-sub-tables/Origin__Nilfgaard_DAfZ8BGKmclyyFYc.json) | `cFrOuK1g41ekdakj`; строка 76; range=[4,10] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Nilfgaard Vassal Origin` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
Nilfgaard Vassal Origin: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator-sub-tables/Origin__Nilfgaard_DAfZ8BGKmclyyFYc.json](Origin__Nilfgaard_DAfZ8BGKmclyyFYc.json.md), [module/data/item/homelandData.js](../../module/data/item/homelandData.js.md), [module/data/item/professionData.js](../../module/data/item/professionData.js.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).
