# packsJson/character-generator-sub-tables/Dwarf___Gnome_Profession_TjKh0NMxyyYxjSDt.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Dwarf___Gnome_Profession_TjKh0NMxyyYxjSDt.json](../../../../../../packsJson/character-generator-sub-tables/Dwarf___Gnome_Profession_TjKh0NMxyyYxjSDt.json) |
| Тип файла | JSON: экспорт RollTable, 7 TableResult (7 text, 0 document) |
| Статус анализа | Проверено |
| Дата проверки | 2026-09-12 |
| Ветка и коммит | rusbar-main, 63e9a79fefa7743fcf709b2fa19ddbe144f353a0 |
| Изменения относительно коммита | Нет; совпадает со срезом TASK-0001 15da5b225535e34af4e132c701b5353ef4eb667f |
| Задача и порция | [TASK-0003.054](../../../../../tasks/task-0003.054.md); 35 файлов / 8319 строк; этот файл — 188 строк |
| Запись перекрёстной сверки | [TASK-0003.054](../../../review-log.md#task-0003054) |
| SHA-256 файла | 72af178bfd4e1586acc678868fa207b328c76450c15e3761a73de1b10ea2513f |

## Назначение файла

Выбирает текст профессии для дворфа/гнома из семи вариантов. Имя документа — `Dwarf / Gnome Profession`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.TjKh0NMxyyYxjSDt`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Dwarf / Gnome Profession`; `TjKh0NMxyyYxjSDt`; `!tables!TjKh0NMxyyYxjSDt` | Имя, идентификатор и ключ корневого RollTable |
| results | 7 вложенных документов, полный перечень ниже | Локальные ID уникальны внутри родителя; порядок определяет порядок выбранных результатов |
| formula, replacement, displayRoll | 1d7, true, false | Бросок выбора, сохранение доступности результатов и показ основного броска в собственном сообщении |
| img, description | `icons/svg/d20-grey.svg`; описание отдельно ниже | Корневая иконка и дополнительный текст таблицы |
| flags | `{}` | Сохранённые пространства флагов; настроек автоматизации внутри нет |
| _stats | coreVersion=13.341; compendiumSource=`null`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!TjKh0NMxyyYxjSDt.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `txWHzHySbKVqvVJy` | [1,1] | text | `Your profession is: Bard` | 7–29 |
| 2 | `gknOuUjTzhNobwBf` | [2,2] | text | `Your profession is: Craftsman` | 30–52 |
| 3 | `4KzVuwtnuwZv7YtD` | [3,3] | text | `Your profession is: Criminal` | 53–75 |
| 4 | `nazai4pjRrZQol7v` | [4,4] | text | `Your profession is: Doctor` | 76–98 |
| 5 | `1ESjbfj4crmoTCGI` | [5,5] | text | `Your profession is: Man At Arms` | 99–121 |
| 6 | `mq2PwohwHs5RnB5N` | [6,6] | text | `Your profession is: Merchant` | 122–144 |
| 7 | `TAqa3U3Hkb74BuhC` | [7,7] | text | `Your profession is: Noble` | 145–167 |

Семь одиночных диапазонов 1–7 дают по 1/7. По сравнению с Elf/Human отсутствуют Mage и Priest; это содержание списка, соответствие расовым ограничениям рулбука не проверялось. Прямой потребитель найден также у Background Generator: Halfling, несмотря на название Dwarf / Gnome.
...
| --- | --- | --- |
| RollTable.getResultsForRoll(value) | Находит все записи с drawn=false и диапазоном, включающим value | Порядок соответствует JSON; при 0 и 8 массив пуст |
| RollTable.roll({recursive}) | Исполняет 1d7; при recursive:true разрешает documentUuid на RollTable и раскрывает вложенный roll | Стандартно recursive=true; без подготовки чата; заданная formula не требует нормализации |
...
| RollTable.normalize({save:false}) | По 7 единичным весам создаёт клон с 1d7 и одиночными диапазонами | Формула и диапазоны здесь сохраняются; save=true записал бы изменения |
...
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json](../../../../../../packsJson/character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json) | `fHqJQkniV2WVHHIf`; строка 124; range=[1,2] | При recursive:true вызывает эту RollTable |
| [packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json](../../../../../../packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json) | `pbEqX2RnJhB453Ch`; строка 128; range=[1,2] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Dwarf / Gnome Profession` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
Шанс каждой отдельной записи определяется шириной её range относительно 7 равновероятных граней. При перекрытии выбираются все записи; сумма их отдельных вероятностей не обязана равняться 100%. Одинаковые тексты с разными ID остаются отдельными результатами. Системные флаги этой таблицы не задают отдельной обработки описанных правил.
...
| --- | --- | --- |
| [Background Generator: Dwarves](../character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json.md) | `fHqJQkniV2WVHHIf` | 124; [1,2] |
| [Background Generator: Halfling](../character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json.md) | `pbEqX2RnJhB453Ch` | 128; [1,2] |
...
Dwarf / Gnome Profession: 1d7, возможные totals 1…7; 7 результатов (7 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 2. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json](../character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json.md), [packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json](../character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json.md), [module/data/item/homelandData.js](../../module/data/item/homelandData.js.md), [module/data/item/professionData.js](../../module/data/item/professionData.js.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).
