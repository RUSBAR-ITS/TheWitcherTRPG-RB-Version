# packsJson/character-generator-sub-tables/Siblings__Gender_QbkrG0W11fREICK8.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Siblings__Gender_QbkrG0W11fREICK8.json](../../../../../../packsJson/character-generator-sub-tables/Siblings__Gender_QbkrG0W11fREICK8.json) |
...
| SHA-256 файла | 4e012d7b4f94627c42fbcce5d1cfd52c040a9f22b7325ac24c29b95fbf8f9f4c |

## Назначение файла

Описывает гендер одного брата или сестры. Имя документа — `Siblings: Gender`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.QbkrG0W11fREICK8`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Siblings: Gender`; `QbkrG0W11fREICK8`; `!tables!QbkrG0W11fREICK8` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.4u81YQ3O7rLVcYTQ`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!QbkrG0W11fREICK8.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `ciSfEh9nn9LIoeuR` | [1,4] | text | `Male` | 10–32 |
| 2 | `Gl1XVUlHbuQVyANO` | [5,8] | text | `Female` | 33–55 |
| 3 | `WeN79l7ug6yDM7Qf` | [9,10] | text | `Gender Diverse` | 56–78 |

Диапазоны Male 1–4, Female 5–8, Gender Diverse 9–10 дают 40%, 40%, 20%. normalize построит 1d3 с равными шансами.
...
| --- | --- | --- |
| [packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json](../../../../../../packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json) | `lHDeQbKOzEyvSCTK`; строка 32; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Siblings: Gender` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Siblings Generator](../character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json.md) | `lHDeQbKOzEyvSCTK` | 32; [1,1] |
...
Siblings: Gender: 1d10, возможные totals 1…10; 3 результатов (3 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
