# packsJson/character-generator-sub-tables/Siblings__Dwarves_Halflings_Ty1Hs3G4BXkkTu67.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Siblings__Dwarves_Halflings_Ty1Hs3G4BXkkTu67.json](../../../../../../packsJson/character-generator-sub-tables/Siblings__Dwarves_Halflings_Ty1Hs3G4BXkkTu67.json) |
...
| SHA-256 файла | 0211089e235f5f0eee9595e8c4e56d6c5ba66bb5eadfbe93672280faa4de7df2 |

## Назначение файла

Выбирает текст количества братьев и сестёр дворфа/низушка: от одного до пяти либо единственный ребёнок. Имя документа — `Siblings: Dwarves/Halflings`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.Ty1Hs3G4BXkkTu67`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Siblings: Dwarves/Halflings`; `Ty1Hs3G4BXkkTu67`; `!tables!Ty1Hs3G4BXkkTu67` | Имя, идентификатор и ключ корневого RollTable |
...
| img, description | `icons/sundries/flags/banner-silver-white.webp`; описание отдельно ниже | Корневая иконка и дополнительный текст таблицы |
| flags | `{"better-rolltables":{},"core":{}}` | Сохранённые пространства флагов; настроек автоматизации внутри нет |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.nLShtklKnMHxebva`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!Ty1Hs3G4BXkkTu67.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `uZEDUG0b1QS3X7aE` | [1,1] | text | `You have 1 Sibling` | 10–32 |
| 2 | `owOV7YfwzHeKjQgE` | [2,2] | text | `You have 2 Siblings` | 33–55 |
| 3 | `5qNX6NeAArgThim7` | [3,3] | text | `You have 3 Siblings` | 56–78 |
| 4 | `nIYe2CnYdX2javLK` | [4,4] | text | `You have 4 Siblings` | 79–101 |
| 5 | `r1LxXfZHW30aoSVf` | [5,5] | text | `You have 5 Siblings` | 102–124 |
| 6 | `RBBFLtK9dH9FBcyJ` | [6,6] | text | `You are an only child` | 125–147 |
| 7 | `H0o6cOQwByLAlDxW` | [7,7] | text | `You are an only child` | 148–170 |
| 8 | `pWN64mjZ9JqI6UZR` | [8,8] | text | `You are an only child` | 171–193 |
| 9 | `kfXUMiqdlmBeSlbX` | [9,9] | text | `You are an only child` | 194–216 |
| 10 | `cHIc5jk2KytQfMji` | [10,10] | text | `You are an only child` | 217–239 |

При 1–5 текст сообщает соответствующее число братьев/сестёр, по 10% каждый; пять разных ID на 6–10 содержат одинаковое You are an only child (суммарно 50%). Результаты целиком совпадают с Siblings: Nilfgaard; корневая иконка — серебристо-белое знамя.
...
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json](../../../../../../packsJson/character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json) | `lQTRDF2dEv3hPTOh`; строка 100; range=[1,2] | При recursive:true вызывает эту RollTable |
| [packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json](../../../../../../packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json) | `dsneFT5aK1HggTKQ`; строка 104; range=[1,2] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Siblings: Dwarves/Halflings` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Background Generator: Dwarves](../character-generator/Background_Generator__Dwarves_PCAssN2Ms7yLzuCv.json.md) | `lQTRDF2dEv3hPTOh` | 100; [1,2] |
| [Background Generator: Halfling](../character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json.md) | `dsneFT5aK1HggTKQ` | 104; [1,2] |
...
Siblings: Dwarves/Halflings: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 2. Inline-выражений в результатах нет.
