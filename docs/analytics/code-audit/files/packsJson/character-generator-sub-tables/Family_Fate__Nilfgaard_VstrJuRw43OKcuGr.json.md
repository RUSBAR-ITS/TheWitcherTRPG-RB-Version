...
| SHA-256 файла | 9320f552bf9b36753df23020c419a11458259dc77f1c0a92998d37520ffc9441 |

## Назначение файла

Описывает один из десяти вариантов судьбы семьи в Нильфгаарде. Имя документа — `Family Fate: Nilfgaard`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.VstrJuRw43OKcuGr`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Family Fate: Nilfgaard`; `VstrJuRw43OKcuGr`; `!tables!VstrJuRw43OKcuGr` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.e3gdpwCQhrMc4g14`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!VstrJuRw43OKcuGr.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `WLL4GH2QBfngXygQ` | [1,1] | text | `Family Fate: Your family was indentured for crimes against the Empire or on trumped-up charges. Only you escaped.` | 10–32 |
| 2 | `4h4Sw8Y1i17AihjX` | [2,2] | text | `Family Fate: Your family was exiled to the Korath Desert and you likely spent most of your early life struggling to survive in the deadly wasteland.` | 33–55 |
| 3 | `5VXUJzVqGqVahzfy` | [3,3] | text | `Family Fate: Your family was killed by a rogue mage who either had a vendetta against your family, or just wanted blood. Either way, you are alone.` | 56–78 |
| 4 | `oQQUBuaPK1pTABqq` | [4,4] | text | `Family Fate: Your family disappeared and you have no idea where they went. One day they just up and left.` | 79–101 |
| 5 | `SdTrfAZtLN9cLgew` | [5,5] | text | `Family Fate: Your family was executed for treason against the Empire. You were the only one to escape this fate.` | 102–124 |
| 6 | `7yYGp4SSAi9ymCGL` | [6,6] | text | `Family Fate: Your family was stripped of its title for some reason. You were evicted from your home and left scrambling to survive among the un-washed masses.` | 125–147 |
| 7 | `ts84cRjou8LMlLD4` | [7,7] | text | `Family Fate: Your family name was tarnished by a magic relative who flaunted their magical gift disgracefully like a Northern mage.` | 148–170 |
| 8 | `LEz6AVGxGmTiJS5K` | [8,8] | text | `Family Fate: You disgraced your family in the eyes of the Empire. Something you did or failed to do has ruined your personal name and harmed your family.` | 171–193 |
| 9 | `X4C3zlEyKnAX96jL` | [9,9] | text | `Family Fate: Your family has a deep, dark secret that if discovered would destroy them and their name forever. You must protect this secret with your life` | 194–216 |
| 10 | `XEB17msQM6PA4Wl4` | [10,10] | text | `Family Fate: Your family was assassinated. They may have been in the way of someone’s plan or they may have been used to get at someone more powerful. Either way, your family is gone now` | 217–239 |

Десять text-результатов по 10%, исходящих documentUuid нет. Family and Parents: Nilfgaard при исходе 2 самостоятельно добавляет Family Status; эта листовая таблица возвращает только судьбу семьи.
...
| --- | --- | --- |
| [packsJson/character-generator-sub-tables/Family_and_Parents__Nilfgaard_5OcHT6WrJ8pL9cYp.json](../../../../../../packsJson/character-generator-sub-tables/Family_and_Parents__Nilfgaard_5OcHT6WrJ8pL9cYp.json) | `rx40q7d9iV7PIvy0`; строка 100; range=[2,2] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Family Fate: Nilfgaard` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
Family Fate: Nilfgaard: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
