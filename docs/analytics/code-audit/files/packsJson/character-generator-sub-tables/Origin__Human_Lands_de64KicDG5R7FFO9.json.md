# packsJson/character-generator-sub-tables/Origin__Human_Lands_de64KicDG5R7FFO9.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Origin__Human_Lands_de64KicDG5R7FFO9.json](../../../../../../packsJson/character-generator-sub-tables/Origin__Human_Lands_de64KicDG5R7FFO9.json) |
...
| Задача и порция | [TASK-0003.054](../../../../../tasks/task-0003.054.md); 35 файлов / 8319 строк; этот файл — 78 строк |
| Запись перекрёстной сверки | [TASK-0003.054](../../../review-log.md#task-0003054) |
| SHA-256 файла | 493908cc72b278ee6c54761bc249c230550604d7424276d009252ebbbe9e16a4 |
...
Выбирает северную или нильфгаардскую ветвь человеческого происхождения. Имя документа — `Origin: Human Lands`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.de64KicDG5R7FFO9`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Origin: Human Lands`; `de64KicDG5R7FFO9`; `!tables!de64KicDG5R7FFO9` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.GEHVAT1ngGLGO5Hc`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!de64KicDG5R7FFO9.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `nX4mbx8F2ITxvBNF` | [1,1] | document | `Origin: Northern Kingdom` | 10–33 |
| 2 | `yWqJljl7QFD5RvAg` | [2,2] | document | `Origin: Nilfgaard` | 34–57 |

Равновероятный выбор на d2: 1 — Origin: Northern Kingdom, 2 — Origin: Nilfgaard. Рекурсия возвращает один текст либо заголовок и вассальное происхождение; вложенная нильфгаардская ветвь имеет собственный бросок d10.
...
| --- | --- | --- |
| `nX4mbx8F2ITxvBNF`, строка 30 | [Origin: Northern Kingdom](Origin__Northern_Kingdom_u0EwVGZtkHA4Knoa.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.u0EwVGZtkHA4Knoa` |
| `yWqJljl7QFD5RvAg`, строка 54 | [Origin: Nilfgaard](Origin__Nilfgaard_DAfZ8BGKmclyyFYc.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.DAfZ8BGKmclyyFYc` |

## Известные потребители

| Файл / компонент | Запись / используемые данные | Условия |
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json](../../../../../../packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json) | `0dQ3NR8YsZjPzHul`; строка 56; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Origin: Human Lands` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Background Generator: Halfling](../character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json.md) | `0dQ3NR8YsZjPzHul` | 56; [1,1] |
...
Origin: Human Lands: 1d2, возможные totals 1…2; 2 результатов (0 text / 2 document), replacement=true, displayRoll=true. Рекурсивные переходы: Origin: Northern Kingdom; Origin: Nilfgaard. По разрешимому графу от этого прямого входа 1–2 конечных текстов, максимальная глубина 2; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json](../character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json.md), [packsJson/character-generator-sub-tables/Origin__Northern_Kingdom_u0EwVGZtkHA4Knoa.json](Origin__Northern_Kingdom_u0EwVGZtkHA4Knoa.json.md), [packsJson/character-generator-sub-tables/Origin__Nilfgaard_DAfZ8BGKmclyyFYc.json](Origin__Nilfgaard_DAfZ8BGKmclyyFYc.json.md), [module/data/item/homelandData.js](../../module/data/item/homelandData.js.md), [module/data/item/professionData.js](../../module/data/item/professionData.js.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).
