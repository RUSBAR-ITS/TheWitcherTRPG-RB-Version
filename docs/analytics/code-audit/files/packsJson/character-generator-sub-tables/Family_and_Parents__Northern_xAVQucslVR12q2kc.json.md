# packsJson/character-generator-sub-tables/Family_and_Parents__Northern_xAVQucslVR12q2kc.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Family_and_Parents__Northern_xAVQucslVR12q2kc.json](../../../../../../packsJson/character-generator-sub-tables/Family_and_Parents__Northern_xAVQucslVR12q2kc.json) |
...
| SHA-256 файла | 4e731c18aa573238ee3520335a1766831a32c0803d8d23a16d9effbf220ad32e |
...
Составляет сведения о семье и родителях на Севере через связанные подтаблицы. Имя документа — `Family and Parents: Northern`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.xAVQucslVR12q2kc`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Family and Parents: Northern`; `xAVQucslVR12q2kc`; `!tables!xAVQucslVR12q2kc` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.puzfClwZNrRti88d`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!xAVQucslVR12q2kc.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| 2 | `VySuFOPm5I8jY8r9` | [1,1] | document | `Parents: Northern` | 33–56 |
| 3 | `g4JnOCAOpDRIw8tQ` | [2,2] | text | `Something Happened to Your Family` | 57–79 |
| 4 | `rx40q7d9iV7PIvy0` | [2,2] | document | `Family Fate: Northern` | 80–103 |

При 1 выбираются текст и Parents: Northern; при 2 — текст и Family Fate: Northern. Прямой ссылки на Family Status здесь нет: на 1–9 её содержит Family Fate. Поэтому на сочетании этого исхода 2 и Fate=10 итог ограничен двумя текстами.
...
| --- | --- | --- |
| `VySuFOPm5I8jY8r9`, строка 53 | [Parents: Northern](Parents__Northern_SGuXziIZzzst1LVJ.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.SGuXziIZzzst1LVJ` |
| `rx40q7d9iV7PIvy0`, строка 100 | [Family Fate: Northern](Family_Fate__Northern_CN0lwXPHxkV2vDeY.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.CN0lwXPHxkV2vDeY` |

## Известные потребители

| Файл / компонент | Запись / используемые данные | Условия |
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Human_g9I3DpPOi5CfOFBz.json](../../../../../../packsJson/character-generator/Background_Generator__Human_g9I3DpPOi5CfOFBz.json) | `bgm46MMbKjQdbD6L`; строка 80; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Family and Parents: Northern` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Background Generator: Human](../character-generator/Background_Generator__Human_g9I3DpPOi5CfOFBz.json.md) | `bgm46MMbKjQdbD6L` | 80; [1,1] |
...
Family and Parents: Northern: 1d2, возможные totals 1…2; 4 результатов (2 text / 2 document), replacement=true, displayRoll=false. Рекурсивные переходы: Parents: Northern; Family Fate: Northern. По разрешимому графу от этого прямого входа 2–6 конечных текстов, максимальная глубина 4; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator/Background_Generator__Human_g9I3DpPOi5CfOFBz.json](../character-generator/Background_Generator__Human_g9I3DpPOi5CfOFBz.json.md), [packsJson/character-generator-sub-tables/Parents__Northern_SGuXziIZzzst1LVJ.json](Parents__Northern_SGuXziIZzzst1LVJ.json.md), [packsJson/character-generator-sub-tables/Family_Fate__Northern_CN0lwXPHxkV2vDeY.json](Family_Fate__Northern_CN0lwXPHxkV2vDeY.json.md).
