...
| SHA-256 файла | 8f8f458072a62442f4fc7ff425166a5eb07c81b20a905d630857b4a5a4627cd0 |
...
Составляет сведения о семье и родителях в землях старших рас через связанные подтаблицы. Имя документа — `Family and Parents: Elderland`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.d7NLtNEdvkagBLOP`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Family and Parents: Elderland`; `d7NLtNEdvkagBLOP`; `!tables!d7NLtNEdvkagBLOP` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.xGKpYv3tSn01WYn8`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!d7NLtNEdvkagBLOP.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| 2 | `VySuFOPm5I8jY8r9` | [1,1] | document | `Parents: Elderland` | 33–56 |
| 3 | `g4JnOCAOpDRIw8tQ` | [2,2] | text | `Something Happened to Your Family` | 57–79 |
| 4 | `rx40q7d9iV7PIvy0` | [2,2] | document | `Family Fate: Elderland` | 80–103 |
| 5 | `hNdtp3IHu6NaWkJo` | [2,2] | document | `Family Status: Elderland` | 104–127 |

При 1 выбираются текст о живой семье и Parents: Elderland; при 2 — текст о событии, Family Fate и Family Status. На втором пути Fate не содержит собственной ссылки на статус, поэтому отдельная ссылка обеспечивает его выбор. Через первый путь полная семейная цепочка может содержать шесть текстов.
...
| --- | --- | --- |
| `VySuFOPm5I8jY8r9`, строка 53 | [Parents: Elderland](Parents__Elderland_zSTMrICDELIRaNyL.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.zSTMrICDELIRaNyL` |
| `rx40q7d9iV7PIvy0`, строка 100 | [Family Fate: Elderland](Family_Fate__Elderland_W6mgmCP3219eYdf0.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.W6mgmCP3219eYdf0` |
| `hNdtp3IHu6NaWkJo`, строка 124 | [Family Status: Elderland](Family_Status__Elderland_GOp8mGE4MiGaqjk2.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.GOp8mGE4MiGaqjk2` |

## Известные потребители

| Файл / компонент | Запись / используемые данные | Условия |
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json](../../../../../../packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json) | `L1KBkf196CGL9GXv`; строка 80; range=[1,1] | При recursive:true вызывает эту RollTable |
| [packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json](../../../../../../packsJson/character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json) | `Lq6SDOHwOyB0ZrZt`; строка 199; range=[2,2] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Family and Parents: Elderland` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Background Generator: Halfling](../character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json.md) | `L1KBkf196CGL9GXv` | 80; [1,1] |
| [Background Generator: Halfling](../character-generator/Background_Generator__Halfling_ioZTpiW6W2eVwRiY.json.md) | `Lq6SDOHwOyB0ZrZt` | 199; [2,2] |
...
Family and Parents: Elderland: 1d2, возможные totals 1…2; 5 результатов (2 text / 3 document), replacement=true, displayRoll=false. Рекурсивные переходы: Parents: Elderland; Family Fate: Elderland; Family Status: Elderland. По разрешимому графу от этого прямого входа 4–6 конечных текстов, максимальная глубина 4; входящих файлов 1. Inline-выражений в результатах нет.
