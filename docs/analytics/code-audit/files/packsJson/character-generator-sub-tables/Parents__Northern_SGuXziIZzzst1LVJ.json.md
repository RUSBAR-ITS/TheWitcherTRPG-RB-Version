# packsJson/character-generator-sub-tables/Parents__Northern_SGuXziIZzzst1LVJ.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Parents__Northern_SGuXziIZzzst1LVJ.json](../../../../../../packsJson/character-generator-sub-tables/Parents__Northern_SGuXziIZzzst1LVJ.json) |
...
| SHA-256 файла | 3214f95a95cc2e23805e6cc2ff5a010c134700cf2bf4112ebd032d50da27640e |
...
Выбирает состояние родителей на Севере и раскрывает семейное положение либо судьбу родителей. Имя документа — `Parents: Northern`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.SGuXziIZzzst1LVJ`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Parents: Northern`; `SGuXziIZzzst1LVJ`; `!tables!SGuXziIZzzst1LVJ` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.g5NAULNGDXrVJKTf`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!SGuXziIZzzst1LVJ.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| 2 | `MWl2EoIWD33CT3oV` | [1,1] | document | `Family Status: Northern` | 33–56 |
| 3 | `liefj3VOt3jFZnfH` | [2,2] | text | `Something Happened To Your Parents` | 57–79 |
| 4 | `a6YCIDdtwpoL5ytY` | [2,2] | document | `Parental Fate: Northern` | 80–103 |

На 1 выбираются текст Your Parents Are Alive и Family Status: Northern; на 2 — текст Something Happened To Your Parents и Parental Fate: Northern. По 50% для каждой группы. Первая группа даёт три текста; состав второй зависит от подтаблицы судьбы.
...
| --- | --- | --- |
| `MWl2EoIWD33CT3oV`, строка 53 | [Family Status: Northern](Family_Status__Northern_EeOlp8UMRiYS1AEt.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.EeOlp8UMRiYS1AEt` |
| `a6YCIDdtwpoL5ytY`, строка 100 | [Parental Fate: Northern](Parental_Fate__Northern_FQEyr6n57ae1ySLC.json.md) | `Compendium.TheWitcherTRPG-RB-Version.Character-gen_Sub-tables.RollTable.FQEyr6n57ae1ySLC` |

## Известные потребители

| Файл / компонент | Запись / используемые данные | Условия |
| --- | --- | --- |
| [packsJson/character-generator-sub-tables/Family_and_Parents__Northern_xAVQucslVR12q2kc.json](../../../../../../packsJson/character-generator-sub-tables/Family_and_Parents__Northern_xAVQucslVR12q2kc.json) | `VySuFOPm5I8jY8r9`; строка 53; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Parents: Northern` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
