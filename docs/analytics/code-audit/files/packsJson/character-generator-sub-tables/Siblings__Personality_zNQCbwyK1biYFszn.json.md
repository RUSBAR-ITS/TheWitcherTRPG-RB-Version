# packsJson/character-generator-sub-tables/Siblings__Personality_zNQCbwyK1biYFszn.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Siblings__Personality_zNQCbwyK1biYFszn.json](../../../../../../packsJson/character-generator-sub-tables/Siblings__Personality_zNQCbwyK1biYFszn.json) |
...
| SHA-256 файла | 6cc73b3f57db2e9141a19a38a7b0e9eb2ce4f674dda649930992220a9307feb4 |

## Назначение файла

Описывает характер одного брата или сестры. Имя документа — `Siblings: Personality`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.zNQCbwyK1biYFszn`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Siblings: Personality`; `zNQCbwyK1biYFszn`; `!tables!zNQCbwyK1biYFszn` | Имя, идентификатор и ключ корневого RollTable |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.tsFYQoIe3uxJ2tPX`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!zNQCbwyK1biYFszn.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `ciSfEh9nn9LIoeuR` | [1,1] | text | `Shy` | 10–32 |
| 2 | `yMqVK3tiav0L8fQk` | [2,2] | text | `Aggressive` | 33–55 |
| 3 | `j2lDuepiTSG7Pwem` | [3,3] | text | `Kind` | 56–78 |
| 4 | `YN5dkMg8cg9oF81P` | [4,4] | text | `Strange` | 79–101 |
| 5 | `d5F4mYU9fpQwiuB1` | [5,5] | text | `Thoughtful` | 102–124 |
| 6 | `htXu6vKZbTPVjQtn` | [6,6] | text | `Talkative` | 125–147 |
| 7 | `woyxU9KOvRD10vrT` | [7,7] | text | `Romantic` | 148–170 |
| 8 | `7F8XPaw6iN19ZxYz` | [8,8] | text | `Stern` | 171–193 |
| 9 | `K1FjnYrs9T6QF8lr` | [9,9] | text | `Depressive` | 194–216 |
| 10 | `Lc1qzvjevzlL9Lvg` | [10,10] | text | `Immature` | 217–239 |

Десять равновероятных текстовых черт характера. Отдельный Siblings Generator получает по одному результату этой таблицы, Gender, Age и Feelings About You; числом братьев/сестёр он не управляет.
...
| --- | --- | --- |
| [packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json](../../../../../../packsJson/character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json) | `ZHa5PBGJnRAbdzZr`; строка 104; range=[1,1] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Siblings: Personality` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Siblings Generator](../character-generator/Siblings_Generator_Lem0B3XxmJeEVQms.json.md) | `ZHa5PBGJnRAbdzZr` | 104; [1,1] |
...
Siblings: Personality: 1d10, возможные totals 1…10; 10 результатов (10 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
