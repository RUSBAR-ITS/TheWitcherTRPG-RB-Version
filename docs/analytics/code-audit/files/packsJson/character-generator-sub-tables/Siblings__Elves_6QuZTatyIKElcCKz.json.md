# packsJson/character-generator-sub-tables/Siblings__Elves_6QuZTatyIKElcCKz.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Siblings__Elves_6QuZTatyIKElcCKz.json](../../../../../../packsJson/character-generator-sub-tables/Siblings__Elves_6QuZTatyIKElcCKz.json) |
...
| SHA-256 файла | 6df88090f1186964e7fa24f16a7f8cd889077cd3880eccd21576e594e8ba12c3 |

## Назначение файла

Выбирает текст количества братьев и сестёр эльфа: один, два либо единственный ребёнок. Имя документа — `Siblings: Elves`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
| img, description | `icons/sundries/flags/banner-worn-green.webp`; описание отдельно ниже | Корневая иконка и дополнительный текст таблицы |
| flags | `{"better-rolltables":{},"core":{}}` | Сохранённые пространства флагов; настроек автоматизации внутри нет |
| _stats | coreVersion=13.341; compendiumSource=`RollTable.QzUqgzDS7UqXbA2X`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!6QuZTatyIKElcCKz.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| --- | --- | --- | --- | --- | --- |
| 1 | `uZEDUG0b1QS3X7aE` | [1,2] | text | `You have 1 Sibling` | 10–32 |
| 2 | `owOV7YfwzHeKjQgE` | [3,8] | text | `You are an only child` | 33–55 |
| 3 | `cHIc5jk2KytQfMji` | [9,10] | text | `You have 2 Siblings` | 56–78 |

Один брат/сестра на 1–2 (20%), единственный ребёнок на 3–8 (60%), два на 9–10 (20%). normalize построит 1d3 с равными шансами. Корневая иконка — зелёное знамя.
...
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json](../../../../../../packsJson/character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json) | `6stg4kxtfUkySns5`; строка 100; range=[1,2] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Siblings: Elves` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Background Generator: Elves](../character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json.md) | `6stg4kxtfUkySns5` | 100; [1,2] |
...
Siblings: Elves: 1d10, возможные totals 1…10; 3 результатов (3 text / 0 document), replacement=true, displayRoll=true. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.
