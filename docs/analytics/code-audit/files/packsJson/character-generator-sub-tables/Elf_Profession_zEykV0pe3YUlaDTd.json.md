# packsJson/character-generator-sub-tables/Elf_Profession_zEykV0pe3YUlaDTd.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/character-generator-sub-tables/Elf_Profession_zEykV0pe3YUlaDTd.json](../../../../../../packsJson/character-generator-sub-tables/Elf_Profession_zEykV0pe3YUlaDTd.json) |
...
| SHA-256 файла | e0544794b126876a3d4a913e2458ef4201db4003fc89ac65881ccbabde82ade8 |

## Назначение файла

Выбирает текст профессии эльфа из девяти вариантов. Имя документа — `Elf Profession`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
Адрес по манифесту и ID — `Compendium.TheWitcherTRPG.Character-gen_Sub-tables.RollTable.zEykV0pe3YUlaDTd`. Штатный лист/каталог Foundry вызывает draw; другой генератор может получить таблицу через documentUuid. При стандартном recursive:true выбираются конечные текстовые результаты, при false документные записи остаются ссылками. Наличие экспорта не доказывает состояние установленной БД или доступ пользователя.

## Введённые сущности и действия с ними

| Сущность / поля | Значение | Действия и смысл |
| --- | --- | --- |
| name, _id, _key | `Elf Profession`; `zEykV0pe3YUlaDTd`; `!tables!zEykV0pe3YUlaDTd` | Имя, идентификатор и ключ корневого RollTable |
...
| formula, replacement, displayRoll | 1d9, true, false | Бросок выбора, сохранение доступности результатов и показ основного броска в собственном сообщении |
...
| Result _id, _key | ID ниже; `!tables.results!zEykV0pe3YUlaDTd.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
Девять одиночных диапазонов 1–9 дают по 1/9. Тексты, ID, диапазоны и порядок девяти результатов совпадают с Human Profession; корневые ID/имя и displayRoll различаются. Название профессии в description не создаёт profession Item.
...
| --- | --- | --- |
| [packsJson/character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json](../../../../../../packsJson/character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json) | `zpupYNP2QGhmUGIU`; строка 124; range=[1,2] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Elf Profession` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
| --- | --- | --- |
| [Background Generator: Elves](../character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json.md) | `zpupYNP2QGhmUGIU` | 124; [1,2] |
...
Elf Profession: 1d9, возможные totals 1…9; 9 результатов (9 text / 0 document), replacement=true, displayRoll=false. Листовая выдача текста без documentUuid. По разрешимому графу от этого прямого входа 1–1 конечных текстов, максимальная глубина 0; входящих файлов 1. Inline-выражений в результатах нет.

Сопоставленные определения и потребители: [system.json](../../system.json.md), [utils/packs.mjs](../../utils/packs.mjs.md), [utils/extract.mjs](../../utils/extract.mjs.md), [module/item/witcherItem.js](../../module/item/witcherItem.js.md), [packsJson/character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json](../character-generator/Background_Generator__Elves_L8o8Rz85um05VUW4.json.md), [module/data/item/homelandData.js](../../module/data/item/homelandData.js.md), [module/data/item/professionData.js](../../module/data/item/professionData.js.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md).
