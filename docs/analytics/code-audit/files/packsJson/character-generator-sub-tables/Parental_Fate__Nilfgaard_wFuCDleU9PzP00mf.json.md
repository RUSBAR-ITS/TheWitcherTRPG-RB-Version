...
Выбирает судьбу нильфгаардских родителей и затронутого родителя; исходы 1–9 добавляют положение и знакомого. Имя документа — `Parental Fate: Nilfgaard`. Это экспорт таблицы для получения текста и раскрытия ссылок; собственных изменений параметров Actor, создания вещей или сущностей биографии в данных нет.
...
| --- | --- | --- |
| name, _id, _key | `Parental Fate: Nilfgaard`; `wFuCDleU9PzP00mf`; `!tables!wFuCDleU9PzP00mf` | Имя, идентификатор и ключ корневого RollTable |
| results | 20 вложенных документов, полный перечень ниже | Локальные ID уникальны внутри родителя; порядок определяет порядок выбранных результатов |
...
| _stats | coreVersion=13.341; compendiumSource=`RollTable.uLTZ0SXsQupensWI`; systemId/systemVersion/duplicateSource/exportSource=null | История экспорта; источник происхождения не используется рекурсивным roll |
...
| Result _id, _key | ID ниже; `!tables.results!wFuCDleU9PzP00mf.<resultId>` | Уникальные ключи вложенных документов; _key не входит в toObject модели |
...
| 2 | `k0AliUujlhUKLN0G` | [1,1] | document | `Family Status: Nilfgaard` | 33–56 |
...
| 5 | `ITgHqKEQCar6G6Et` | [2,2] | document | `Family Status: Nilfgaard` | 104–127 |
...
| 7 | `ONmmUjbSaveNjrSy` | [3,3] | document | `Family Status: Nilfgaard` | 151–174 |
...
| 9 | `R36SexxMtQ1y3Mct` | [4,4] | document | `Family Status: Nilfgaard` | 198–221 |
...
| 11 | `fbee84iwrEvwp5A7` | [5,5] | document | `Family Status: Nilfgaard` | 245–268 |
...
| 13 | `AErFz1jut68I9mna` | [6,6] | document | `Family Status: Nilfgaard` | 292–315 |
...
| 15 | `fb47S0gLn0cLA2eD` | [7,7] | document | `Family Status: Nilfgaard` | 339–362 |
...
| 17 | `98Ry4FUdzahLh10P` | [8,8] | document | `Family Status: Nilfgaard` | 386–409 |
...
| 19 | `P2j9J9NLbtkzp04u` | [9,9] | document | `Family Status: Nilfgaard` | 433–456 |
...
Общая запись Which Parent [1,10] выбирается на любой грани. На 1–9 также выбираются судьба и Family Status: Nilfgaard: в итоге четыре текста. На 10 ссылки на статус нет, остаются Which Parent и судьба. Все десять текстов судьбы совпадают с Elderland, но количество документных ссылок различается. На 1 общая запись стоит после пары судьба/статус; на 2–10 она оказывается перед текстом выбранной судьбы.
...
| RollTable.normalize({save:false}) | По 20 единичным весам создаёт клон с 1d20 и одиночными диапазонами | Меняет формулу/диапазоны; для составных выдач разделяет совместно выбранные записи; save=true записал бы изменения |
...
| --- | --- | --- |
| [packsJson/character-generator-sub-tables/Parents__Nilfgaard_Nnf1BMSJOnc5Mx3m.json](../../../../../../packsJson/character-generator-sub-tables/Parents__Nilfgaard_Nnf1BMSJOnc5Mx3m.json) | `a6YCIDdtwpoL5ytY`; строка 100; range=[2,2] | При recursive:true вызывает эту RollTable |
...
| [module/item/witcherItem.js](../../../../../../module/item/witcherItem.js):257–315 | Совпадение Item.name с `Parental Fate: Nilfgaard` | Условный поиск при экспорте добычи; метод ожидает Item, а данный граф выдаёт text |
...
Parental Fate: Nilfgaard: 1d10, возможные totals 1…10; 20 результатов (10 text / 10 document), replacement=true, displayRoll=true. Рекурсивные переходы: Family Status: Nilfgaard; Which Parent. По разрешимому графу от этого прямого входа 2–4 конечных текстов, максимальная глубина 2; входящих файлов 1. Inline-выражений в результатах нет.
