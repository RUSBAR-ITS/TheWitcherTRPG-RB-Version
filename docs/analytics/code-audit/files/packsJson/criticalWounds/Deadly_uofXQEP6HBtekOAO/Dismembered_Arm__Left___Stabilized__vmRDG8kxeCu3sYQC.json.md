# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left___Stabilized__vmRDG8kxeCu3sYQC.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left___Stabilized__vmRDG8kxeCu3sYQC.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left___Stabilized__vmRDG8kxeCu3sYQC.json) |
...
Экспорт Dismembered Arm (Left - Stabilized): состояние stabilized, локация leftArm. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Arm (Left - Stabilized) | Корневой Item; name:3, _id:30 | ID `vmRDG8kxeCu3sYQC`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.vmRDG8kxeCu3sYQC` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!vmRDG8kxeCu3sYQC"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.8Z1iHJLXrFm2i3Fb"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |
...
Item `vmRDG8kxeCu3sYQC` («Dismembered Arm (Left - Stabilized)»): `deadly/stabilized/leftArm`; 0 ActiveEffect, 0 changes. Предшественник: `KQZRzczsSx1XY63m`; `followUp` ведёт к `8Z1iHJLXrFm2i3Fb` («Dismembered Arm (Left - Treated)», `treated/leftArm`).
