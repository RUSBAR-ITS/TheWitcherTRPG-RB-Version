# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right___Stabilized__hKgvgj4lJ74wPt8N.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right___Stabilized__hKgvgj4lJ74wPt8N.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right___Stabilized__hKgvgj4lJ74wPt8N.json) |
...
Экспорт Dismembered Arm (Right - Stabilized): состояние stabilized, локация rightArm. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Arm (Right - Stabilized) | Корневой Item; name:3, _id:30 | ID `hKgvgj4lJ74wPt8N`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.hKgvgj4lJ74wPt8N` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!hKgvgj4lJ74wPt8N"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.gPeOdwZ0OTVF9E3w"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |
...
Item `hKgvgj4lJ74wPt8N` («Dismembered Arm (Right - Stabilized)»): `deadly/stabilized/rightArm`; 0 ActiveEffect, 0 changes. Предшественник: `ZxvWPJPDD9fm34Pc`; `followUp` ведёт к `gPeOdwZ0OTVF9E3w` («Dismembered Arm (Right - Treated)», `treated/rightArm`).

Этот Item отсекается начальным фильтром `treatment=none`, но доступен через ссылку предыдущего состояния. Расчёт срока оставляет экспортный `healingTime=0`; `heal` не завершает Deadly автоматически. Изменений числовых полей не задано; текст/статусы рассматриваются отдельно.
