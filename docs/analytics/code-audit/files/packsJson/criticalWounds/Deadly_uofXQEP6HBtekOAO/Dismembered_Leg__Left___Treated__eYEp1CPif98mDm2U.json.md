# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Left___Treated__eYEp1CPif98mDm2U.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Left___Treated__eYEp1CPif98mDm2U.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Left___Treated__eYEp1CPif98mDm2U.json) |
...
| Изменения относительно коммита | Нет; 37 строк; SHA-256 8ea5f4ee3e6c2516175939e9286cdc9c668c1f4f4873f3227700e8c4d0451f77 |
| Задача и порция | [TASK-0003.061](../../../../../../tasks/task-0003.061.md) |
| Запись перекрёстной сверки | [Протокол .061](../../../../review-log.md#task-0003061) |

## Назначение файла

Экспорт Dismembered Leg (Left - Treated): состояние treated, локация leftLeg. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Leg (Left - Treated) | Корневой Item; name:3, _id:30 | ID `eYEp1CPif98mDm2U`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.eYEp1CPif98mDm2U` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!eYEp1CPif98mDm2U"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
Item `eYEp1CPif98mDm2U` («Dismembered Leg (Left - Treated)»): `deadly/treated/leftLeg`; 0 ActiveEffect, 0 changes. Предшественник: `lck0EEySmuLZXMrA`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
