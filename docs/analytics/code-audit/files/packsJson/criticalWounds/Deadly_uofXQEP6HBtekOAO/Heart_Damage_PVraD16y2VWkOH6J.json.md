# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Heart_Damage_PVraD16y2VWkOH6J.json

## Текущий численный контракт — 14.3.1.00069

TASK-0010.010: численные строки находятся в `system.changes`, с `type`/`phase` и настройками канала. Классификация: native: 1, П: 3. П — параметр, Р — бросок, Р? — выбор условия; native — периодика. Все fullEffect/shiftsCap/affectsAdvancement выключены. Исключения: stats.body → sta. UUID/стадия/переходы/заживление, прочие поля Item/AE сохранены.
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.O8EM4quPU4A5VOHH"` |
...
Экспорт Heart Damage: состояние none, локация torso. Содержит 2 ActiveEffect и 4 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| description | `"<ul><li><p>The blow damages your heart. Make an immediate Death save. </p></li><li><p> quarter your Stamina, SPD, and BODY</p></li></ul>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
ID `QoRqhs1YIg3vMmOT`; _key=`"!items.effects!PVraD16y2VWkOH6J.QoRqhs1YIg3vMmOT"`. Origin=`"Item.PVraD16y2VWkOH6J"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
| 2 / 108 | `system.stats.body.max` | 1 / `"0.25"` / `null` | multiply; `0.25`; priority=10; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); 1 |
| 3 / 114 | `system.derivedStats.sta.max` | 1 / `"0.25"` / `null` | multiply; `0.25`; priority=10; phase=initial | NumberField; [derivedStatsData.js](../../../../../../../module/data/actor/templates/common/stats/derivedStatsData.js); 25 |
...
Папка: [Deadly](_Folder.json.md). Предшественников по followUp в Deadly нет. Следующий Item: [Heart Damage (Stabilized)](Heart_Damage__Stabilized__O8EM4quPU4A5VOHH.json.md), `O8EM4quPU4A5VOHH`, treatment=stabilized. Реальный treat инициировал создание этого документа, затем удаление текущего. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.

Контрольный Actor: восемь unmodifiedMax=5, HP.value=25, dodge.value=athletics.value=8 до эффектов, броня/вес=0. Результат: INT=5, WILL=5, REF=5, DEX=5, BODY=5, SPD=5; BODY.max=1, SPD.max=1; RUN=15, LEAP.value=3, LEAP.max=0, ENC=50, STUN=5, REC=5, HP.max=25, HP.value=25, RESOLVE.max=25, FOCUS.max=15, STA.max=25, STA.value=0; healingTime=0. Навыки: dodge.value=8, dodge.activeEffectModifiers=0; athletics.value=8, athletics.activeEffectModifiers=0; awareness.value=0, awareness.activeEffectModifiers=0. Статусы=`["bleed"]`; turnStartEffects={}. После initial, до расчётов: `{"staMax":0,"spdMax":1,"bodyMax":1}`. Значения прочитаны непосредственно из подготовленных полей; DataModel.toObject возвращает источник и не заменяет такую проверку.
...
Немедленный Death save указан только в HTML; эффект не вызывает бросок и не меняет HP.value. Статически проверенный обработчик листа deathSaveMixin вызывается пользователем; при HP≤0 он читает BODY.max/WILL.max, поэтому отсутствие изменения BODY.value не означает отсутствия всех последствий изменения max.
...
Item `PVraD16y2VWkOH6J` («Heart Damage»): `deadly/none/torso`; 2 ActiveEffect, 4 changes. Предшественник: нет; `followUp` ведёт к `O8EM4quPU4A5VOHH` («Heart Damage (Stabilized)», `stabilized/torso`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. Расчёт срока оставляет экспортный `healingTime=0`; `heal` не завершает Deadly автоматически. Сверены адреса: `system.combatEffects.turnStartEffects.bleed`, `system.stats.spd.max`, `system.stats.body.max`, `system.derivedStats.sta.max`.
