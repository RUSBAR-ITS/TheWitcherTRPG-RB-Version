# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right___Treated__gPeOdwZ0OTVF9E3w.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right___Treated__gPeOdwZ0OTVF9E3w.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right___Treated__gPeOdwZ0OTVF9E3w.json) |
...
| Изменения относительно коммита | Нет; 37 строк; SHA-256 d709df5faeee8fa559a99f59f831d962ee13123bdd7da0a2dc498fb84c74392a |
| Задача и порция | [TASK-0003.061](../../../../../../tasks/task-0003.061.md) |
| Запись перекрёстной сверки | [Протокол .061](../../../../review-log.md#task-0003061) |

## Назначение файла

Экспорт Dismembered Arm (Right - Treated): состояние treated, локация rightArm. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Arm (Right - Treated) | Корневой Item; name:3, _id:30 | ID `gPeOdwZ0OTVF9E3w`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.gPeOdwZ0OTVF9E3w` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!gPeOdwZ0OTVF9E3w"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>The arm can be replaced with a prosthetic.</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
Item `gPeOdwZ0OTVF9E3w` («Dismembered Arm (Right - Treated)»): `deadly/treated/rightArm`; 0 ActiveEffect, 0 changes. Предшественник: `hKgvgj4lJ74wPt8N`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
