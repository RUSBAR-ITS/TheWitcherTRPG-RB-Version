# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Stabilized__LNy3P3HUw2Ja9DNu.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Stabilized__LNy3P3HUw2Ja9DNu.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Stabilized__LNy3P3HUw2Ja9DNu.json) |
...
Экспорт Damaged Eye (Stabilized): состояние stabilized, локация head. Содержит 1 ActiveEffect и 2 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Damaged Eye (Stabilized) | Корневой Item; name:3, _id:91 | ID `LNy3P3HUw2Ja9DNu`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.LNy3P3HUw2Ja9DNu` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!LNy3P3HUw2Ja9DNu"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>-3 to sight-based Awareness</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.XQeXSAhvjrQZNpAE"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Damaged Eye

ID `a9KpYZkIQ4eyIqTP`; _key=`"!items.effects!LNy3P3HUw2Ja9DNu.a9KpYZkIQ4eyIqTP"`. Origin=`"Item.zHK1XmZ77V8c26Xv"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.stats.dex.totalModifiers` | 2 / `"-2"` / `null` | add; `-2`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -2 |
| 2 / 51 | `system.skills.int.awareness.activeEffectModifiers` | 2 / `"-3"` / `null` | add; `-3`; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js); -3 |
...
Папка: [Deadly](_Folder.json.md). Предшественник: [Damaged Eye](Damaged_Eye_zHK1XmZ77V8c26Xv.json.md). Следующий Item: [Damaged Eye (Treated)](Damaged_Eye__Treated__XQeXSAhvjrQZNpAE.json.md), `XQeXSAhvjrQZNpAE`, treatment=treated. Реальный treat инициировал создание этого документа, затем удаление текущего. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.

Контрольный Actor: восемь unmodifiedMax=5, HP.value=25, dodge.value=athletics.value=8 до эффектов, броня/вес=0. Результат: INT=5, WILL=5, REF=5, DEX=3, BODY=5, SPD=5; BODY.max=5, SPD.max=5; RUN=15, LEAP.value=3, LEAP.max=3, ENC=50, STUN=5, REC=5, HP.max=25, HP.value=25, RESOLVE.max=25, FOCUS.max=15, STA.max=25, STA.value=0; healingTime=0. Навыки: dodge.value=8, dodge.activeEffectModifiers=0; athletics.value=8, athletics.activeEffectModifiers=0; awareness.value=0, awareness.activeEffectModifiers=-3. Статусы=`[]`; turnStartEffects={}. После initial, до расчётов: `{"staMax":0,"spdMax":5,"bodyMax":5}`. Значения прочитаны непосредственно из подготовленных полей; DataModel.toObject возвращает источник и не заменяет такую проверку.

HTML ограничивает штраф Awareness проверками зрения; change адресует общий awareness.activeEffectModifiers. Настоящий addActiveEffects('awareness') вернул `" +-3[Damaged Eye]"`. В rollSkill/rollSkillCheck нет аргумента «зрение»; описание не создаёт условие автоматически. Применение или компенсация в конкретном броске остаётся за пользователем/другим механизмом. Это граница автоматизации без оценки рулбука.
