# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right__ZxvWPJPDD9fm34Pc.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.hKgvgj4lJ74wPt8N"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.gPeOdwZ0OTVF9E3w"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right__ZxvWPJPDD9fm34Pc.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right__ZxvWPJPDD9fm34Pc.json) |
...
| Имя / ID | Dismembered Arm (Right) / `ZxvWPJPDD9fm34Pc` |
| Строк / SHA-256 | 94 / `1cd991ddcd03eb54dfa4665b2e4529cb49d9f4fdc4763701d1a294b254037ed1` |

### Выполненные исправления

B16: исправить сторону в имени эффекта; B17: восстановить конкретное пропущенное условие в description. Остальные поля сохранены. Текущие значения и связи перечислены ниже; архив в конце описывает прежние срезы.

| Поле system | Текущее значение |
| --- | --- |
| `htmlFields` | ["description"] |
| `description` | "<p>The blow rends your arm from your body or damages it beyond repair. </p><p>Your right arm cannot be used. This wound induces bleeding.</p>" |
...
| `healingTime` | 0 |
| `sterilized` | false |
| `followUp` | "Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.hKgvgj4lJ74wPt8N" |

1 ActiveEffect; 1 changes. ID и `_key` сохранённых эффектов, origin, transfer, duration и несвязанные значения сохранены.

#### Dismembered Arm (Right) — iyJh4F9lnKEynZzt

Строка `_id`: 36. `disabled=false`, `transfer=true`; statuses: `["bleed"]`.

| key | mode | value | priority |
| --- | --- | --- | --- |
| `system.combatEffects.turnStartEffects.bleed` | 2 | "{\"damage\": {\"amount\": 2, \"ignoreArmor\": \"true\"}, \"img\": \"icons/svg/blood.svg\", \"name\": \"WITCHER.statusEffects.bleed\"}" | null |
...
Следующий этап: [Dismembered Arm (Right - Stabilized)](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right___Stabilized__hKgvgj4lJ74wPt8N.json) — `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.hKgvgj4lJ74wPt8N`.
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right__ZxvWPJPDD9fm34Pc.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Right__ZxvWPJPDD9fm34Pc.json) |
...
Экспорт Dismembered Arm (Right): состояние none, локация rightArm. Содержит 1 ActiveEffect и 1 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Arm (Right) | Корневой Item; name:3, _id:87 | ID `ZxvWPJPDD9fm34Pc`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.ZxvWPJPDD9fm34Pc` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!ZxvWPJPDD9fm34Pc"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>The blow rends your arm from your body or damages it beyond repair. </p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.hKgvgj4lJ74wPt8N"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Dismembered Arm (Left)

ID `iyJh4F9lnKEynZzt`; _key=`"!items.effects!ZxvWPJPDD9fm34Pc.iyJh4F9lnKEynZzt"`. Origin=`"Item.KQZRzczsSx1XY63m"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
None содержит только эффект bleed; stabilized/treated не содержат effects. Неиспользование руки и протезирование описаны HTML, изменений экипировки или действий руки в этих JSON нет. У правого none bleed.name и мировой origin скопированы с левой руки; перенос всё равно относится к владельцу правого Item. Этот эффект не формирует числовой модификатор навыка.
...
Item `ZxvWPJPDD9fm34Pc` («Dismembered Arm (Right)»): `deadly/none/rightArm`; 1 ActiveEffect, 1 changes. Предшественник: нет; `followUp` ведёт к `hKgvgj4lJ74wPt8N` («Dismembered Arm (Right - Stabilized)», `stabilized/rightArm`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. Расчёт срока оставляет экспортный `healingTime=0`; `heal` не завершает Deadly автоматически. Сверены адреса: `system.combatEffects.turnStartEffects.bleed`.
