# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Right___Treated__KFbDbrS3OCs0C1h4.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Right___Treated__KFbDbrS3OCs0C1h4.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Right___Treated__KFbDbrS3OCs0C1h4.json) |
...
| Изменения относительно коммита | Нет; 37 строк; SHA-256 11c3caf80509c538cf80ccb15045a440f51da96ccc3bfc7b0e7409163d5a5113 |
| Задача и порция | [TASK-0003.061](../../../../../../tasks/task-0003.061.md) |
| Запись перекрёстной сверки | [Протокол .061](../../../../review-log.md#task-0003061) |

## Назначение файла

Экспорт Dismembered Leg (Right - Treated): состояние treated, локация rightLeg. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Leg (Right - Treated) | Корневой Item; name:3, _id:30 | ID `KFbDbrS3OCs0C1h4`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.KFbDbrS3OCs0C1h4` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!KFbDbrS3OCs0C1h4"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>The leg can be replaced with a prosthetic</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
Item `KFbDbrS3OCs0C1h4` («Dismembered Leg (Right - Treated)»): `deadly/treated/rightLeg`; 0 ActiveEffect, 0 changes. Предшественник: `vYza9bpK13G36YRV`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
