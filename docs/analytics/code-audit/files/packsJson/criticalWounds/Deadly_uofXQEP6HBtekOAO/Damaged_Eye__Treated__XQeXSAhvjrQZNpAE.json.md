# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Treated__XQeXSAhvjrQZNpAE.json

## Текущий численный контракт — 14.3.1.00069

TASK-0010.010: численные строки находятся в `system.changes`, с `type`/`phase` и настройками канала. Классификация: П: 1, Р?: 1. П — параметр, Р — бросок, Р? — выбор условия; native — периодика. Все fullEffect/shiftsCap/affectsAdvancement выключены. Исключения: нет. UUID/стадия/переходы/заживление, прочие поля Item/AE сохранены.
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Treated__XQeXSAhvjrQZNpAE.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Treated__XQeXSAhvjrQZNpAE.json) |
...
| Имя / ID | Damaged Eye (Treated) / `XQeXSAhvjrQZNpAE` |
| Строк / SHA-256 | 98 / `4e6bedecadd99ceb41778cd7373062af7d11cb704e7034107654c3c902c5021d` |
...
| `description` | "<p>You suffer a permanent -1 penalty to sight-based Awareness and DEX.</p>" |
...
| `system.skills.int.awareness.activeEffectModifiers` | 2 | "-1" | null |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Treated__XQeXSAhvjrQZNpAE.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Damaged_Eye__Treated__XQeXSAhvjrQZNpAE.json) |
...
| Изменения относительно коммита | Нет; 98 строк; SHA-256 8e4fe0a59aaff8a5dd197e611309830d45c6cf141c8ed28961c28e7e17887f0b |
| Задача и порция | [TASK-0003.061](../../../../../../tasks/task-0003.061.md) |
| Запись перекрёстной сверки | [Протокол .061](../../../../review-log.md#task-0003061) |

## Назначение файла

Экспорт Damaged Eye (Treated): состояние treated, локация head. Содержит 1 ActiveEffect и 2 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Damaged Eye (Treated) | Корневой Item; name:3, _id:91 | ID `XQeXSAhvjrQZNpAE`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.XQeXSAhvjrQZNpAE` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!XQeXSAhvjrQZNpAE"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>-1 to sight-based Awareness</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
ID `a9KpYZkIQ4eyIqTP`; _key=`"!items.effects!XQeXSAhvjrQZNpAE.a9KpYZkIQ4eyIqTP"`. Origin=`"Item.zHK1XmZ77V8c26Xv"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.stats.dex.totalModifiers` | 2 / `"-1"` / `null` | add; `-1`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -1 |
| 2 / 51 | `system.skills.int.awareness.activeEffectModifiers` | 2 / `"-1"` / `null` | add; `-1`; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js); -1 |
...
Папка: [Deadly](_Folder.json.md). Предшественник: [Damaged Eye (Stabilized)](Damaged_Eye__Stabilized__LNy3P3HUw2Ja9DNu.json.md). followUp=null; реальный treat инициировал только удаление текущего Item. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.

Контрольный Actor: восемь unmodifiedMax=5, HP.value=25, dodge.value=athletics.value=8 до эффектов, броня/вес=0. Результат: INT=5, WILL=5, REF=5, DEX=4, BODY=5, SPD=5; BODY.max=5, SPD.max=5; RUN=15, LEAP.value=3, LEAP.max=3, ENC=50, STUN=5, REC=5, HP.max=25, HP.value=25, RESOLVE.max=25, FOCUS.max=15, STA.max=25, STA.value=0; healingTime=0. Навыки: dodge.value=8, dodge.activeEffectModifiers=0; athletics.value=8, athletics.activeEffectModifiers=0; awareness.value=0, awareness.activeEffectModifiers=-1. Статусы=`[]`; turnStartEffects={}. После initial, до расчётов: `{"staMax":0,"spdMax":5,"bodyMax":5}`. Значения прочитаны непосредственно из подготовленных полей; DataModel.toObject возвращает источник и не заменяет такую проверку.

HTML ограничивает штраф Awareness проверками зрения; change адресует общий awareness.activeEffectModifiers. Настоящий addActiveEffects('awareness') вернул `" +-1[Damaged Eye]"`. В rollSkill/rollSkillCheck нет аргумента «зрение»; описание не создаёт условие автоматически. Применение или компенсация в конкретном броске остаётся за пользователем/другим механизмом. Это граница автоматизации без оценки рулбука.
...
[issue-00121](../../../../../../issues/closed/issue-00121.md) — ожидание замены, [issue-00127](../../../../../../issues/closed/issue-00127.md) — граница завершения лечения, [issue-00288](../../../../../../issues/closed/issue-00288.md) — повтор name/type. Статусы potential сохранены, подтверждения/исправления не выполнялись. Наличие этих общих проблем не объявляет дефектом каждое поле или папку.
...
Item `XQeXSAhvjrQZNpAE` («Damaged Eye (Treated)»): `deadly/treated/head`; 1 ActiveEffect, 2 changes. Предшественник: `LNy3P3HUw2Ja9DNu`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
