# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left__KQZRzczsSx1XY63m.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.vmRDG8kxeCu3sYQC"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.8Z1iHJLXrFm2i3Fb"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left__KQZRzczsSx1XY63m.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left__KQZRzczsSx1XY63m.json) |
...
| Имя / ID | Dismembered Arm (Left) / `KQZRzczsSx1XY63m` |
| Строк / SHA-256 | 94 / `3714bca3d875e43b96806cada6336afcf1e71e90dd97958aec4be5f0073a0602` |
...
| `description` | "<p>The blow rends your arm from your body or damages it beyond repair. </p><p>Your left arm cannot be used. This wound induces bleeding.</p>" |
...
| `healingTime` | 0 |
| `sterilized` | false |
| `followUp` | "Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.vmRDG8kxeCu3sYQC" |

1 ActiveEffect; 1 changes. ID и `_key` сохранённых эффектов, origin, transfer, duration и несвязанные значения сохранены.

#### Dismembered Arm (Left) — iyJh4F9lnKEynZzt

Строка `_id`: 37. `disabled=false`, `transfer=true`; statuses: `["bleed"]`.
...
Следующий этап: [Dismembered Arm (Left - Stabilized)](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left___Stabilized__vmRDG8kxeCu3sYQC.json) — `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.vmRDG8kxeCu3sYQC`.
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left__KQZRzczsSx1XY63m.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Arm__Left__KQZRzczsSx1XY63m.json) |
...
Экспорт Dismembered Arm (Left): состояние none, локация leftArm. Содержит 1 ActiveEffect и 1 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Arm (Left) | Корневой Item; name:3, _id:5 | ID `KQZRzczsSx1XY63m`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.KQZRzczsSx1XY63m` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!KQZRzczsSx1XY63m"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.vmRDG8kxeCu3sYQC"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Dismembered Arm (Left)

ID `iyJh4F9lnKEynZzt`; _key=`"!items.effects!KQZRzczsSx1XY63m.iyJh4F9lnKEynZzt"`. Origin=`"Item.KQZRzczsSx1XY63m"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Item `KQZRzczsSx1XY63m` («Dismembered Arm (Left)»): `deadly/none/leftArm`; 1 ActiveEffect, 1 changes. Предшественник: нет; `followUp` ведёт к `vmRDG8kxeCu3sYQC` («Dismembered Arm (Left - Stabilized)», `stabilized/leftArm`).
