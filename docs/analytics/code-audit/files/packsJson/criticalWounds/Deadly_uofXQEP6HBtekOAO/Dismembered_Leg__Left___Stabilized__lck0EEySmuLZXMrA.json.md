# packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Left___Stabilized__lck0EEySmuLZXMrA.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Left___Stabilized__lck0EEySmuLZXMrA.json](../../../../../../../packsJson/criticalWounds/Deadly_uofXQEP6HBtekOAO/Dismembered_Leg__Left___Stabilized__lck0EEySmuLZXMrA.json) |
...
Экспорт Dismembered Leg (Left - Stabilized): состояние stabilized, локация leftLeg. Содержит 1 ActiveEffect и 3 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Dismembered Leg (Left - Stabilized) | Корневой Item; name:3, _id:97 | ID `lck0EEySmuLZXMrA`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.lck0EEySmuLZXMrA` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!lck0EEySmuLZXMrA"` / `"uofXQEP6HBtekOAO"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.eYEp1CPif98mDm2U"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Dismembered Leg (Left - Stabilized)

ID `3atgpxSf0fttNYLr`; _key=`"!items.effects!lck0EEySmuLZXMrA.3atgpxSf0fttNYLr"`. Origin=`"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.lck0EEySmuLZXMrA"` — UUID текущего Item в компедиуме, сопоставлен с экспортом. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Папка: [Deadly](_Folder.json.md). Предшественник: [Dismembered Leg (Left)](Dismembered_Leg__Left__Us9OmoKhRydSqA8z.json.md). Следующий Item: [Dismembered Leg (Left - Treated)](Dismembered_Leg__Left___Treated__eYEp1CPif98mDm2U.json.md), `eYEp1CPif98mDm2U`, treatment=treated. Реальный treat инициировал создание этого документа, затем удаление текущего. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.
...
Item `lck0EEySmuLZXMrA` («Dismembered Leg (Left - Stabilized)»): `deadly/stabilized/leftLeg`; 1 ActiveEffect, 3 changes. Предшественник: `Us9OmoKhRydSqA8z`; `followUp` ведёт к `eYEp1CPif98mDm2U` («Dismembered Leg (Left - Treated)», `treated/leftLeg`).
