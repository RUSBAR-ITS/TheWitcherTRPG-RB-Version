# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Jaw__Stabilized__h15wRehQQoIkxcf0.json
...
Экспорт Cracked Jaw (Stabilized): документ травмы с состоянием `stabilized` и локацией `head`. Содержит 1 ActiveEffect и 10 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| effects | EmbeddedCollection, строка 19 | Собственные воздействия Item | 1 документов | При переносе Item остаются вложенными; активные transfer-эффекты собираются для Actor |
...
Item `h15wRehQQoIkxcf0` («Cracked Jaw (Stabilized)»): `simple/stabilized/head`; 1 ActiveEffect, 10 changes. Предшественник: `UnWBI9Sgu4AJv1z1`; `followUp` ведёт к `AODuTRNu2RtJJhLD` («Cracked Jaw (Treated)», `treated/head`).
