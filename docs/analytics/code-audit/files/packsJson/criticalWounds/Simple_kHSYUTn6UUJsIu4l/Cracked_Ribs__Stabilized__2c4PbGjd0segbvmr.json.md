# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Ribs__Stabilized__2c4PbGjd0segbvmr.json

## Текущий численный контракт — 14.3.1.00069

TASK-0010.010: численные строки находятся в `system.changes`, с `type`/`phase` и настройками канала. Классификация: П: 1. П — параметр, Р — бросок, Р? — выбор условия; native — периодика. Все fullEffect/shiftsCap/affectsAdvancement выключены. Исключения: stats.body → hp. UUID/стадия/переходы/заживление, прочие поля Item/AE сохранены.
...
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.oe4y6zxH2WUR9gSj"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Ribs__Stabilized__2c4PbGjd0segbvmr.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Ribs__Stabilized__2c4PbGjd0segbvmr.json) |
...
| Имя / ID | Cracked Ribs (Stabilized) / `2c4PbGjd0segbvmr` |
| Строк / SHA-256 | 92 / `10d6f51a368f987875bd8f5f9c861fe6b6169fe9dd84d08f5211f95ea7faf552` |
...
| `description` | "<p>You suffer a -1 penalty to BODY. This penalty does not affect your Health Points.</p>" |
...
| `healingTime` | 0 |
| `sterilized` | false |
| `followUp` | "Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.oe4y6zxH2WUR9gSj" |
...
| --- | --- | --- | --- |
| `system.stats.body.totalModifiers` | 2 | "-1" | null |
...
Следующий этап: [Cracked Ribs (Treated)](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Ribs__Treated__oe4y6zxH2WUR9gSj.json) — `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.oe4y6zxH2WUR9gSj`.
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Ribs__Stabilized__2c4PbGjd0segbvmr.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Ribs__Stabilized__2c4PbGjd0segbvmr.json) |
...
Экспорт Cracked Ribs (Stabilized): документ травмы с состоянием `stabilized` и локацией `torso`. Содержит 1 ActiveEffect и 1 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Cracked Ribs (Stabilized) | Корневой Item, name: 3, _id: 89 | Шаблон травмы | `2c4PbGjd0segbvmr`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.2c4PbGjd0segbvmr` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!2c4PbGjd0segbvmr"` / `"kHSYUTn6UUJsIu4l"` / `1200000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.oe4y6zxH2WUR9gSj"` | DocumentUUIDField типа Item; treat загружает адресата и инициирует замену. |
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.stats.body.totalModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js) |
...
Item `2c4PbGjd0segbvmr` («Cracked Ribs (Stabilized)»): `simple/stabilized/torso`; 1 ActiveEffect, 1 changes. Предшественник: `7TzGQ2y4yZnG01im`; `followUp` ведёт к `oe4y6zxH2WUR9gSj` («Cracked Ribs (Treated)», `treated/torso`).

Этот Item отсекается начальным фильтром `treatment=none`, но доступен через ссылку предыдущего состояния. При контрольном `BODY.max=5` срок 3 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.stats.body.totalModifiers`.
