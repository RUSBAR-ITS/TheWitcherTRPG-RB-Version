# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Disfiguring_Scar__Treated__kbeASc2PnnkYc5SR.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Disfiguring_Scar__Treated__kbeASc2PnnkYc5SR.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Disfiguring_Scar__Treated__kbeASc2PnnkYc5SR.json) |
...
Экспорт Disfiguring Scar (Treated): документ травмы с состоянием `treated` и локацией `head`. Содержит 1 ActiveEffect и 1 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Disfiguring Scar (Treated) | Корневой Item, name: 3, _id: 85 | Шаблон травмы | `kbeASc2PnnkYc5SR`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.kbeASc2PnnkYc5SR` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!kbeASc2PnnkYc5SR"` / `"kHSYUTn6UUJsIu4l"` / `900000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.skills.emp.seduction.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
...
Item `kbeASc2PnnkYc5SR` («Disfiguring Scar (Treated)»): `simple/treated/head`; 1 ActiveEffect, 1 changes. Предшественник: `AJeuZeFF29nEI5fc`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
