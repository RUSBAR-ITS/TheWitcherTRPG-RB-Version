# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Leg__Right___Stabilized__fNiSVOJzpaxVvZTE.json
...
Экспорт Sprained Leg (Right - Stabilized): документ травмы с состоянием `stabilized` и локацией `rightLeg`. Содержит 1 ActiveEffect и 3 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
Item `fNiSVOJzpaxVvZTE` («Sprained Leg (Right - Stabilized)»): `simple/stabilized/rightLeg`; 1 ActiveEffect, 3 changes. Предшественник: `VhwzsUlv5csYSJTM`; `followUp` ведёт к `8qatuNeEROueRDcZ` («Sprained Leg (Right - Treated)», `treated/rightLeg`).
