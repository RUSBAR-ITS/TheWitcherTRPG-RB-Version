# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Stabilized__yGy3oWvmpX6WMm56.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Stabilized__yGy3oWvmpX6WMm56.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Stabilized__yGy3oWvmpX6WMm56.json) |
| Тип файла | JSON: Item типа criticalWound |
| Статус анализа | Проверено |
| Дата проверки | 2026-09-12 |
| Ветка и коммит | rusbar-main, 5283da15a49422fc339c44add4e7d7d02c174ce4 |
| Изменения относительно коммита | Нет; 37 строк; SHA-256 6d2a69836f958ef24b6336cb0eba707891ca819bb663db777addb252ab4b405d |
...
Экспорт Sprained Arm (Right - Stabilized): документ травмы с состоянием `stabilized` и локацией `rightArm`. Содержит 0 ActiveEffect и 0 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Sprained Arm (Right - Stabilized) | Корневой Item, name: 3, _id: 34 | Шаблон травмы | `yGy3oWvmpX6WMm56`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.yGy3oWvmpX6WMm56` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!yGy3oWvmpX6WMm56"` / `"kHSYUTn6UUJsIu4l"` / `2000000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| followUp | `"Compendium.TheWitcherTRPG.criticalWounds.Item.YiusbDXfFDhqwtQT"` | DocumentUUIDField типа Item; treat загружает адресата и инициирует замену. |
...
Item `yGy3oWvmpX6WMm56` («Sprained Arm (Right - Stabilized)»): `simple/stabilized/rightArm`; 0 ActiveEffect, 0 changes. Предшественник: `umPVfrJeNU65S48O`; `followUp` ведёт к `YiusbDXfFDhqwtQT` («Sprained Arm (Right - Treated)», `treated/rightArm`).
