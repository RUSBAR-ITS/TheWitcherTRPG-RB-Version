# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Foreign_Object_mylVzp10NMor44XR.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.fnYssldrMLVqbF22"` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Foreign_Object_mylVzp10NMor44XR.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Foreign_Object_mylVzp10NMor44XR.json) |
...
Экспорт Foreign Object: документ травмы с состоянием `none` и локацией `torso`. Содержит 0 ActiveEffect и 0 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Foreign Object | Корневой Item, name: 2, _id: 34 | Шаблон травмы | `mylVzp10NMor44XR`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.mylVzp10NMor44XR` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!mylVzp10NMor44XR"` / `"kHSYUTn6UUJsIu4l"` / `1300000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.fnYssldrMLVqbF22"` | DocumentUUIDField типа Item; treat загружает адресата и инициирует замену. |
...
Item `mylVzp10NMor44XR` («Foreign Object»): `simple/none/torso`; 0 ActiveEffect, 0 changes. Предшественник: нет; `followUp` ведёт к `fnYssldrMLVqbF22` («Foreign Object (Stabilized)», `stabilized/torso`).
