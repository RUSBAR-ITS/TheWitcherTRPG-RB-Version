# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Leg__Right__VhwzsUlv5csYSJTM.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.fNiSVOJzpaxVvZTE"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.8qatuNeEROueRDcZ"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Leg__Right__VhwzsUlv5csYSJTM.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Leg__Right__VhwzsUlv5csYSJTM.json) |
...
Экспорт Sprained Leg (Right): документ травмы с состоянием `none` и локацией `rightLeg`. Содержит 1 ActiveEffect и 3 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Sprained Leg (Right) | Корневой Item, name: 3, _id: 101 | Шаблон травмы | `VhwzsUlv5csYSJTM`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.VhwzsUlv5csYSJTM` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!VhwzsUlv5csYSJTM"` / `"kHSYUTn6UUJsIu4l"` / `2500000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.fNiSVOJzpaxVvZTE"` | DocumentUUIDField типа Item; treat загружает адресата и инициирует замену. |
...
Item `VhwzsUlv5csYSJTM` («Sprained Leg (Right)»): `simple/none/rightLeg`; 1 ActiveEffect, 3 changes. Предшественник: нет; `followUp` ведёт к `fNiSVOJzpaxVvZTE` («Sprained Leg (Right - Stabilized)», `stabilized/rightLeg`).
