# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Jaw_UnWBI9Sgu4AJv1z1.json

## Текущий численный контракт — 14.3.1.00069

TASK-0010.010: численные строки находятся в `system.changes`, с `type`/`phase` и настройками канала. Классификация: Р: 3, Р?: 7. П — параметр, Р — бросок, Р? — выбор условия; native — периодика. Все fullEffect/shiftsCap/affectsAdvancement выключены. Исключения: нет. UUID/стадия/переходы/заживление, прочие поля Item/AE сохранены.
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.h15wRehQQoIkxcf0"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.AODuTRNu2RtJJhLD"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Jaw_UnWBI9Sgu4AJv1z1.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Cracked_Jaw_UnWBI9Sgu4AJv1z1.json) |
...
Экспорт Cracked Jaw: документ травмы с состоянием `none` и локацией `head`. Содержит 1 ActiveEffect и 10 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Cracked Jaw | Корневой Item, name: 3, _id: 5 | Шаблон травмы | `UnWBI9Sgu4AJv1z1`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.UnWBI9Sgu4AJv1z1` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!UnWBI9Sgu4AJv1z1"` / `"kHSYUTn6UUJsIu4l"` / `400000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| location | `"head"` | StringField; отбор по локации и lookup подписи на листе. |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.h15wRehQQoIkxcf0"` | DocumentUUIDField типа Item; treat загружает адресата и инициирует замену. |
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.skills.will.spellcast.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 2 / 51 | `system.skills.will.ritcraft.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 3 / 57 | `system.skills.will.hexweave.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 4 / 63 | `system.skills.emp.charisma.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 5 / 69 | `system.skills.emp.persuasion.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 6 / 75 | `system.skills.emp.seduction.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 7 / 81 | `system.skills.emp.leadership.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 8 / 87 | `system.skills.emp.deceit.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 9 / 93 | `system.skills.int.socialetq.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 10 / 99 | `system.skills.will.intimidation.activeEffectModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
...
Item `UnWBI9Sgu4AJv1z1` («Cracked Jaw»): `simple/none/head`; 1 ActiveEffect, 10 changes. Предшественник: нет; `followUp` ведёт к `h15wRehQQoIkxcf0` («Cracked Jaw (Stabilized)», `stabilized/head`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. При контрольном `BODY.max=5` срок 3 дней независимо от штрафов к `BODY.value`. Сверены адреса: `skill.activeEffectModifiers`.
