# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Disfiguring_Scar__Stabilized__AJeuZeFF29nEI5fc.json

## Текущий численный контракт — 14.3.1.00069

TASK-0010.010: численные строки находятся в `system.changes`, с `type`/`phase` и настройками канала. Классификация: Р?: 6. П — параметр, Р — бросок, Р? — выбор условия; native — периодика. Все fullEffect/shiftsCap/affectsAdvancement выключены. Исключения: нет. UUID/стадия/переходы/заживление, прочие поля Item/AE сохранены.
...
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.kbeASc2PnnkYc5SR"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Disfiguring_Scar__Stabilized__AJeuZeFF29nEI5fc.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Disfiguring_Scar__Stabilized__AJeuZeFF29nEI5fc.json) |
...
Экспорт Disfiguring Scar (Stabilized): документ травмы с состоянием `stabilized` и локацией `head`. Содержит 1 ActiveEffect и 6 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Disfiguring Scar (Stabilized) | Корневой Item, name: 3, _id: 115 | Шаблон травмы | `AJeuZeFF29nEI5fc`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.AJeuZeFF29nEI5fc` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!AJeuZeFF29nEI5fc"` / `"kHSYUTn6UUJsIu4l"` / `800000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.kbeASc2PnnkYc5SR"` | DocumentUUIDField типа Item; treat загружает адресата и инициирует замену. |
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.skills.emp.charisma.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 2 / 51 | `system.skills.emp.persuasion.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 3 / 57 | `system.skills.emp.seduction.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 4 / 63 | `system.skills.emp.leadership.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 5 / 69 | `system.skills.emp.deceit.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 6 / 75 | `system.skills.int.socialetq.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
...
Item `AJeuZeFF29nEI5fc` («Disfiguring Scar (Stabilized)»): `simple/stabilized/head`; 1 ActiveEffect, 6 changes. Предшественник: `8tqapNHVCmSwijJw`; `followUp` ведёт к `kbeASc2PnnkYc5SR` («Disfiguring Scar (Treated)», `treated/head`).
