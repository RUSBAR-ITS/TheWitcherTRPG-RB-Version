# packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Treated__YiusbDXfFDhqwtQT.json

## Текущий численный контракт — 14.3.1.00069

TASK-0010.010: численные строки находятся в `system.changes`, с `type`/`phase` и настройками канала. Классификация: Р: 1. П — параметр, Р — бросок, Р? — выбор условия; native — периодика. Все fullEffect/shiftsCap/affectsAdvancement выключены. Исключения: нет. UUID/стадия/переходы/заживление, прочие поля Item/AE сохранены.
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Treated__YiusbDXfFDhqwtQT.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Treated__YiusbDXfFDhqwtQT.json) |
...
| Имя / ID | Sprained Arm (Right - Treated) / `YiusbDXfFDhqwtQT` |
| Строк / SHA-256 | 92 / `daa4917ecd542acb566228e10b2288d395988f3778c3cf7a6576a480c56d0aa3` |
...
#### Sprained Arm (Right - Treated) — 0xOANRTUxzvDtCV0

Строка `_id`: 36. `disabled=false`, `transfer=true`; statuses: `[]`.

| key | mode | value | priority |
| --- | --- | --- | --- |
| `system.skills.body.physique.activeEffectModifiers` | 2 | "-1" | null |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Treated__YiusbDXfFDhqwtQT.json](../../../../../../../packsJson/criticalWounds/Simple_kHSYUTn6UUJsIu4l/Sprained_Arm__Right___Treated__YiusbDXfFDhqwtQT.json) |
| Тип файла | JSON: Item типа criticalWound |
| Статус анализа | Проверено |
| Дата проверки | 2026-09-12 |
| Ветка и коммит | rusbar-main, 5283da15a49422fc339c44add4e7d7d02c174ce4 |
| Изменения относительно коммита | Нет; 92 строк; SHA-256 73ba2f3170451e83c8ec7d3e531987a5cee65e0e0837ac9f6f27af3b9a18fcf7 |
...
Экспорт Sprained Arm (Right - Treated): документ травмы с состоянием `treated` и локацией `rightArm`. Содержит 1 ActiveEffect и 1 изменений. Разбор описывает структуру и работу потребителей; соответствие игровым правилам не проверяется.
...
| --- | --- | --- | --- | --- |
| Sprained Arm (Right - Treated) | Корневой Item, name: 3, _id: 89 | Шаблон травмы | `YiusbDXfFDhqwtQT`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.YiusbDXfFDhqwtQT` | Загрузка, копирование к Actor, подготовка, лечение и удаление |
| _key / folder / sort | Корневые поля | Идентификация экспортного документа и порядок | `"!items!YiusbDXfFDhqwtQT"` / `"kHSYUTn6UUJsIu4l"` / `2100000` | Потребление ядром и инструментами экспорта; sort не задаёт приоритет изменений |
...
| location | `"rightArm"` | StringField; отбор по локации и lookup подписи на листе. |
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.skills.body.physique.activeEffectModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
...
Item `YiusbDXfFDhqwtQT` («Sprained Arm (Right - Treated)»): `simple/treated/rightArm`; 1 ActiveEffect, 1 changes. Предшественник: `yGy3oWvmpX6WMm56`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
