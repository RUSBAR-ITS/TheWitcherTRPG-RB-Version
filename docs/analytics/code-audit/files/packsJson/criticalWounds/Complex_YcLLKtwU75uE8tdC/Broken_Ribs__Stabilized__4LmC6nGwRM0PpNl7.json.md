# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Broken_Ribs__Stabilized__4LmC6nGwRM0PpNl7.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Broken_Ribs__Stabilized__4LmC6nGwRM0PpNl7.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Broken_Ribs__Stabilized__4LmC6nGwRM0PpNl7.json) |
...
Экспорт Broken Ribs (Stabilized): состояние stabilized, локация torso. Содержит 1 ActiveEffect и 2 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Broken Ribs (Stabilized) | Корневой Item; name:2, _id:91 | ID `4LmC6nGwRM0PpNl7`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.4LmC6nGwRM0PpNl7` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!4LmC6nGwRM0PpNl7"` / `"YcLLKtwU75uE8tdC"` / `1100000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.77evBMjaJOlKTaRv"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Broken Ribs

ID `iNO7kcB6uTNKMcLq`; экспортный _key=`"!items.effects!4LmC6nGwRM0PpNl7.iNO7kcB6uTNKMcLq"`. Origin=`"Item.VfgJzcV75cGqsjuF"` — ссылка на мировой Item; разрешение такого UUID в действующем мире не проверялось. Она не становится ссылкой на компедиум при совпадении последнего ID и не задаёт цель применения. Цель при transfer=true — Actor-владелец этого Item.
...
| --- | --- | --- | --- | --- |
| 1 / 44 | `system.stats.body.totalModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js) |
| 2 / 50 | `system.stats.ref.totalModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js) |
...
Папка: [Complex](_Folder.json.md). Предшественник: [Broken Ribs](Broken_Ribs_VfgJzcV75cGqsjuF.json.md). Следующий Item: [Broken Ribs (Treated)](Broken_Ribs__Treated__77evBMjaJOlKTaRv.json.md), `77evBMjaJOlKTaRv`, treatment=treated. Реальный treat инициировал создание этого документа, затем удаление текущего. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.

Контрольный Actor: восемь unmodifiedMax=5, HP.value=25, броня/вес=0. Результат: INT=5, WILL=5, REF=4, DEX=5, BODY=4, SPD=5; BODY.max=5; RUN=15, LEAP=3, ENC=40, STUN=4, REC=4, HP.max=20, RESOLVE.max=25, FOCUS.max=15; healingTime=7. Все changes дали числовые значения своих целевых полей. Производные рассчитываются из полученных характеристик; базовый max не заменяется totalModifiers.
...
Item `4LmC6nGwRM0PpNl7` («Broken Ribs (Stabilized)»): `complex/stabilized/torso`; 1 ActiveEffect, 2 changes. Предшественник: `VfgJzcV75cGqsjuF`; `followUp` ведёт к `77evBMjaJOlKTaRv` («Broken Ribs (Treated)», `treated/torso`).

Этот Item отсекается начальным фильтром `treatment=none`, но доступен через ссылку предыдущего состояния. При контрольном `BODY.max=5` срок 7 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.stats.body.totalModifiers`, `system.stats.ref.totalModifiers`.
