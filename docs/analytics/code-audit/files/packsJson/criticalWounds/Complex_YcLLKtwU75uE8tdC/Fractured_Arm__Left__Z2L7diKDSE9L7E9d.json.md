# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Left__Z2L7diKDSE9L7E9d.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.eCiJjDqfUyaXCW2z"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.kPIkW1AXuybKZPEh"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Left__Z2L7diKDSE9L7E9d.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Left__Z2L7diKDSE9L7E9d.json) |
...
Экспорт Fractured Arm (Left): состояние none, локация leftArm. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Fractured Arm (Left) | Корневой Item; name:3, _id:5 | ID `Z2L7diKDSE9L7E9d`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.Z2L7diKDSE9L7E9d` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!Z2L7diKDSE9L7E9d"` / `"YcLLKtwU75uE8tdC"` / `2500000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| system / effects | Объекты, строки 7 / 21 | criticalWound / 0 embedded ActiveEffect | Схема и воздействия перечислены далее |
...
| description | `"<p>You take a -3 to actions with that arm</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.eCiJjDqfUyaXCW2z"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |
...
Item `Z2L7diKDSE9L7E9d` («Fractured Arm (Left)»): `complex/none/leftArm`; 0 ActiveEffect, 0 changes. Предшественник: нет; `followUp` ведёт к `eCiJjDqfUyaXCW2z` («Fractured Arm (Left - Stabilized)», `stabilized/leftArm`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. При контрольном `BODY.max=5` срок 7 дней независимо от штрафов к `BODY.value`. Изменений числовых полей не задано; текст/статусы рассматриваются отдельно.
