# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Right___Stabilized__t9bcLB3zDGUX9Uuq.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Right___Stabilized__t9bcLB3zDGUX9Uuq.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Right___Stabilized__t9bcLB3zDGUX9Uuq.json) |
...
Экспорт Fractured Arm (Right - Stabilized): состояние stabilized, локация rightArm. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Fractured Arm (Right - Stabilized) | Корневой Item; name:3, _id:30 | ID `t9bcLB3zDGUX9Uuq`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.t9bcLB3zDGUX9Uuq` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!t9bcLB3zDGUX9Uuq"` / `"YcLLKtwU75uE8tdC"` / `1700000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>You take a -2 to actions with that arm</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.djnBTcYyGVsykdoy"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |
...
Item `t9bcLB3zDGUX9Uuq` («Fractured Arm (Right - Stabilized)»): `complex/stabilized/rightArm`; 0 ActiveEffect, 0 changes. Предшественник: `ZOyO6WlnMTDjzBj9`; `followUp` ведёт к `djnBTcYyGVsykdoy` («Fractured Arm (Right - Treated)», `treated/rightArm`).
