# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Left__r34NuXwHfPGZCpTu.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.LF0C1HVgY4hNZOFE"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.nM9wqZXmrkFRRGTW"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Left__r34NuXwHfPGZCpTu.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Left__r34NuXwHfPGZCpTu.json) |
...
Экспорт Fractured Leg (Left): состояние none, локация leftLeg. Содержит 1 ActiveEffect и 3 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Fractured Leg (Left) | Корневой Item; name:3, _id:5 | ID `r34NuXwHfPGZCpTu`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.r34NuXwHfPGZCpTu` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!r34NuXwHfPGZCpTu"` / `"YcLLKtwU75uE8tdC"` / `1900000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.LF0C1HVgY4hNZOFE"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Fractured Leg (Left)

ID `hF9ez6RQNIpixxd9`; экспортный _key=`"!items.effects!r34NuXwHfPGZCpTu.hF9ez6RQNIpixxd9"`. Origin=`"Item.r34NuXwHfPGZCpTu"` — ссылка на мировой Item; разрешение такого UUID в действующем мире не проверялось. Она не становится ссылкой на компедиум при совпадении последнего ID и не задаёт цель применения. Цель при transfer=true — Actor-владелец этого Item.
...
| --- | --- | --- | --- | --- |
| 1 / 46 | `system.stats.spd.totalModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js) |
| 2 / 52 | `system.skills.ref.dodge.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 3 / 58 | `system.skills.dex.athletics.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
...
Item `r34NuXwHfPGZCpTu` («Fractured Leg (Left)»): `complex/none/leftLeg`; 1 ActiveEffect, 3 changes. Предшественник: нет; `followUp` ведёт к `LF0C1HVgY4hNZOFE` («Fractured Leg (Left - Stabilized)», `stabilized/leftLeg`).
