# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Right___Treated__Aa8wCz1OGM4gflmc.json
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Right___Treated__Aa8wCz1OGM4gflmc.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Right___Treated__Aa8wCz1OGM4gflmc.json) |
...
| Имя / ID | Fractured Leg (Right - Treated) / `Aa8wCz1OGM4gflmc` |
| Строк / SHA-256 | 104 / `1ae8f7fcf5987b2c8cc3d44c69125c477d9be9d6c561fad395eeb4c1230302a3` |

### Выполненные исправления

B07: три штрафа treated правой ноги −3 → −1; B16: исправить сторону в имени эффекта. Остальные поля сохранены. Текущие значения и связи перечислены ниже; архив в конце описывает прежние срезы.
...
| --- | --- | --- | --- |
| `system.stats.spd.totalModifiers` | 2 | "-1" | null |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Right___Treated__Aa8wCz1OGM4gflmc.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Right___Treated__Aa8wCz1OGM4gflmc.json) |
...
| Изменения относительно коммита | Нет; 104 строк; SHA-256 e484623fbb7a604278542758d4b1a9d9e32dbeca74215c5945f7844f02b64ffb |
| Задача и порция | [TASK-0003.059](../../../../../../tasks/task-0003.059.md) |
| Запись перекрёстной сверки | [Протокол .059](../../../../review-log.md#task-0003059) |

## Назначение файла

Экспорт Fractured Leg (Right - Treated): состояние treated, локация rightLeg. Содержит 1 ActiveEffect и 3 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Fractured Leg (Right - Treated) | Корневой Item; name:3, _id:97 | ID `Aa8wCz1OGM4gflmc`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.Aa8wCz1OGM4gflmc` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!Aa8wCz1OGM4gflmc"` / `"YcLLKtwU75uE8tdC"` / `2400000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
ID `hF9ez6RQNIpixxd9`; экспортный _key=`"!items.effects!Aa8wCz1OGM4gflmc.hF9ez6RQNIpixxd9"`. Origin=`"Item.r34NuXwHfPGZCpTu"` — ссылка на мировой Item; разрешение такого UUID в действующем мире не проверялось. Она не становится ссылкой на компедиум при совпадении последнего ID и не задаёт цель применения. Цель при transfer=true — Actor-владелец этого Item.
...
В этой цепочке три штрафа SPD/dodge/athletics меняются −3 → −2 → −3, а слева −3 → −2 → −1. Метод treat загружает данные следующего Item без обещания монотонного уменьшения штрафа. Расхождение зафиксировано; правильность чисел требует отдельной проверки контента и здесь не установлена.
...
[issue-00121](../../../../../../issues/potential/issue-00121.md) — ожидание замены, [issue-00127](../../../../../../issues/potential/issue-00127.md) — граница завершения лечения, [issue-00288](../../../../../../issues/potential/issue-00288.md) — повтор name/type. [issue-00326](../../../../../../issues/closed/issue-00326.md) — подпись эффекта правой ноги. Статусы potential сохранены, подтверждения/исправления не выполнялись. Наличие этих общих проблем не объявляет дефектом каждое поле или папку.
...
Item `Aa8wCz1OGM4gflmc` («Fractured Leg (Right - Treated)»): `complex/treated/rightLeg`; 1 ActiveEffect, 3 changes. Предшественник: `WNjcD3F3Hs5IdAaa`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.

Этот Item отсекается начальным фильтром `treatment=none`, но доступен через ссылку предыдущего состояния. При контрольном `BODY.max=5` срок 7 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.stats.spd.totalModifiers`, `skill.activeEffectModifiers`.
