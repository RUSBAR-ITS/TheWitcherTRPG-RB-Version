# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Left___Stabilized__LF0C1HVgY4hNZOFE.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Left___Stabilized__LF0C1HVgY4hNZOFE.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Leg__Left___Stabilized__LF0C1HVgY4hNZOFE.json) |
...
Экспорт Fractured Leg (Left - Stabilized): состояние stabilized, локация leftLeg. Содержит 1 ActiveEffect и 3 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Fractured Leg (Left - Stabilized) | Корневой Item; name:3, _id:97 | ID `LF0C1HVgY4hNZOFE`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.LF0C1HVgY4hNZOFE` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!LF0C1HVgY4hNZOFE"` / `"YcLLKtwU75uE8tdC"` / `2000000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.nM9wqZXmrkFRRGTW"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Fractured Leg (Left)

ID `hF9ez6RQNIpixxd9`; экспортный _key=`"!items.effects!LF0C1HVgY4hNZOFE.hF9ez6RQNIpixxd9"`. Origin=`"Item.r34NuXwHfPGZCpTu"` — ссылка на мировой Item; разрешение такого UUID в действующем мире не проверялось. Она не становится ссылкой на компедиум при совпадении последнего ID и не задаёт цель применения. Цель при transfer=true — Actor-владелец этого Item.
...
Item `LF0C1HVgY4hNZOFE` («Fractured Leg (Left - Stabilized)»): `complex/stabilized/leftLeg`; 1 ActiveEffect, 3 changes. Предшественник: `r34NuXwHfPGZCpTu`; `followUp` ведёт к `nM9wqZXmrkFRRGTW` («Fractured Leg (Left - Treated)», `treated/leftLeg`).
