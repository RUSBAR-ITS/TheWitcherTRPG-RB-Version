...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.t9bcLB3zDGUX9Uuq"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.djnBTcYyGVsykdoy"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Right__ZOyO6WlnMTDjzBj9.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Fractured_Arm__Right__ZOyO6WlnMTDjzBj9.json) |
...
Экспорт Fractured Arm (Right): состояние none, локация rightArm. Содержит 0 ActiveEffect и 0 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Fractured Arm (Right) | Корневой Item; name:3, _id:30 | ID `ZOyO6WlnMTDjzBj9`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.ZOyO6WlnMTDjzBj9` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!ZOyO6WlnMTDjzBj9"` / `"YcLLKtwU75uE8tdC"` / `1600000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.t9bcLB3zDGUX9Uuq"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |
...
Item `ZOyO6WlnMTDjzBj9` («Fractured Arm (Right)»): `complex/none/rightArm`; 0 ActiveEffect, 0 changes. Предшественник: нет; `followUp` ведёт к `t9bcLB3zDGUX9Uuq` («Fractured Arm (Right - Stabilized)», `stabilized/rightArm`).
