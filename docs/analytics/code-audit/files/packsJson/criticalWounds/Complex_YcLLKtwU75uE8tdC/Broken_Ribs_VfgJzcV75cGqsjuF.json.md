# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Broken_Ribs_VfgJzcV75cGqsjuF.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.4LmC6nGwRM0PpNl7"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.77evBMjaJOlKTaRv"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Broken_Ribs_VfgJzcV75cGqsjuF.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Broken_Ribs_VfgJzcV75cGqsjuF.json) |
...
Экспорт Broken Ribs: состояние none, локация torso. Содержит 1 ActiveEffect и 3 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Broken Ribs | Корневой Item; name:2, _id:4 | ID `VfgJzcV75cGqsjuF`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.VfgJzcV75cGqsjuF` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!VfgJzcV75cGqsjuF"` / `"YcLLKtwU75uE8tdC"` / `1000000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.4LmC6nGwRM0PpNl7"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Broken Ribs

ID `iNO7kcB6uTNKMcLq`; экспортный _key=`"!items.effects!VfgJzcV75cGqsjuF.iNO7kcB6uTNKMcLq"`. Origin=`"Item.VfgJzcV75cGqsjuF"` — ссылка на мировой Item; разрешение такого UUID в действующем мире не проверялось. Она не становится ссылкой на компедиум при совпадении последнего ID и не задаёт цель применения. Цель при transfer=true — Actor-владелец этого Item.
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.stats.body.totalModifiers` | 2 / `"-2"` / `null` | add; -2; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js) |
| 2 / 51 | `system.stats.ref.totalModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js) |
| 3 / 57 | `system.stats.dex.totalModifiers` | 2 / `"-1"` / `null` | add; -1; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js) |
...
Папка: [Complex](_Folder.json.md). Предшественников по followUp в Complex нет. Следующий Item: [Broken Ribs (Stabilized)](Broken_Ribs__Stabilized__4LmC6nGwRM0PpNl7.json.md), `4LmC6nGwRM0PpNl7`, treatment=stabilized. Реальный treat инициировал создание этого документа, затем удаление текущего. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.

Контрольный Actor: восемь unmodifiedMax=5, HP.value=25, броня/вес=0. Результат: INT=5, WILL=5, REF=4, DEX=4, BODY=3, SPD=5; BODY.max=5; RUN=15, LEAP=3, ENC=30, STUN=4, REC=4, HP.max=20, RESOLVE.max=25, FOCUS.max=15; healingTime=7. Все changes дали числовые значения своих целевых полей. Производные рассчитываются из полученных характеристик; базовый max не заменяется totalModifiers.
...
Item `VfgJzcV75cGqsjuF` («Broken Ribs»): `complex/none/torso`; 1 ActiveEffect, 3 changes. Предшественник: нет; `followUp` ведёт к `4LmC6nGwRM0PpNl7` («Broken Ribs (Stabilized)», `stabilized/torso`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. При контрольном `BODY.max=5` срок 7 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.stats.body.totalModifiers`, `system.stats.ref.totalModifiers`, `system.stats.dex.totalModifiers`.
