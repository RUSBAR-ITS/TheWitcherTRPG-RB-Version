# packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Lost_Teeth_IMLpjhiZ0yg6hjKI.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.wccN560fe7WQgT0N"` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Lost_Teeth_IMLpjhiZ0yg6hjKI.json](../../../../../../../packsJson/criticalWounds/Complex_YcLLKtwU75uE8tdC/Lost_Teeth_IMLpjhiZ0yg6hjKI.json) |
...
Экспорт Lost Teeth: состояние none, локация head. Содержит 1 ActiveEffect и 10 изменений. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Lost Teeth | Корневой Item; name:2, _id:4 | ID `IMLpjhiZ0yg6hjKI`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.IMLpjhiZ0yg6hjKI` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!IMLpjhiZ0yg6hjKI"` / `"YcLLKtwU75uE8tdC"` / `100000` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.wccN560fe7WQgT0N"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Lost Teeth

ID `W3nKiUcheFOw6vZv`; экспортный _key=`"!items.effects!IMLpjhiZ0yg6hjKI.W3nKiUcheFOw6vZv"`. Origin=`"Item.IMLpjhiZ0yg6hjKI"` — ссылка на мировой Item; разрешение такого UUID в действующем мире не проверялось. Она не становится ссылкой на компедиум при совпадении последнего ID и не задаёт цель применения. Цель при transfer=true — Actor-владелец этого Item.
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.skills.will.spellcast.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 2 / 51 | `system.skills.will.ritcraft.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 3 / 57 | `system.skills.will.hexweave.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 4 / 63 | `system.skills.emp.charisma.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 5 / 69 | `system.skills.emp.persuasion.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 6 / 75 | `system.skills.emp.seduction.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 7 / 81 | `system.skills.emp.leadership.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 8 / 87 | `system.skills.emp.deceit.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 9 / 93 | `system.skills.int.socialetq.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
| 10 / 99 | `system.skills.will.intimidation.activeEffectModifiers` | 2 / `"-3"` / `null` | add; -3; priority=20; phase=initial | NumberField; [skillData.js](../../../../../../../module/data/actor/templates/common/skills/skillData.js) |
...
Item `IMLpjhiZ0yg6hjKI` («Lost Teeth»): `complex/none/head`; 1 ActiveEffect, 10 changes. Предшественник: нет; `followUp` ведёт к `wccN560fe7WQgT0N` («Lost Teeth (Stabilized)», `stabilized/head`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. При контрольном `BODY.max=5` срок 7 дней независимо от штрафов к `BODY.value`. Сверены адреса: `skill.activeEffectModifiers`.
