# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Left__saPd4IMUCv5qZE60.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.tdcGSOZGCUhNArDc"` |
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.zaKPfDQFGTR8FKju"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Left__saPd4IMUCv5qZE60.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Left__saPd4IMUCv5qZE60.json) |
...
Экспорт Compound Arm Fracture (Left): состояние none, локация leftArm. Вложенных ActiveEffect: 1; изменений: 1. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Compound Arm Fracture (Left) | Корневой Item; name:3, _id:5 | ID `saPd4IMUCv5qZE60`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.saPd4IMUCv5qZE60` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!saPd4IMUCv5qZE60"` / `"ox3lLmV3zp0K67Ht"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>The arm is rendered useless</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.tdcGSOZGCUhNArDc"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Compound Arm Fracture (Bleed)

ID `32L2d5MfPiaENuqI`; _key=`"!items.effects!saPd4IMUCv5qZE60.32L2d5MfPiaENuqI"`. Origin=`"Item.saPd4IMUCv5qZE60"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Item `saPd4IMUCv5qZE60` («Compound Arm Fracture (Left)»): `difficult/none/leftArm`; 1 ActiveEffect, 1 changes. Предшественник: нет; `followUp` ведёт к `tdcGSOZGCUhNArDc` («Compound Arm Fracture (Left - Stabilized)», `stabilized/leftArm`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. При контрольном `BODY.max=5` срок 10 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.combatEffects.turnStartEffects.bleed`.
