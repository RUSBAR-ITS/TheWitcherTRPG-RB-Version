# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Right___Treated__ujz1IMKCXoJF9w91.json
...
| Поле | Значение |
| --- | --- |
| `woundTypeId` | `"compound-arm-fracture"` |
| `location` | `"rightArm"` |
| `treatment` | `"treated"` |
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Right___Treated__ujz1IMKCXoJF9w91.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Right___Treated__ujz1IMKCXoJF9w91.json) |
...
| Имя / ID | Compound Arm Fracture (Right - Treated) / `ujz1IMKCXoJF9w91` |
| Строк / SHA-256 | 37 / `020b2c41318af465464a82c5cce6d09f3d16ec961fa0b40f88bc54060417be84` |

### Выполненные исправления

B08: убрать лишний эффект кровотечения из treated рук. Остальные поля сохранены. Текущие значения и связи перечислены ниже; архив в конце описывает прежние срезы.

| Поле system | Текущее значение |
| --- | --- |
| `htmlFields` | ["description"] |
| `description` | "<p>That arm must remain in a sling, but it can hold things.</p>" |
...
0 ActiveEffect; 0 changes. ID и `_key` сохранённых эффектов, origin, transfer, duration и несвязанные значения сохранены.
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Right___Treated__ujz1IMKCXoJF9w91.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Right___Treated__ujz1IMKCXoJF9w91.json) |
...
| Изменения относительно коммита | Нет; 94 строк; SHA-256 6bd97fce50004b9b6db6fc99e6ceb8531073becd50d2e2d3aa1013cf74db69b8 |
| Задача и порция | [TASK-0003.060](../../../../../../tasks/task-0003.060.md) |
| Запись перекрёстной сверки | [Протокол .060](../../../../review-log.md#task-0003060) |

## Назначение файла

Экспорт Compound Arm Fracture (Right - Treated): состояние treated, локация rightArm. Вложенных ActiveEffect: 1; изменений: 1. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Compound Arm Fracture (Right - Treated) | Корневой Item; name:3, _id:87 | ID `ujz1IMKCXoJF9w91`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.ujz1IMKCXoJF9w91` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!ujz1IMKCXoJF9w91"` / `"ox3lLmV3zp0K67Ht"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| description | `"<p>That arm must remain in a sling, but it can hold things.</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
ID `32L2d5MfPiaENuqI`; _key=`"!items.effects!ujz1IMKCXoJF9w91.32L2d5MfPiaENuqI"`. Origin=`"Item.saPd4IMUCv5qZE60"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Папка: [Difficult](_Folder.json.md). Предшественник: [Compound Arm Fracture (Right - Stabilized)](Compound_Arm_Fracture__Right___Stabilized__NHNctAuhsapGZXiP.json.md). followUp=null; реальный treat инициировал только удаление текущего Item. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.

Контрольный Actor: восемь unmodifiedMax=5, HP.value=25, dodge.value=athletics.value=8 до эффектов, броня/вес=0. Результат: INT=5, WILL=5, REF=5, DEX=5, BODY=5, SPD=5; BODY.max=5, SPD.max=5; RUN=15, LEAP.value=3, LEAP.max=3, ENC=50, STUN=5, REC=5, HP.max=25, RESOLVE.max=25, FOCUS.max=15; healingTime=10. Навыки: dodge.value=8, dodge.activeEffectModifiers=0; athletics.value=8, athletics.activeEffectModifiers=0. Статусы=`["bleed"]`; turnStartEffects={}. Значения прочитаны непосредственно из подготовленных полей; сериализация DataModel/toObject возвращает источник и не заменяет такую проверку.
...
Item `ujz1IMKCXoJF9w91` («Compound Arm Fracture (Right - Treated)»): `difficult/treated/rightArm`; 1 ActiveEffect, 1 changes. Предшественник: `NHNctAuhsapGZXiP`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.

Этот Item отсекается начальным фильтром `treatment=none`, но доступен через ссылку предыдущего состояния. При контрольном `BODY.max=5` срок 10 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.combatEffects.turnStartEffects.bleed`.

В обеих none/treated bleed-object amount2, stabilized effects пуст. У правого stabilized location=rightLeg, остальные rightArm: переходы сохраняют поля нового шаблона. Это ошибка данных 00327, отдельная от ADD00328 и ожидания 00121. HTML использования руки не становится автоматическим запретом. Общая связь periodic-записи с обработчиком разобрана в R012-24; status сам по себе не вызывает урон.

Сопоставленные определения и потребители: [module/data/item/criticalWoundData.js](../../../module/data/item/criticalWoundData.js.md), [module/actor/mixins/damageMixin.js](../../../module/actor/mixins/damageMixin.js.md), [module/actor/witcherActor.js](../../../module/actor/witcherActor.js.md), [module/activeEffect/witcherActiveEffect.js](../../../module/activeEffect/witcherActiveEffect.js.md), [templates/partials/crit-wounds-table.hbs](../../../templates/partials/crit-wounds-table.hbs.md), [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Right___Stabilized__NHNctAuhsapGZXiP.json](Compound_Arm_Fracture__Right___Stabilized__NHNctAuhsapGZXiP.json.md).

[Протокол и границы](../../../../review-log.md#task-0004012) — TASK-0004.012; процессы [R012-16](../../../../cross-check-0002.md#r012-16), [R012-24](../../../../cross-check-0002.md#r012-24). В этой порции выполнена статическая сверка; прежние опыты сохраняют свои даты и фасады. Новых поведенческих запусков нет; браузер, мир, сеть и запись в БД не запускались.
