# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Concussion__Stabilized__AFkm8KjxkwYxOCQo.json

## Текущий численный контракт — 14.3.1.00069

TASK-0010.010: численные строки находятся в `system.changes`, с `type`/`phase` и настройками канала. Классификация: П: 3. П — параметр, Р — бросок, Р? — выбор условия; native — периодика. Все fullEffect/shiftsCap/affectsAdvancement выключены. Исключения: нет. UUID/стадия/переходы/заживление, прочие поля Item/AE сохранены.
...
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.ItXAMwWil2A7IqRv"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Concussion__Stabilized__AFkm8KjxkwYxOCQo.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Concussion__Stabilized__AFkm8KjxkwYxOCQo.json) |
...
Экспорт Concussion (Stabilized): состояние stabilized, локация head. Вложенных ActiveEffect: 1; изменений: 3. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Concussion (Stabilized) | Корневой Item; name:3, _id:97 | ID `AFkm8KjxkwYxOCQo`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.AFkm8KjxkwYxOCQo` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!AFkm8KjxkwYxOCQo"` / `"ox3lLmV3zp0K67Ht"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.ItXAMwWil2A7IqRv"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Concussion

ID `ZafZYnNAfTfMqNmS`; _key=`"!items.effects!AFkm8KjxkwYxOCQo.ZafZYnNAfTfMqNmS"`. Origin=`"Item.IK7pM8p3NcM4thcz"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
| 2 / 51 | `system.stats.ref.totalModifiers` | 2 / `"-1"` / `null` | add; `-1`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -1 |
| 3 / 57 | `system.stats.dex.totalModifiers` | 2 / `"-1"` / `null` | add; `-1`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -1 |
...
Item `AFkm8KjxkwYxOCQo` («Concussion (Stabilized)»): `difficult/stabilized/head`; 1 ActiveEffect, 3 changes. Предшественник: `IK7pM8p3NcM4thcz`; `followUp` ведёт к `ItXAMwWil2A7IqRv` («Concussion (Treated)», `treated/head`).

Этот Item отсекается начальным фильтром `treatment=none`, но доступен через ссылку предыдущего состояния. При контрольном `BODY.max=5` срок 10 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.stats.int.totalModifiers`, `system.stats.ref.totalModifiers`, `system.stats.dex.totalModifiers`.
