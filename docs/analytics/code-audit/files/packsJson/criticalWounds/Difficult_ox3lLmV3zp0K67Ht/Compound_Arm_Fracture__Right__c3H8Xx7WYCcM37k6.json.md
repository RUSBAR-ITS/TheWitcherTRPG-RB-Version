# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Arm_Fracture__Right__c3H8Xx7WYCcM37k6.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.NHNctAuhsapGZXiP"` |
...
Экспорт Compound Arm Fracture (Right): состояние none, локация rightArm. Вложенных ActiveEffect: 1; изменений: 1. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
ID `32L2d5MfPiaENuqI`; _key=`"!items.effects!c3H8Xx7WYCcM37k6.32L2d5MfPiaENuqI"`. Origin=`"Item.saPd4IMUCv5qZE60"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Item `c3H8Xx7WYCcM37k6` («Compound Arm Fracture (Right)»): `difficult/none/rightArm`; 1 ActiveEffect, 1 changes. Предшественник: нет; `followUp` ведёт к `NHNctAuhsapGZXiP` («Compound Arm Fracture (Right - Stabilized)», `stabilized/rightLeg`).
