# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Left___Treated__kfyfxEVsMRUDDk1A.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Left___Treated__kfyfxEVsMRUDDk1A.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Left___Treated__kfyfxEVsMRUDDk1A.json) |
...
| Изменения относительно коммита | Нет; 104 строк; SHA-256 d5f5448ba2a47f80c5b35d65e2ee6b872d3f1fc57597b51de0f4dbcbcb654452 |
| Задача и порция | [TASK-0003.060](../../../../../../tasks/task-0003.060.md) |
| Запись перекрёстной сверки | [Протокол .060](../../../../review-log.md#task-0003060) |

## Назначение файла

Экспорт Compound Leg Fracture (Left - Treated): состояние treated, локация leftLeg. Вложенных ActiveEffect: 1; изменений: 3. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Compound Leg Fracture (Left - Treated) | Корневой Item; name:3, _id:97 | ID `kfyfxEVsMRUDDk1A`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.kfyfxEVsMRUDDk1A` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!kfyfxEVsMRUDDk1A"` / `"ox3lLmV3zp0K67Ht"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
ID `HPHyTnnqile8vU2V`; _key=`"!items.effects!kfyfxEVsMRUDDk1A.HPHyTnnqile8vU2V"`. Origin=`"Item.kfyfxEVsMRUDDk1A"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Item `kfyfxEVsMRUDDk1A` («Compound Leg Fracture (Left - Treated)»): `difficult/treated/leftLeg`; 1 ActiveEffect, 3 changes. Предшественник: `NiGtzaHs4dUj8Pmd`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
