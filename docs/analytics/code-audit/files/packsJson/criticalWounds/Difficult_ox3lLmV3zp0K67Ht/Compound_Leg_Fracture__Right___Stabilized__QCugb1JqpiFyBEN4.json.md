...
| `treatedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.3SwpPbi2ddEJkebh"` |
| `daysHealed` | `0` |
| `sterilized` | `false` |
...
| Поле | Значение |
| --- | --- |
| Источник | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Right___Stabilized__QCugb1JqpiFyBEN4.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Right___Stabilized__QCugb1JqpiFyBEN4.json) |
...
| Имя / ID | Compound Leg Fracture (Right - Stabilized) / `QCugb1JqpiFyBEN4` |
| Строк / SHA-256 | 104 / `5c8c51f8beb6a434d25a76221508b0f53dc841799697d03089602874be1b2ed6` |
...
| `description` | "<p>halves SPD, Dodge/Escape, and Athletics</p>" |
...
| `healingTime` | 0 |
| `sterilized` | false |
| `followUp` | "Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.3SwpPbi2ddEJkebh" |

1 ActiveEffect; 3 changes. ID и `_key` сохранённых эффектов, origin, transfer, duration и несвязанные значения сохранены.

#### Compound Leg Fracture (Right - Stabilized) — XOBIfzJ65964vrHX

Строка `_id`: 36. `disabled=false`, `transfer=true`; statuses: `[]`.

| key | mode | value | priority |
| --- | --- | --- | --- |
| `system.stats.spd.max` | 1 | "0.5" | null |
| `system.skills.ref.dodge.value` | 1 | "0.5" | null |
| `system.skills.dex.athletics.value` | 1 | "0.5" | null |
...
Следующий этап: [Compound Leg Fracture (Right - Treated)](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Right___Treated__3SwpPbi2ddEJkebh.json) — `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.3SwpPbi2ddEJkebh`.
...
| --- | --- | --- | --- |
| Compound Leg Fracture (Right - Stabilized) | Корневой Item; name:3, _id:97 | ID `QCugb1JqpiFyBEN4`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.QCugb1JqpiFyBEN4` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.3SwpPbi2ddEJkebh"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Compound Leg Fracture (Right - Stabilized)

ID `XOBIfzJ65964vrHX`; _key=`"!items.effects!QCugb1JqpiFyBEN4.XOBIfzJ65964vrHX"`. Origin=`"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.QCugb1JqpiFyBEN4"` — UUID текущего Item в компедиуме, сопоставлен с экспортом. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
[issue-00121](../../../../../../issues/closed/issue-00121.md) — ожидание замены, [issue-00127](../../../../../../issues/closed/issue-00127.md) — граница завершения лечения, [issue-00288](../../../../../../issues/closed/issue-00288.md) — повтор name/type. [issue-00036](../../../../../../issues/potential/issue-00036.md) — max/value характеристики. [issue-00327](../../../../../../issues/closed/issue-00327.md) — локации стабилизированных правых переломов. Статусы potential сохранены, подтверждения/исправления не выполнялись. Наличие этих общих проблем не объявляет дефектом каждое поле или папку.
...
Item `QCugb1JqpiFyBEN4` («Compound Leg Fracture (Right - Stabilized)»): `difficult/stabilized/rightArm`; 1 ActiveEffect, 3 changes. Предшественник: `Rf0m4mGjeHEl0PxP`; `followUp` ведёт к `3SwpPbi2ddEJkebh` («Compound Leg Fracture (Right - Treated)», `treated/rightLeg`).

Этот Item отсекается начальным фильтром `treatment=none`, но доступен через ссылку предыдущего состояния. При контрольном `BODY.max=5` срок 10 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.stats.spd.max`, `skill.value`.
