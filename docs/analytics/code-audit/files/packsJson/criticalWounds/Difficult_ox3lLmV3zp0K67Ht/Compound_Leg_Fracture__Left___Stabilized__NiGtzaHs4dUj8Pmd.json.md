# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Left___Stabilized__NiGtzaHs4dUj8Pmd.json

| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Left___Stabilized__NiGtzaHs4dUj8Pmd.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Left___Stabilized__NiGtzaHs4dUj8Pmd.json) |
...
| Изменения относительно коммита | Нет; 104 строк; SHA-256 affa326de4b209b513c4f7ca3d5d0f7871b0abb64f5b04ab16b0cbc20367174e |
| Задача и порция | [TASK-0003.060](../../../../../../tasks/task-0003.060.md) |
| Запись перекрёстной сверки | [Протокол .060](../../../../review-log.md#task-0003060) |

## Назначение файла

Экспорт Compound Leg Fracture (Left - Stabilized): состояние stabilized, локация leftLeg. Вложенных ActiveEffect: 1; изменений: 3. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Compound Leg Fracture (Left - Stabilized) | Корневой Item; name:3, _id:97 | ID `NiGtzaHs4dUj8Pmd`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.NiGtzaHs4dUj8Pmd` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!NiGtzaHs4dUj8Pmd"` / `"ox3lLmV3zp0K67Ht"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG.criticalWounds.Item.kfyfxEVsMRUDDk1A"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Compound Leg Fracture (Left - Stabilized)

ID `SZ1HqkNycACTg82X`; _key=`"!items.effects!NiGtzaHs4dUj8Pmd.SZ1HqkNycACTg82X"`. Origin=`"Compendium.TheWitcherTRPG.criticalWounds.Item.NiGtzaHs4dUj8Pmd"` — UUID текущего Item в компедиуме, сопоставлен с экспортом. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Папка: [Difficult](_Folder.json.md). Предшественник: [Compound Leg Fracture (Left)](Compound_Leg_Fracture__Left__flpxY7FVPGevwfcg.json.md). Следующий Item: [Compound Leg Fracture (Left - Treated)](Compound_Leg_Fracture__Left___Treated__kfyfxEVsMRUDDk1A.json.md), `kfyfxEVsMRUDDk1A`, treatment=treated. Реальный treat инициировал создание этого документа, затем удаление текущего. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.
...
[issue-00121](../../../../../../issues/potential/issue-00121.md) — ожидание замены, [issue-00127](../../../../../../issues/potential/issue-00127.md) — граница завершения лечения, [issue-00288](../../../../../../issues/potential/issue-00288.md) — повтор name/type. [issue-00036](../../../../../../issues/potential/issue-00036.md) — max/value характеристики. Статусы potential сохранены, подтверждения/исправления не выполнялись. Наличие этих общих проблем не объявляет дефектом каждое поле или папку.
...
Item `NiGtzaHs4dUj8Pmd` («Compound Leg Fracture (Left - Stabilized)»): `difficult/stabilized/leftLeg`; 1 ActiveEffect, 3 changes. Предшественник: `flpxY7FVPGevwfcg`; `followUp` ведёт к `kfyfxEVsMRUDDk1A` («Compound Leg Fracture (Left - Treated)», `treated/leftLeg`).
