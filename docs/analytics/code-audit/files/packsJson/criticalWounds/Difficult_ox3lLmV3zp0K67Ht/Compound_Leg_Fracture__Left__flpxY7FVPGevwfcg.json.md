# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Compound_Leg_Fracture__Left__flpxY7FVPGevwfcg.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.NiGtzaHs4dUj8Pmd"` |
...
Экспорт Compound Leg Fracture (Left): состояние none, локация leftLeg. Вложенных ActiveEffect: 2; изменений: 4. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
ID `j4w4dskXupoQQTmq`; _key=`"!items.effects!flpxY7FVPGevwfcg.j4w4dskXupoQQTmq"`. Origin=`"Item.flpxY7FVPGevwfcg"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Item `flpxY7FVPGevwfcg` («Compound Leg Fracture (Left)»): `difficult/none/leftLeg`; 2 ActiveEffect, 4 changes. Предшественник: нет; `followUp` ведёт к `NiGtzaHs4dUj8Pmd` («Compound Leg Fracture (Left - Stabilized)», `stabilized/leftLeg`).
