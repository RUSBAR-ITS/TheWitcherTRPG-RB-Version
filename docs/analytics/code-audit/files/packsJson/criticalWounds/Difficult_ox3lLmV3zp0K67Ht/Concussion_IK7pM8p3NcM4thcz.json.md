# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Concussion_IK7pM8p3NcM4thcz.json
...
| `canHeal` | `false` |
| `healingDuration` | `""` |
| `stabilizedWound` | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.AFkm8KjxkwYxOCQo"` |
...
Экспорт Concussion: состояние none, локация head. Вложенных ActiveEffect: 1; изменений: 3. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| description | `"<p>Make a Stun save every 1d6 rounds</p>"` | HTMLField; enrichedText/createEnrichedText, редактор и сообщение applyCritWound. Текст не преобразуется в changes. |
...
ID `ZafZYnNAfTfMqNmS`; _key=`"!items.effects!IK7pM8p3NcM4thcz.ZafZYnNAfTfMqNmS"`. Origin=`"Item.IK7pM8p3NcM4thcz"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
| --- | --- | --- | --- | --- |
| 1 / 46 | `system.stats.int.totalModifiers` | 2 / `"-2"` / `null` | add; `-2`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -2 |
| 2 / 52 | `system.stats.ref.totalModifiers` | 2 / `"-2"` / `null` | add; `-2`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -2 |
| 3 / 58 | `system.stats.dex.totalModifiers` | 2 / `"-2"` / `null` | add; `-2`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -2 |
...
Item `IK7pM8p3NcM4thcz` («Concussion»): `difficult/none/head`; 1 ActiveEffect, 3 changes. Предшественник: нет; `followUp` ведёт к `AFkm8KjxkwYxOCQo` («Concussion (Stabilized)», `stabilized/head`).

Этот Item участвует в исходных кандидатах индекса по сохранённой степени и локации. При контрольном `BODY.max=5` срок 10 дней независимо от штрафов к `BODY.value`. Сверены адреса: `system.stats.int.totalModifiers`, `system.stats.ref.totalModifiers`, `system.stats.dex.totalModifiers`.
