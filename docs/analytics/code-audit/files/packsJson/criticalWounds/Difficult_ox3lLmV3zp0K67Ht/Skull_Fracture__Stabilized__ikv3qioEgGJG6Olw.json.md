# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Skull_Fracture__Stabilized__ikv3qioEgGJG6Olw.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Skull_Fracture__Stabilized__ikv3qioEgGJG6Olw.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Skull_Fracture__Stabilized__ikv3qioEgGJG6Olw.json) |
...
Экспорт Skull Fracture (Stabilized): состояние stabilized, локация head. Вложенных ActiveEffect: 1; изменений: 2. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Skull Fracture (Stabilized) | Корневой Item; name:3, _id:91 | ID `ikv3qioEgGJG6Olw`; UUID `Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.ikv3qioEgGJG6Olw` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!ikv3qioEgGJG6Olw"` / `"ox3lLmV3zp0K67Ht"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
| followUp | `"Compendium.TheWitcherTRPG-RB-Version.criticalWounds.Item.v4RVIshohh1PPuAu"` | DocumentUUIDField типа Item; treat загружает адресата или удаляет конечный Item. |

### ActiveEffect 1: Skull Fracture

ID `XMpXbZE84msMUpnF`; _key=`"!items.effects!ikv3qioEgGJG6Olw.XMpXbZE84msMUpnF"`. Origin=`"Item.UImIh794nOy21jg2"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
Item `ikv3qioEgGJG6Olw` («Skull Fracture (Stabilized)»): `difficult/stabilized/head`; 1 ActiveEffect, 2 changes. Предшественник: `UImIh794nOy21jg2`; `followUp` ведёт к `v4RVIshohh1PPuAu` («Skull Fracture (Treated)», `treated/head`).
