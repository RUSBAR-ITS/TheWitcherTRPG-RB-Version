# packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Sucking_Chest_Wound__Treated__wFul3Zr7mMaKjA5I.json
...
| Поле | Значение |
| --- | --- |
| Исходный файл | [packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Sucking_Chest_Wound__Treated__wFul3Zr7mMaKjA5I.json](../../../../../../../packsJson/criticalWounds/Difficult_ox3lLmV3zp0K67Ht/Sucking_Chest_Wound__Treated__wFul3Zr7mMaKjA5I.json) |
...
| Изменения относительно коммита | Нет; 98 строк; SHA-256 f84134cff89f8e3ab6ec12456fed61ff27ba9d5dcfe7780aba623db63d707f19 |
| Задача и порция | [TASK-0003.060](../../../../../../tasks/task-0003.060.md) |
| Запись перекрёстной сверки | [Протокол .060](../../../../review-log.md#task-0003060) |

## Назначение файла

Экспорт Sucking Chest Wound (Treated): состояние treated, локация torso. Вложенных ActiveEffect: 1; изменений: 2. Данные задают шаблон травмы и её переход; действия выполняют потребители системы и Foundry.
...
| --- | --- | --- | --- |
| Sucking Chest Wound (Treated) | Корневой Item; name:3, _id:91 | ID `wFul3Zr7mMaKjA5I`; UUID `Compendium.TheWitcherTRPG.criticalWounds.Item.wFul3Zr7mMaKjA5I` | Загрузка, копирование к Actor, подготовка, лечение/удаление |
| _key / folder / sort | Корневые поля | `"!items!wFul3Zr7mMaKjA5I"` / `"ox3lLmV3zp0K67Ht"` / `0` | Ключ экспорта, родительская папка и сортировка; sort не приоритет эффекта |
...
ID `whVWC3pPXx4UJTXH`; _key=`"!items.effects!wFul3Zr7mMaKjA5I.whVWC3pPXx4UJTXH"`. Origin=`"Item.tiVrEesPSzZ64HpZ"` — ссылка на мировой Item; существование адресата не проверялось. Цель переноса при transfer=true — Actor-владелец текущего Item; origin не выбирает другого владельца.
...
| --- | --- | --- | --- | --- |
| 1 / 45 | `system.stats.body.totalModifiers` | 2 / `"-1"` / `null` | add; `-1`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -1 |
| 2 / 51 | `system.stats.spd.totalModifiers` | 2 / `"-1"` / `null` | add; `-1`; priority=20; phase=initial | NumberField; [statData.js](../../../../../../../module/data/actor/templates/common/stats/statData.js); -1 |
...
Папка: [Difficult](_Folder.json.md). Предшественник: [Sucking Chest Wound (Stabilized)](Sucking_Chest_Wound__Stabilized__cfQ2OHPNVVKMDsDo.json.md). followUp=null; реальный treat инициировал только удаление текущего Item. Создание/удаление перехвачены и возвращали pending Promise. Переход загружает другой шаблон с его собственными эффектами; не переписывает изменения прежнего эффекта. На новой подготовке без Item его воздействия не применяются.

Контрольный Actor: восемь unmodifiedMax=5, HP.value=25, dodge.value=athletics.value=8 до эффектов, броня/вес=0. Результат: INT=5, WILL=5, REF=5, DEX=5, BODY=4, SPD=4; BODY.max=5, SPD.max=5; RUN=12, LEAP.value=2, LEAP.max=3, ENC=40, STUN=4, REC=4, HP.max=20, RESOLVE.max=25, FOCUS.max=15; healingTime=10. Навыки: dodge.value=8, dodge.activeEffectModifiers=0; athletics.value=8, athletics.activeEffectModifiers=0. Статусы=`[]`; turnStartEffects={}. Значения прочитаны непосредственно из подготовленных полей; сериализация DataModel/toObject возвращает источник и не заменяет такую проверку.
...
Item `wFul3Zr7mMaKjA5I` («Sucking Chest Wound (Treated)»): `difficult/treated/torso`; 1 ActiveEffect, 2 changes. Предшественник: `cfQ2OHPNVVKMDsDo`; `followUp=null`: ручной `treat()` запрашивает удаление этого Item.
