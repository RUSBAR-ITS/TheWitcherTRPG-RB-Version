...
При showLootFormula=true добавлен отдельный подписанный input system.lootQuantityFormula под названием. Этот параметр передаёт только активный лист монстра; ru/en подпись и подсказка используют новые ключи. Формулу проверяет тот же обработчик через parseLootQuantityFormula; она не меняет вес и фактическое количество при редактировании.
