...
**Назначение:** Два списка компонентов ритуала.

**Основные методы, сущности и действия:** В data-uuid записан component.uuid, а не component.item.uuid: недоступный документ не мешает редактировать/удалять сохранённую строку.

**Зависимости и потребители:** ritualData.prepareDerivedData; WitcherRitualSheet edit/remove. Другие разделы шаблона сохранены.
...
- `127`: `<a class="remove-component" title="{{localize "WITCHER.Item.RemoveComponent"}}"><i`
- `147`: `<a class="remove-component" title="{{localize "WITCHER.Item.RemoveComponent"}}"><i`
