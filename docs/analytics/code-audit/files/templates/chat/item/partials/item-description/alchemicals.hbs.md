# templates/chat/item/partials/item-description/alchemicals.hbs

| Поле | Значение |
| --- | --- |
| Исходный файл | [templates/chat/item/partials/item-description/alchemicals.hbs](../../../../../../../../../templates/chat/item/partials/item-description/alchemicals.hbs) |
| Тип файла | Handlebars |
| Статус анализа | Проверено |
| Дата проверки | 2026-09-12 |
| Ветка и коммит | rusbar-main, ee24c2605f4db98fad1ff6db024d2b0c26883670 |
| Изменения относительно коммита | Нет; совпадает со срезом TASK-0001 15da5b225535e34af4e132c701b5353ef4eb667f |
| Задача и порция | [TASK-0003.048](../../../../../../../../tasks/task-0003.048.md), 16 файлов / 920 логических строк; данный файл — 19 |
| Запись перекрёстной сверки | [TASK-0003.048](../../../../../../review-log.md#task-0003048) |

## Назначение файла

Текстовый partial описания алхимии, мутагенов и места получения компонента.
...
Для alchemical/mutagen при truthy effect создаётся list-item-description с заголовком WITCHER.Item.Description. Только mutagen добавляет жирную подпись WITCHER.Item.Effect перед текстом. minorMutation проверяется внутри той же общей ветви, но поле объявлено только MutagenData. component получает list-item-description и заголовок всегда, location может быть пустой. У прочих типов остаётся пустой section. effect/minorMutation/location — обычные {{}}: HTML экранируется, собственный enrichHTML отсутствует.
...
Буквальные пути чтения: `item.system.effect`, `item.system.location`, `item.system.minorMutation`.



## Основные функции и методы

Собственных функций нет. if/or/eq выбирают три типа и необязательный текст. Разные условия заголовка сохранены буквально: component рисует заголовок даже при пустой location.
...
| [module/data/item/alchemicalData.js](../../../../../../../../../module/data/item/alchemicalData.js) | AlchemicalData.effect | StringField эффекта. | Определение и место обращения сверены |
| [module/data/item/mutagenData.js](../../../../../../../../../module/data/item/mutagenData.js) | MutagenData.effect/minorMutation | Строки эффекта и малой мутации. | Определение и место обращения сверены |
| [module/data/item/componentData.js](../../../../../../../../../module/data/item/componentData.js) | ComponentData.location | Место получения; не описание CommonItemData. | Определение и место обращения сверены |
...
| --- | --- | --- | --- |
| Полнота | 19 строк исходного HBS | Все ветви, пути чтения и helpers описаны | Соседние файлы проверены только по связи |
| Рендер/модель | Настоящие Handlebars, модели и helpers | Группы 01, 05, 16: defaults и заполненные настоящие модели; HTML в effect/minorMutation/location стал экранированным текстом. Не поддерживаемые этой частью типы дают section без описания. | UUID/Document/чат — фасады, запись отсутствует |
...
Текущая статическая сверка завершена; прежние пофайловые опыты сохраняют свои даты и фасады. Условные alchemical/mutagen effect и minorMutation, безусловный component location и префикс Effect только для mutagen сопоставлены с моделями. Обычные {{}} экранируют строки; эффект не применяется этим partial. Непроверенные границы и следующий критерий: [U016-06](../../../../../../cross-check-0002.md#u016-06), [U016-02](../../../../../../cross-check-0002.md#u016-02), [U016-08](../../../../../../cross-check-0002.md#u016-08). Полный браузерный цикл, мир, HTTP и запись в БД не выполнялись; смысл перевода/игровых правил не оценивался.
...
Условные alchemical/mutagen effect и minorMutation, безусловный component location и префикс Effect только для mutagen сопоставлены с моделями. Обычные {{}} экранируют строки; эффект не применяется этим partial.
