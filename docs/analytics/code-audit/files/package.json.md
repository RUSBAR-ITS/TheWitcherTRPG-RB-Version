...
Новый install/ci/build/extract и повторное скачивание CLI не проводились. Публикация архива и native LevelDB остаются внешней границей .018; package-lock/.github используются только как прежние справочные источники.

## Связанные проблемы

Проблемы скрипта извлечения описаны в [00313](../../../issues/closed/issue-00313.md), [00314](../../../issues/closed/issue-00314.md), [00315](../../../issues/closed/issue-00315.md). Для самого package.json новая issue не зарегистрирована.
...
build:db запускает utils/packs.mjs; переданные package pack не читаются скриптом. extract запускает utils/extract.mjs. Это операции с записью, не команды тестов. Диапазоны CLI ^3.0.3/prettier ^3.3.3 отделены от lock 3.0.3/3.3.3 и от фактического разрешения зависимостей; внешний контракт CLI 3.0.3 берётся из проверенного опубликованного исходника .050, а не приписывается работающей установке.

Сопоставленные определения и потребители: [utils/packs.mjs](utils/packs.mjs.md), [utils/extract.mjs](utils/extract.mjs.md), [build.json](build.json.md).
