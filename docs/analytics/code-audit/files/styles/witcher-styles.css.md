...
Проверка наличия/подключения статическая; C01/C02 .050 использованы как прежние доказательства. HTTP CSS/font, браузерный каскад, геометрия и сторонние темы не проверены; адресат — .016.

## Связанные проблемы

[issue-00002](../../../../issues/closed/issue-00002.md). Сохраняют статус potential; регистрация не означает подтверждения или исправления.
...
system.json подключает этот файл; 35 относительных импортов ведут ко всем остальным CSS реестра. Font URL существует локально. useWitcherFont→ready→witcher-style применяет объявленный Thewitcher только после индекса. Собственные .grid и .skill-column связаны с текущими Item/skill-шаблонами; наличие прежнего monster-skill-tab не делает его действующим PARTS.

Сопоставленные определения и потребители: [system.json](../system.json.md), [module/TheWitcherTRPG.js](../module/TheWitcherTRPG.js.md), [module/setup/settings.js](../module/setup/settings.js.md), [styles/system-styles.css](system-styles.css.md), [styles/dialog.css](dialog.css.md), [templates/partials/character/tab-skills.hbs](../templates/partials/character/tab-skills.hbs.md).
