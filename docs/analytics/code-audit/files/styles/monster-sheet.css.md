...
Текущая статическая сверка завершена; прежние пофайловые опыты сохраняют свои даты и фасады. Прежний полный Monster шаблон не является текущим PARTS, но глобальные monster-img/category/armor правила остаются в cascade. Новые размеры перекрывают старые при совпадении более специфичных селекторов; width/float не сбрасываются новым margin. Непроверенные границы и следующий критерий: [U016-04](../../cross-check-0002.md#u016-04). Полный браузерный цикл, мир, HTTP и запись в БД не выполнялись; смысл перевода/игровых правил не оценивался.
...
Прежний полный Monster шаблон не является текущим PARTS, но глобальные monster-img/category/armor правила остаются в cascade. Новые размеры перекрывают старые при совпадении более специфичных селекторов; width/float не сбрасываются новым margin. Подключение: импорт № 5 в общем CSS; 51 правил / 148 деклараций.
