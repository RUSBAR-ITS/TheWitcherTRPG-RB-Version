...
Текущая статическая сверка завершена; прежние пофайловые опыты сохраняют свои даты и фасады. skill-info переключается class active. Четыре IP-строки и totalSkills/totalProfSkills сопоставлены: readonly+disabled итоги исключаются core FormDataExtended, одно readonly по умолчанию допускается. Непроверенные границы и следующий критерий: [U016-02](../../cross-check-0002.md#u016-02). Полный браузерный цикл, мир, HTTP и запись в БД не выполнялись; смысл перевода/игровых правил не оценивался.
...
skill-info переключается class active. Четыре IP-строки и totalSkills/totalProfSkills сопоставлены: readonly+disabled итоги исключаются core FormDataExtended, одно readonly по умолчанию допускается. Подключение: импорт № 9 в общем CSS; 26 правил / 75 деклараций.
