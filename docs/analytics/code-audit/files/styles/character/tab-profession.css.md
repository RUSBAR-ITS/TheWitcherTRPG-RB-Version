...
Импорт идёт после общего profession-sheet.css; более конкретный scope активной profession вкладки Character/Monster задаёт row вместо общего column путей. PARTS и cssClass дают реальную связь с Actor, а Item профессии использует общий стиль вне этого scope. CSS не применяет расовые эффекты и не изменяет skill.value.

Сопоставленные определения и потребители: [styles/witcher-styles.css](../witcher-styles.css.md), [styles/profession-sheet.css](../profession-sheet.css.md), [module/actor/sheets/WitcherCharacterSheet.js](../../module/actor/sheets/WitcherCharacterSheet.js.md), [templates/partials/character/tab-profession.hbs](../../templates/partials/character/tab-profession.hbs.md).
