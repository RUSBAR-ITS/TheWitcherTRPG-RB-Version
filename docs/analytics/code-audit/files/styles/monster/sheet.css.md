...
Текущая статическая сверка завершена; прежние пофайловые опыты сохраняют свои даты и фасады. Поздний импорт повторяет общую grid-схему и задаёт sidebar span. Классы actor+monster формирует наследование options; вычисленный layout и взаимодействие размеров не проверены. Непроверенные границы и следующий критерий: [U016-04](../../../cross-check-0002.md#u016-04). Полный браузерный цикл, мир, HTTP и запись в БД не выполнялись; смысл перевода/игровых правил не оценивался.
...
Поздний импорт повторяет общую grid-схему и задаёт sidebar span. Классы actor+monster формирует наследование options; вычисленный layout и взаимодействие размеров не проверены. Подключение: импорт № 32 в общем CSS; 6 правил / 11 деклараций.
