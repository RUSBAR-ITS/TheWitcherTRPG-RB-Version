...
Новые правила ограничены profession-configuration: пределы viewport, компактный заголовок, закреплённая навигация, прокрутка вкладки, полноширинные карточки, grid280px/остаток с зазором16px и container query одной колонки до640px. Используется переменная границы ApplicationV2 --color-border. Остальные profession-card не переопределены.

[Исходник](../../../../../styles/profession-sheet.css) · [Проверки и границы](../../../task-0012-checks.md#fixes-00142). Локальная приёмка выполнена; игровой повтор впереди. Более ранние датированные описания ниже — история.
...
Общий стиль задаёт карточки, три цветных пути и hover-анимацию profession-roll; текущий Item HBS создаёт десять карточек, но не Actor-кнопки броска. Более поздний character/tab-profession уточняет направление раскладки в Actor scope. Анимация и CSS-подписи не вычисляют профессиональный бонус и не запускают Roll.

Сопоставленные определения и потребители: [templates/sheets/item/profession-sheet.hbs](../templates/sheets/item/profession-sheet.hbs.md), [templates/partials/character/tab-profession.hbs](../templates/partials/character/tab-profession.hbs.md), [styles/character/tab-profession.css](character/tab-profession.css.md).
