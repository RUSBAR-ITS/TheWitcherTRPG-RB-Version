...
2026-09-19, TASK-0010.014–.018. carry-section больше не растягивается flex:1. carry-meter — Grid; carry-bar и carry-labels занимают одну ячейку, подписи центрированы без absolute/height100%. Полоса1.75rem. Цифры/условие overweight вычисляются прежними потребителями Actor.

[Исходник](../../../../../styles/tab-inventory.css), [реализация и проверки](../../../task-0010-ui-fixes.md). Локальная проверка пройдена; реальная игровая/визуальная приёмка ожидается (HTTP502). Следующие датированные разделы описывают прежние срезы.
...
Текущая статическая сверка завершена; прежние пофайловые опыты сохраняют свои даты и фасады. overweight включается gte, включая равенство; неверная WebKit-ветвь 00311 отделена от Gecko. background-color тега с 1px solid — отдельная 00310; AST разбирает обе декларации без доказательства принятия браузером. Непроверенные границы и следующий критерий: [U016-04](../../cross-check-0002.md#u016-04). Полный браузерный цикл, мир, HTTP и запись в БД не выполнялись; смысл перевода/игровых правил не оценивался.
...
overweight включается gte, включая равенство; неверная WebKit-ветвь 00311 отделена от Gecko. background-color тега с 1px solid — отдельная 00310; AST разбирает обе декларации без доказательства принятия браузером. Подключение: импорт № 7 в общем CSS; 48 правил / 147 деклараций.

Сопоставленные определения и потребители: [templates/sheets/actor/tabs/tab-inventory.hbs](../templates/sheets/actor/tabs/tab-inventory.hbs.md), [styles/witcher-styles.css](witcher-styles.css.md).

[Протокол и границы](../../review-log.md#task-0004016) — TASK-0004.016; процессы [R016-13](../../cross-check-0002.md#r016-13), [R016-14](../../cross-check-0002.md#r016-14). В этой порции выполнена статическая сверка; прежние опыты сохраняют свои даты и фасады. Новых поведенческих запусков нет; браузер, мир, сеть и запись в БД не запускались.
