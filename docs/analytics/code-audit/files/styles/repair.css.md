...
Текущая статическая сверка завершена; прежние пофайловые опыты сохраняют свои даты и фасады. Первый/последний td и высота прокрутки относятся к repair-dialog; сообщение ремонта не получает весь scope. Глобальная ветвь th из components-list учитывается отдельно от выравнивания значений. Непроверенные границы и следующий критерий: [U016-04](../../cross-check-0002.md#u016-04). Полный браузерный цикл, мир, HTTP и запись в БД не выполнялись; смысл перевода/игровых правил не оценивался.
...
Первый/последний td и высота прокрутки относятся к repair-dialog; сообщение ремонта не получает весь scope. Глобальная ветвь th из components-list учитывается отдельно от выравнивания значений. Подключение: импорт № 23 в общем CSS; 5 правил / 8 деклараций.
