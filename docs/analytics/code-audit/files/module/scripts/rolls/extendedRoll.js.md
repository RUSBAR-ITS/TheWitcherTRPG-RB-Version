...
**14.3.1.00065, TASK-0010.007.** При Number.isFinite(threshold) выполняется сравнение. Обратное <= автоматически проваливается приT<=0, строгое < приT<=1, даже если legacy-крит привёл к0. У отсутствующего порога нет success/failure. Обычные >/>= сохранены. Нет второго глобального сбора модификаторов: вызывающий передаёт готовую формулу и итоговый порог. Другие правила критов не менялись.

[Исходник](../../../../../../../module/scripts/rolls/extendedRoll.js); [проверка и пределы](../../../../../task-0010-007-checks.md).
...
[issue-00033](../../../../../../issues/closed/issue-00033.md), [issue-00103](../../../../../../issues/closed/issue-00103.md), [issue-00184](../../../../../../issues/potential/issue-00184.md). Две прежние проблемы подготовки формулы сопоставлены с реальным парсером. issue-00184 описывает окно между созданием сообщения и сохранением дополнительных флагов.
...
castSpell задаёт RollConfig({showResult:false}) с threshold−1, затем сам ожидает roll.toMessage. difficultyCheck ритуала не передаётся; группа 25 total 15/DC 25 не имеет options.success. Fumble устанавливается настоящим extendedRoll; группа 24 проверяет пропуск эффектов при оставшихся кнопках и вызове области.
