...
2026-09-19. Код .006 сохранён. game.api.applyActiveEffectToActorViaId остаётся конечным API для async чтения/доставки и JSON-результата. Потребители с исходным игровым действием теперь обращаются в effectDelivery.js/createItemEffectDelivery, чтобы получить карточку без повторного consume/урона/защиты. Простые API-вызовы сами карточку не создают; generic query и системная регистрация прежние.
...
Три публичные обёртки возвращают JSON-результат доставки. ViaId ожидает resolveEffectSource из effectDelivery.js, затем передаёт копии в applyActiveEffectToActor. Тот использует deliverActorEffects и локализованное предупреждение notifyEffectDelivery. ToTargets последовательно ожидает каждую цель и возвращает массив результатов. Прямого fromUuidSync/GM query/создания Document здесь больше нет; источник, право и запись разделены. Пустой триггер допустим. Обработка результатов конкретными боевыми потребителями — .008; общая проверка всех целей до первого применения — .007.
...
castSpell фильтрует Item.effects по system.applySelf/applyOnTarget, передаёт damage.duration и не ожидает функций. Группы 22/27/33 исполнили настоящие helpers с façade clone/createEmbeddedDocuments/query: маршруты self/target и remote установлены, завершение записи не гарантировано. Перенос duration в настоящий clone в этой порции заново не воспроизводился (issue-00044).
