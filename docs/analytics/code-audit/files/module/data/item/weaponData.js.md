...
Настоящие WeaponData использованы в полном [потребителе атаки](../../actor/mixins/weaponAttackMixin.js.md). quantity остаётся строкой, isEnoughThrowable сравнивает >0. Нулевой ammo доходит до запроса quantity−1; пустой список не останавливает бросок (issue-00260). fast создаёт два броска, расход до цикла один; соответствие правилу расхода не установлено. enhancementItems в отдельном сценарии задан вручную, полная prepareDerivedData с настоящим владельцем не запускалась.
