...
Стандартный spellAttackSkill='spellcasting' отсутствует в skillMap; getUsedSkill использует fallback spellcast, тогда как getItemAttack сообщает spellcasting. Группа 13 проверила расхождение в полном castSpell, не утверждая отказ всех заклинаний. Опции ctrl/alt/shift в этот вызов не передаются.
