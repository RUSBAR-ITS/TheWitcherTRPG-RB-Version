...
Rewards.currencyRewardDialog перечисляет семь CONFIG.WITCHER.currency, включая falsecoin; не применяет rates/excluded конвертера. Log.addCurrencyReward пишет один валютный остаток плюс историю. Character/Monster/Loot имеют currency, но последние не имеют logs — выдача через Rewards для них несовместима (233). Неизвестный key в изолированном updateSource не стал полем currency (235).
