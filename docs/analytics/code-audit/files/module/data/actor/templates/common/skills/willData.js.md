# module/data/actor/templates/common/skills/willData.js

| Поле | Значение |
| --- | --- |
| Исходный файл | [module/data/actor/templates/common/skills/willData.js](../../../../../../../../../../module/data/actor/templates/common/skills/willData.js) |
...
Определяет Will — группу WILL из 7 базовых навыков Actor и миграцию их ключей подписи.

## Условия использования

[skills()](../../../../../../../../../../module/data/actor/templates/common/skills/skillsData.js) создаёт EmbeddedDataField(Will) по ключу will; [CommonActorData.defineSchema](../../../../../../../../../../module/data/actor/commonActorData.js) включает результат в SchemaField skills. Путь экземпляра: system.skills.will. Для импорта требуются foundry.data.fields и foundry.abstract.DataModel.

## Введённые сущности и действия с ними

Экспорт по умолчанию — класс Will; локальный fields ссылается на API Foundry. Все поля таблицы — EmbeddedDataField([Skill](skillData.js.md), {label}). В колонках подписи приведены полные ключи; ключ skillMap и множитель стоимости проверены по config.js, но не задаются этой моделью.

| Ключ в system.skills.will | label поля группы | label после миграции | Ключ skillMap / costMultiplier |
| --- | --- | --- | --- |
| `courage` | `WITCHER.skills.courage.label` | `WITCHER.skills.courage.label` | `courage` / 1 |
| `hexweave` | `WITCHER.skills.hexWeaving.label` | `WITCHER.skills.hexWeaving.label` | `hexweave` / 2 |
| `intimidation` | `WITCHER.skills.intimidation.label` | `WITCHER.skills.intimidation.label` | `intimidation` / 1 |
| `spellcast` | `WITCHER.skills.spellCasting.label` | `WITCHER.skills.spellCasting.label` | `spellcast` / 2 |
| `resistmagic` | `WITCHER.skills.resistMagic.label` | `WITCHER.skills.resistMagic.label` | `resistmagic` / 2 |
| `resistcoerc` | `WITCHER.skills.resistCoercion.label` | `WITCHER.skills.resistCoercion.label` | `resistcoerc` / 1 |
| `ritcraft` | `WITCHER.skills.ritualCrafting.label` | `WITCHER.skills.ritualCrafting.label` | `ritcraft` / 2 |
...
Пять специальных подписей: hexweave→hexWeaving, spellcast→spellCasting, resistmagic→resistMagic, resistcoerc→resistCoercion, ritcraft→ritualCrafting. В magicSkills конфигурации входят spellcast, ritcraft, hexweave; это внешний список, не условие этой схемы.
...
Динамические потребители всех навыков перечислены с файлами и местами обращения в [карточке Skill](skillData.js.md): листы и edit-skills читают/изменяют поля system.skills.will.<навык>; rollSkillCheck читает value, addActiveEffects отдельно читает activeEffectModifiers; мастер и подсказки эффектов конструируют пути по skillMap; настройки монстра адресуют isVisible. Это связи по структуре данных, не импорты Will.

Особые потребители — [module/actor/mixins/castSpellMixin.js](../../../../../../../../../../module/actor/mixins/castSpellMixin.js):30 (will[usedSkill.name].value), [module/actor/mixins/skillMixin.js](../../../../../../../../../../module/actor/mixins/skillMixin.js):7–40 (повышение, список magicSkills и стоимость из config.js). Изолированное наблюдение расхода очков описано в issue-00017.
...
castSpell прямо индексирует system.skills.will[usedSkill.name].value. spellcast/ritcraft/hexweave работают для стандартных типов; у выбранного навыка вне WILL эта индексация ошибочна, но штатный UI предлагает магические навыки. Уровень и modifiers проверены группами 01/02/09.
