...
| Поле | Значение |
| --- | --- |
| Исходный файл | [module/data/actor/templates/common/skills/dexData.js](../../../../../../../../../../module/data/actor/templates/common/skills/dexData.js) |
...
Определяет Dexterity — группу DEX из 5 базовых навыков Actor и миграцию их ключей подписи.

## Условия использования

[skills()](../../../../../../../../../../module/data/actor/templates/common/skills/skillsData.js) создаёт EmbeddedDataField(Dexterity) по ключу dex; [CommonActorData.defineSchema](../../../../../../../../../../module/data/actor/commonActorData.js) включает результат в SchemaField skills. Путь экземпляра: system.skills.dex. Для импорта требуются foundry.data.fields и foundry.abstract.DataModel.

## Введённые сущности и действия с ними

Экспорт по умолчанию — класс Dexterity; локальный fields ссылается на API Foundry. Все поля таблицы — EmbeddedDataField([Skill](skillData.js.md), {label}). В колонках подписи приведены полные ключи; ключ skillMap и множитель стоимости проверены по config.js, но не задаются этой моделью.

| Ключ в system.skills.dex | label поля группы | label после миграции | Ключ skillMap / costMultiplier |
| --- | --- | --- | --- |
| `archery` | `WITCHER.skills.archery.label` | `WITCHER.skills.archery.label` | `archery` / 1 |
| `athletics` | `WITCHER.skills.athletics.label` | `WITCHER.skills.athletics.label` | `athletics` / 1 |
| `crossbow` | `WITCHER.skills.crossbow.label` | `WITCHER.skills.crossbow.label` | `crossbow` / 1 |
| `sleight` | `WITCHER.skills.sleightOfHand.label` | `WITCHER.skills.sleightOfHand.label` | `sleight` / 1 |
| `stealth` | `WITCHER.skills.stealth.label` | `WITCHER.skills.stealth.label` | `stealth` / 1 |
...
- static defineSchema():6 создаёт 5 вложенных полей. Не принимает параметров, не вычисляет бросок и не вызывает Actor.update.
- static migrateData(source):17 перебирает ключи своей defineSchema(). Для каждого truthy source[skillName] присваивает label по шаблону WITCHER.skills.<ключ>.label, выполняет имеющиеся специальные замены и возвращает super.migrateData(source).
...
Для sleight миграция явно заменяет label на WITCHER.skills.sleightOfHand.label. Ключ данных остаётся sleight. Пути dex.athletics встречаются в JSON-эффектах и для activeEffectModifiers, и для value.
...
Dexterity задаёт 5 EmbeddedDataField(Skill) под группой dex; миграция меняет label только существующих записей, не уровень навыка. Сверены адреса подписей: sleight→sleightOfHand. Опция label внешнего поля не становится аргументом Skill.defineSchema; fresh/reconstructed различие проверено N01 для всех52 навыков. Поля и getter принадлежат вложенным Skill, формулы броска — внешним потребителям.
