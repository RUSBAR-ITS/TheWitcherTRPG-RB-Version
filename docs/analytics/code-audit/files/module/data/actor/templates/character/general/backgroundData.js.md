...
2026-09-11, `12055fee62f01c6de49967044aedef9d7cfe0632`. Полный HBS использует formGroup с systemField/value/enriched из CharacterData.enrichedText. Группа 09 отличила исходный HTML/@UUID от строки enrichHTML-фасада и подтвердила правильную передачу обоих значений. Реальный DOM-жизненный цикл редактора не проверен.
...
35 подтаблиц создания персонажа возвращают тексты о семье, происхождении, профессии и братьях/сёстрах. В фабрике background остаётся один HTMLField value; кода автоматической записи RollTable в него нет. tab-background.hbs:51–53 показывает общий редактор биографии. Генераторы могут дать материал для ввода пользователем; собственных структурированных полей семьи или функции чтения текста таблицы эта фабрика не вводит.
...
Witcher Background Generator и таблицы witcher-lifepath возвращают описания для сообщения; поле background.value остаётся независимым HTMLField. В проверенных коде/шаблонах нет прямого вызова по пакету, именам или ID этих 41 таблиц и автоматического переноса результата в Actor. При прямом генераторе получено шесть текстов, через RandomCharacter — семь; это не запись биографии.
...
13 основных генераторов возвращают тексты и ссылки через RollTable, но не записывают background.value. RandomCharacter выбирает расовую ветвь, а не создаёт Actor. Его имя/description не доказывают заполнения этого HTMLField. Прямые текстовые потребители по ID/имени/пакету в module/templates/utils не найдены; сохранение биографии остаётся отдельным действием.
...
HTMLField.value попадает в system.general.background.value через general() и CharacterData. enrichedText передаёт то же значение createEnrichedText, который возвращает HTML и поле схемы для formInput. Исходный текст не заменяется обогащённым; фактическую запись делает форма. Фабрика не связана с числовыми lifepathModifiers.

Сопоставленные определения и потребители: [module/data/actor/templates/character/generalData.js](../generalData.js.md), [module/data/actor/characterData.js](../../../characterData.js.md), [module/data/dataUtils.js](../../../../dataUtils.js.md), [templates/partials/character/tab-background.hbs](../../../../../../templates/partials/character/tab-background.hbs.md).
