...
В `calculateStat` результат округляется вниз; собственного ограничения 1–10 в этой функции нет. Отдельные ограничения, изменение max эффектами и поздние этапы Actor разобраны в TASK-0003.007/009; они описаны в карточках WitcherActor и ActiveEffect. Подмена смысла всех полей одним термином «характеристика с потолком» была бы неверной.
...
- [issue-00012](../../../../../../../../../issues/closed/issue-00012.md) — два прохода calculateStats повторно прибавляют модификаторы luck/toxicity.max; проблема в потребителе.
...
На `411ab4004a2378a5f96ceeb19f003f834ff9d3d9` повторно сопоставлены схема, prepareBaseData, migrateData и расчёты Actor. M01–M02 подтвердили типы/defaults и различие отсутствующей/нулевой базы. Уточнена устаревшая ссылка на будущие .007/.009; исходные формулы и ограничения не менялись. [Протокол](../../../../../../../cross-check-0001.md).
...
Десять SchemaField: восемь характеристик, luck, toxicity; prepareBaseData копирует базы в max, а migrateData переносит max только при unmodifiedMax==0. В основном Actor-процессе копирование выполняет CommonActorData; собственный метод вложенной Stats не является рекурсивным этапом Foundry. Два calculateStats дают двойной вклад luck/toxicity, но reset модели восстанавливает исходный срез, что проверено N01.

Сопоставленные определения и потребители: [module/data/actor/templates/common/stats/statData.js](statData.js.md), [module/data/actor/commonActorData.js](../../../commonActorData.js.md), [module/actor/witcherActor.js](../../../../../actor/witcherActor.js.md).
