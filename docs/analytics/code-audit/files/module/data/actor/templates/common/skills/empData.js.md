# module/data/actor/templates/common/skills/empData.js

| Поле | Значение |
| --- | --- |
| Исходный файл | [module/data/actor/templates/common/skills/empData.js](../../../../../../../../../../module/data/actor/templates/common/skills/empData.js) |
...
Определяет Empathy — группу EMP из 10 базовых навыков Actor и миграцию их ключей подписи.

## Условия использования

[skills()](../../../../../../../../../../module/data/actor/templates/common/skills/skillsData.js) создаёт EmbeddedDataField(Empathy) по ключу emp; [CommonActorData.defineSchema](../../../../../../../../../../module/data/actor/commonActorData.js) включает результат в SchemaField skills. Путь экземпляра: system.skills.emp. Для импорта требуются foundry.data.fields и foundry.abstract.DataModel.

## Введённые сущности и действия с ними

Экспорт по умолчанию — класс Empathy; локальный fields ссылается на API Foundry. Все поля таблицы — EmbeddedDataField([Skill](skillData.js.md), {label}). В колонках подписи приведены полные ключи; ключ skillMap и множитель стоимости проверены по config.js, но не задаются этой моделью.

| Ключ в system.skills.emp | label поля группы | label после миграции | Ключ skillMap / costMultiplier |
| --- | --- | --- | --- |
| `charisma` | `WITCHER.skills.charisma.label` | `WITCHER.skills.charisma.label` | `charisma` / 1 |
| `deceit` | `WITCHER.skills.deceit.label` | `WITCHER.skills.deceit.label` | `deceit` / 1 |
| `finearts` | `WITCHER.skills.fineArts.label` | `WITCHER.skills.fineArts.label` | `finearts` / 1 |
| `gambling` | `WITCHER.skills.gambling.label` | `WITCHER.skills.gambling.label` | `gambling` / 1 |
| `grooming` | `WITCHER.skills.groomingAndStyle.label` | `WITCHER.skills.groomingAndStyle.label` | `grooming` / 1 |
| `perception` | `WITCHER.skills.humanPerception.label` | `WITCHER.skills.humanPerception.label` | `perception` / 1 |
| `leadership` | `WITCHER.skills.leadership.label` | `WITCHER.skills.leadership.label` | `leadership` / 1 |
| `persuasion` | `WITCHER.skills.persuasion.label` | `WITCHER.skills.persuasion.label` | `persuasion` / 1 |
| `performance` | `WITCHER.skills.performance.label` | `WITCHER.skills.performance.label` | `performance` / 1 |
| `seduction` | `WITCHER.skills.seduction.label` | `WITCHER.skills.seduction.label` | `seduction` / 1 |
...
- static defineSchema():6 создаёт 10 вложенных полей. Не принимает параметров, не вычисляет бросок и не вызывает Actor.update.
- static migrateData(source):22 перебирает ключи своей defineSchema(). Для каждого truthy source[skillName] присваивает label по шаблону WITCHER.skills.<ключ>.label, выполняет имеющиеся специальные замены и возвращает super.migrateData(source).
...
Миграция переопределяет подписи finearts, grooming и perception; ключ perception означает Human Perception, а int.awareness — другой навык. В statusEffects из config.js есть прямые пути к activeEffectModifiers навыков EMP.
...
Динамические потребители всех навыков перечислены с файлами и местами обращения в [карточке Skill](skillData.js.md): листы и edit-skills читают/изменяют поля system.skills.emp.<навык>; rollSkillCheck читает value, addActiveEffects отдельно читает activeEffectModifiers; мастер и подсказки эффектов конструируют пути по skillMap; настройки монстра адресуют isVisible. Это связи по структуре данных, не импорты Empathy.

Прямые строки путей activeEffectModifiers найдены в statusEffects из [module/setup/config.js](../../../../../../../../../../module/setup/config.js):2247–2292; обработчики вербального боя читают value через выбранную запись skillMap.
...
Empathy задаёт 10 EmbeddedDataField(Skill) под группой emp; миграция меняет label только существующих записей, не уровень навыка. Сверены адреса подписей: finearts→fineArts, grooming→groomingAndStyle, perception→humanPerception. Опция label внешнего поля не становится аргументом Skill.defineSchema; fresh/reconstructed различие проверено N01 для всех52 навыков. Поля и getter принадлежат вложенным Skill, формулы броска — внешним потребителям.
