...
2026-09-18, TASK-0010.005. **Назначение:** Общая модель персонажа и монстра; исходные базы подготовки.

**Методы, сущности, действия и зависимости:** prepareBaseData восстанавливает primary max, затем использует derivedStatData. Автоматические базы считаются из unmodifiedMax входов; customStat сохраняет ручные HP/STA/Resolve/Focus. Vigor/Shield всегда ручные. max и value фиксированных производных инициализируются перед initial; текущие запасы не перезаписываются. Это начальные значения для нативной фазы, окончательная формула с дробной базой и эффектами — в derivedPreparation. Остальная схема/валюта/миграции не изменены.

[Исходник](../../../../../../../module/data/actor/commonActorData.js), [проверки и границы](../../../../../task-0010-005-checks.md). Браузерная приёмка отложена. Ниже, если есть, сохранены датированные предыдущие срезы; изменённые расчёты описаны здесь.
