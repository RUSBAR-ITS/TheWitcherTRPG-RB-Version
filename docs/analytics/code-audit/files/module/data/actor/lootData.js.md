...
Новых проблем непосредственно в LootData при этом разборе не зарегистрировано. [issue-00020](../../../../../../issues/potential/issue-00020.md) относится к обмену валют в общем потребителе, а [issue-00005](../../../../../../issues/closed/issue-00005.md) — к другим типам: loot в манифесте присутствует. Это не вывод об исправности всех операций с добычей.
...
Полный Actor-конвертер успешно выполнен на LootData с currency: схема совместима (группа05). Однако WitcherLootSheet не присоединяет примесь листа и текущий HBS не имеет .open-currency-converter, поэтому наличие метода Actor не означает доступную кнопку. Покупка .035 использует собственную цену, не currencyRates; награды .037 остаются отдельным процессом.
...
Наличие currency позволяет обмену .036 работать на LootData, но награды .037 требуют logs. В реальной LootData logs отсутствует, а player-owned Actor отбор getPlayerActors не исключает. Оба handout дают TypeError (группа14). Это не ошибка схемы валюты и не основание автоматически добавлять журнал Loot.
