...
2026-09-19. consume сохраняет прежние лечение и независимые self/removes статусы, затем await createConsumeMessage и createItemEffectDelivery(applySelf). createConsumeMessage возвращает Promise ChatMessage.create. Карточка доставки содержит только AE предмета, повтор не запускает consume, лечение, статусы, списание или токсичность. Списание/токсичность внешних callers и прежние не-await записи этой порцией не перерабатывались.
...
2026-09-18. **Назначение:** Использование расходуемого предмета.

**Методы, сущности, действия и зависимости:** consume ожидает self-AE перед сообщением об использовании. Лечение/статусы и дозы/токсичность не переустраивались; повтор семейства проходит через общий delivery.
