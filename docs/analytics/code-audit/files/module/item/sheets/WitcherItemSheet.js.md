...
Добавлен _processFormData: вызывает родительский ItemSheetV2, проверяет только присутствующие quantity/lootQuantityFormula через импорты из module/data/item/commonItemData.js. Для количества использует исходную строку DOM-input, чтобы не принять результат приведения пустого ввода в FormData. Ошибка останавливает submit; настоящая запись остаётся за Foundry. CommonItemData задаёт числовую схему, item-header.hbs показывает отдельную формулу. submitOnChange/closeOnSubmit не менялись.
...
Новый прямой наследник WitcherMountSheet задаёт width600/PARTS.main mount-sheet.hbs, получает configuration, item/data/config/showConfig/systemFields и handlers из общего класса. Тест19 создал этот дочерний класс и проверил контекст;20 проверил поля формы с настоящими MountData/FormDataExtended. В отличие от note-sheet, mount имеет активный PARTS. Реальное Application-окно/merge конструктора ядра не инстанцировалось.
