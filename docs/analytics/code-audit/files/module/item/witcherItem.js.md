...
createDocuments передаёт assignedItemData опцию preserveEffectStart только при operation[WOUND_INTERNAL] === true. Внутренняя запись травмы сохраняет уже подготовленные часы; обычный импорт и assignedSkillIds, контейнерная маршрутизация, withParameterChanges и updateDocuments прежние.
...
2026-09-20, dev. createDocuments перед существующей нормализацией предметов вызывает assignedSkillIds. Новому навыку/слотам назначается ID один раз, копии уже идентифицированных предметов сохраняют ID. Контейнеры и UUID экземпляров не подменяются.

Связанные потребители/границы: skillIdentity. Статические проверки выполнены; браузер и БД не запускались. [Исходник](../../../../../../module/item/witcherItem.js) · [TASK-0012](../../../../../tasks/task-0012-temporary-hp.md) · [Проверки](../../../../task-0012-checks.md).
...
checkIfItemHasRollTable принимает целое количество повторений ≥0, проверяя его до получения документа таблицы/бросков/записей. Ноль означает0 бросков. Для нового Item из результата таблицы очищает lootQuantityFormula при наличии поля в модели; количество шаблона сохраняется и формула повторно не бросается. Объединение с имеющейся стопкой (+1), удаление генератора и границы отказа сохраняются. Источник поля — CommonItemData; таблицы и материализация используются прежними путями.
...
2026-09-18. **Назначение:** Item и вложенные источники эффектов.

**Методы, сущности, действия и зависимости:** createDocuments/updateDocuments используют assignedItemData перед существующими маршрутами контейнеров/травм и внутри withParameterChanges. Импорт собственного Item назначает порядок и часы вложенным AE; повторная активация обновляет порядок. Остальные контейнерные/травматические алгоритмы сохранены.
...
2026-09-18, TASK-0010.006. **Назначение:** Item; существующие операции контейнеров/травм и граница смены параметров владельца.

**Методы, сущности, действия и зависимости:** createDocuments/updateDocuments/deleteDocuments целиком обёрнуты в parameterPersistence.withParameterChanges(parameterActor(operation.parent), action). Сохранены внутренние guards, очереди травм, импорт/удаление контейнеров; обрезание базы/ресурсов выполняется после результата всей операции. toCompendium/clone и прочие действия не менялись.

[Исходник](../../../../../../module/item/witcherItem.js), [проверки/границы](../../../../task-0010-006-checks.md). Ниже, если есть, сохранены датированные предыдущие срезы; изменённые операции описаны здесь.

## Актуализация 2026-09-17 — 14.3.1.00049

**Назначение:** Нативные входы Item с защитой ключей травм.

**Методы и действия:** createDocuments делегирует criticalWound Actor в createWoundDocuments до прежней контейнерной фабрики. updateDocuments проверяет итоговые ключи партии в очереди. WOUND_INTERNAL отключает повторный вход, но не права/hooks. Прочие методы/контейнерные пути сохранены.

**Непосредственные зависимости/потребители:** [module/item/criticalWoundOperations.js](../../../../../../module/item/criticalWoundOperations.js), [module/item/containerOperations.js](../../../../../../module/item/containerOperations.js), [module/item/containerTemplates.js](../../../../../../module/item/containerTemplates.js). Foundry Document/Roll/Collection/UI — внешние API.
...
**Назначение:** Документ Item, включая жизненный цикл контейнера.

**Основные методы, сущности и действия:** Добавлены createDocuments/deleteDocuments для materialization/cascade через containerOperations; внутренний Symbol предотвращает повторный вход, не обходя native permission/hooks. toCompendium сериализует контейнер; clone(...,{save:true}) обслуживает существующую команду Duplicate самостоятельной копией дерева. deleteDialog для наполненного контейнера использует одно подтверждение удаления дерева; пустой Item делегируется ядру. Остальные методы сохранены.

**Зависимости и потребители:** containerTemplates.js: CONTAINER_INTERNAL/serializeContainer/contentOf; containerOperations.js: createContainerDocuments/deleteContainerDocuments; базовый Item и его обычные create/delete hooks. Удаление Actor целиком остаётся операцией ядра над Actor, без запуска отдельного диалога на каждый дочерний Item.
...
М05; [реализация и пределы проверок](../../../../../issues/closed/issue-00332.md).

Новый getLootRollTables возвращает все одноимённые RollTable из индексов библиотек, включая дубли в одном pack. checkIfItemHasRollTable возвращает not-found/generated/failed/partial и список выполненных записей. До записей разрешает каждый результат через documentUuid/fromUuid, проверяет Item и готовит getHTML. Затем последовательно ждёт create/update, удаляет генератор и отправляет сообщения. Пустая/не-Item/неоднозначная выдача не превращается в обычную добычу; ошибки содержат причину и состояние generatorRemoved. Если ошибка случилась уже при отправке чата, генератор может быть удалён: откат и автоматический повтор не заявлены. При создании сохраняется количество исходного Item, при совпадении name/type стопка растёт на 1 за результат.

Непосредственные зависимости и потребители: [module/actor/sheets/WitcherMonsterSheet.js](../../../../../../module/actor/sheets/WitcherMonsterSheet.js), [module/rollTable/witcherRollTable.js](../../../../../../module/rollTable/witcherRollTable.js), [lang/ru.json](../../../../../../lang/ru.json), [lang/en.json](../../../../../../lang/en.json).
...
Исходник и текущие связи сопоставлены в TASK-0004.006. Прежние ссылки на будущий пофайловый разбор TASK-0003 больше не являются очередью: он завершён. Остались конкретные границы сквозной проверки: [U006-02](../../../cross-check-0002.md#u006-02); [U006-03](../../../cross-check-0002.md#u006-03); [U006-04](../../../cross-check-0002.md#u006-04); [U006-08](../../../cross-check-0002.md#u006-08). Craft/генератор имеют разные потерянные ожидания; старые collection/id таблицы не гарантируют результат Item.
...
Поиск компедиумов RollTable по точному совпадению Item.name технически может выбрать и таблицу style: специального ограничения на генераторы добычи нет. Настоящий checkIfItemHasRollTable отдельно вызван для имён всех семи реальных таблиц style с реальными текстовыми TableResult. Каждый случай дошёл до exportLootInvalidItemError: текст не ссылается на Item. Контроль с несовпадающим именем вернул false. Индекс/pack, уведомления и persistence подменены; наличия таких Item в действующем мире не утверждается. Это установленная граница потребителя, не новый дефект style; issue-00039 не дублировалась.
...
Повторно прочитан checkIfItemHasRollTable: поиск по Item.name может выбрать любой из 21 RollTable lifepath. Реальные рекурсивные roll всего набора дали только text, иногда массивы из пяти/семи записей или пары заголовок/текст. Метод берёт results[0] и пытается разрешить Item; этот формат не предназначен для автоматической выдачи добычи. Вывод о ветке exportLootInvalidItemError сделан из кода и формы результатов; метод с этими 21 именами повторно не запускался, выполненные ранее проверки .052 не выдаются за новый прогон. Issue-00039 остаётся отдельным наблюдением о предположениях потребителя.
...
checkIfItemHasRollTable исполнен для всех 11 точных имён Combat с настоящими RollTable/TableResult. Десять текстовых результатов дали exportLootInvalidItemError, Mounted Control Loss с неразрешимыми UUID дала [] и TypeError documentCollection (issue-00039/00323). Pack/index, уведомления и запись — фасады; наличие таких Item в мире не подтверждено.
...
WitcherItem соединяет системную модель и пять примесей. Разделены getItemAttack, realCraft, RollTable-генератор и применение перенесённых активных улучшений. Последнее использует собственный проход без actor phase-фильтра, сверенный в .005. Craft/генератор имеют разные потерянные ожидания; старые collection/id таблицы не гарантируют результат Item.

Сопоставленные определения и потребители: [module/data/item/commonItemData.js](../data/item/commonItemData.js.md), [module/setup/registerDataModels.js](../setup/registerDataModels.js.md), [module/setup/registerSheets.js](../setup/registerSheets.js.md), [module/TheWitcherTRPG.js](../TheWitcherTRPG.js.md).

[Протокол и границы](../../../review-log.md#task-0004006) — TASK-0004.006; процессы [R006-01](../../../cross-check-0002.md#r006-01), [R006-05](../../../cross-check-0002.md#r006-05), [R006-12](../../../cross-check-0002.md#r006-12), [R006-21](../../../cross-check-0002.md#r006-21), [R006-22](../../../cross-check-0002.md#r006-22). В этой порции выполнена статическая сверка; поведенческие опыты принадлежат датированным прежним протоколам, а не новому прогону.
