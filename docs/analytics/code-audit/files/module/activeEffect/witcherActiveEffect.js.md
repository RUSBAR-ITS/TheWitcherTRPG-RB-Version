...
**Методы, сущности, действия и зависимости:** Native create/update обёрнуты в effectFamilies внутри withParameterChanges; delete сохраняет общий пересчёт. isSourceSuppressed учитывает core expired, активность родителя и триггеры. isSuppressed добавляет подавление семейства. isExpiryTrackable сохраняет отсчёт подавленного источника. _preCreate использует initializeEffectStart с Actor владельца Item и сообщает ядру уже заполненный start; выбор @skill/фазы сохранён.
...
2026-09-18, TASK-0010.006. **Назначение:** ActiveEffect; собственные численные маршруты и нормализация владельца после записи.

**Методы, сущности, действия и зависимости:** Три статических native входа createDocuments/updateDocuments/deleteDocuments ожидают withParameterChanges владельца. Это охватывает создание/изменение/отключение/удаление, в том числе через embedded API; Item-родитель разрешается до Actor. Настройки стадий, выбор @skill, suppression и численная маршрутизация .003/.004 сохранены. Зависимость — parameterPersistence.

[Исходник](../../../../../../module/activeEffect/witcherActiveEffect.js), [проверки/границы](../../../../task-0010-006-checks.md). Ниже, если есть, сохранены датированные предыдущие срезы; изменённые операции описаны здесь.

## Текущее состояние — 14.3.1.00061

2026-09-18, TASK-0010.003. **Назначение:** Документ эффекта: безопасное обновление общей фазы.

**Сущности, действия и зависимости:** _preUpdate принимает nested/flat system updates, отличает отсутствие bool от false; только явная смена общего флага переводит все строки. При обычном submit сохраняет явно заданные phase, при отсутствии заполняет действующей общей фазой. source-массив не мутируется. Остальные методы, suppression и зависимости прежние.

[Исходник](../../../../../../module/activeEffect/witcherActiveEffect.js), [проверки и границы](../../../../task-0010-003-checks.md). Статическая проверка; игровая приёмка не проводилась. Численный контракт следующих стадий ещё не внедрён.
...
[issue-00043](../../../../../issues/closed/issue-00043.md) — частичные обновления и фазы. [issue-00050](../../../../../issues/closed/issue-00050.md) — start и контекст владельца временного улучшения. Связанный маршрут копирования: [issue-00044](../../../../../issues/closed/issue-00044.md). Все карточки остаются potential; код не исправлялся.
...
Настоящий класс применён к 17 эффектам Simple: они активны и нацелены на Actor-владельца; origin не задаёт цель применения. Проверены 55 числовых изменений, контрольные disabled/transfer/applySelf. Core prepareBaseData задаёт 20 для 51 null-приоритета, четыре 0 сохраняет. Исполнение полного клиентского lifecycle и таймера не входит в этот сценарий.
