...
2026-09-20, dev. temporaryHpSum берётся из temporaryHpTotal(actor), как в новом листе и обработчике урона. Остальной контекст не меняется.

Связанные потребители/границы: temporaryHp. Статические проверки выполнены; браузер и БД не запускались. [Исходник](../../../../../../../module/actor/sheets/WitcherActorSheetV1.js) · [TASK-0012](../../../../../../tasks/task-0012-temporary-hp.md) · [Проверки](../../../../../task-0012-checks.md).
...
Оба callbacks _onRecoverSta ожидают Actor.update. V1 остаётся не подключённым штатным листом; формулы и пределы этой реализацией не меняются.
...
Общий prepareActiveEffectCategories теперь исключает повтор полного UUID/ссылки до прежней категоризации; собственный getData V1 не менялся. См. [issue-00334](../../../../../../issues/closed/issue-00334.md). Статическая сверка дополнена приёмкой .00022 в связанной issue-00334. Для V1 проверен прямой вызов общей примеси; регистрации и открытия старого листа не было.
