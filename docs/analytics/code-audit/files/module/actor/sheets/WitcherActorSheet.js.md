...
2026-09-20, dev. temporaryHpSum берётся из temporaryHpTotal(actor): тот же набор активных источников, что расходуется уроном, включая ручные сроки. Остальная компоновка не меняется.

Связанные потребители/границы: temporaryHp. Статические проверки выполнены; браузер и БД не запускались. [Исходник](../../../../../../../module/actor/sheets/WitcherActorSheet.js) · [TASK-0012](../../../../../../tasks/task-0012-temporary-hp.md) · [Проверки](../../../../../task-0012-checks.md).
...
2026-09-19: Recovery Action записывает min(действующего STA.max, STA.value+REC.value) при value<max; оба callback по-прежнему ожидают Actor.update. Полный/превышенный запас не меняется, Full Recovery сохраняет прежнее поведение. Данные читаются при подтверждении. Формулы модели не менялись.
...
Только callbacks восстановления STA в _onRecoverSta теперь ожидают Actor.update: восстановление по REC и полное. Render диалога не означает ожидания выбора кнопки. Формулы и прежний guard STA>=max сохранены, итоговое ограничение oldSTA+REC не добавлено.

Непосредственные зависимости и потребители: [module/actor/witcherActor.js](../../../../../../../module/actor/witcherActor.js).
...
Базовый V2-лист Actor: маршрут открытия конвертера. Изменение 2026-09-17 по [issue-00334](../../../../../../issues/closed/issue-00334.md); основание — исходники, а не новая браузерная приёмка.
...
Сверены код, маршруты графа и адресные статические проверки. Отложенная приёмка в работающем Foundry .00022 выполнена 2026-09-17 после команды пользователя; результаты соответствующего маршрута и ограничения приведены в связанной issue-00334, раздел «Браузерная приёмка». Датированные результаты ниже относятся к прежним срезам; прежние утверждения об изменённых методах заменены контрактом этой секции.
...
Прежняя оговорка о totalCost теперь проверена полным разбором LootSheet: он непосредственно extends ActorSheetV2 и не получает _prepareItems/totalCost этого класса. Поэтому totalCost span в loot пуст (новый issue219); это не пропуск расчёта в данном общем листе. Подготовка enhancementItems также не наследуется Loot: applied там только фильтруется, прежний issue166 не переносится. В рамках общей сверки .031–.035 сохранены разные ветви Character/Monster/Loot.
...
Полностью описана module/actor/sheets/mixins/currencyConverterMixin.js: import8, Object.assign321, вызов244 внутри activateListeners, который запускается _onRender213–217. В каждом вызове новый bind Actor для всех .open-currency-converter; повтор на сохранённом узле удваивает click (issue230). Частичный render ядра может сохранить не обновлённые части; полный render с новой кнопкой не объявлен дублирующим. Сам exchange-method находится в Actor-примеси по другому пути.
...
Полные профессиональные HBS используют унаследованные itemListeners/skillListener. Базовый context даёт config/system/editable; profession/race выбирает специализация. Не смешивать безымянные data-field inline Item с автоматической формой Actor. Rich-text сохранение вложенных Item в браузере в .038 не исполнялось.
...
_prepareSpells собирает spell по class Spells/Invocations/Witcher и level novice/journeyman/master, MagicalGift отдельно, hex/ritual по типу. getList исключает isStored и сортирует sort; learned не проверяет. Группа 03 сверила полный контекст tab-magic/spell-type-list; activateListeners подключает itemListener и itemContextMenu.
