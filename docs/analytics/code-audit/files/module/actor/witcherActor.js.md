...
2026-09-18, TASK-0010.006. **Назначение:** Actor; численный расчёт и предварительная нормализация записи.

**Методы, сущности, действия и зависимости:** _initialize сохраняет несохраняемый parameterPreview до обычной инициализации; в preview пропущен только applyStatus брони. updateDocuments рассчитывает итоговые поправки вместе с исходным payload до единственной native записи Actor; для вложенной операции и PARAMETER_INTERNAL обрезание завершает внешний владелец операции. World Actor и Actor от operation.parent.actor поддержаны; pack делегируется без нормализации. Расчёты .004/.005 сохранены. Новый импорт — parameterPersistence. Native updateDocuments/clone и коллекция game.actors — внешние границы.

[Исходник](../../../../../../module/actor/witcherActor.js), [проверки/границы](../../../../task-0010-006-checks.md). Ниже, если есть, сохранены датированные предыдущие срезы; изменённые операции описаны здесь.
...
2026-09-10, `rusbar-main`, `c26eb64dd54cc434087f54c3c6b678b6092b15a2`. professionMixin смешан в prototype строкой 438. Через него используются 10 навыков первого getList('profession')[0]; сумма уровней и выбор навыка по имени отделены от списка обычных professionSkills. itemMixin при Drop сбрасывает/ставит isProfession, не повышая уровни. Проверены выбор вида использования и граница временных HP до query; полный бой/листы/расход не исполнялись. Одноимённый definingSkill выбирается раньше путей (issue-00110).
...
| --- | --- | --- |
| healMixin | [module/actor/mixins/healMixin.js](../../../../../../module/actor/mixins/healMixin.js) | import 14/Object.assign 444: методы расчёта и чата принадлежат Actor. calculateHealValue не пишет HP; createHealMessage ошибочно берёт this.actor для speaker (issue-00126). |
...
Полностью описана module/actor/mixins/currencyConverterMixin.js: import17/Object.assign452 добавляют handleCurrencyConverter/getCurrencyRates/openCurrencyConverter. Actor event adapter возвращает Promise; процесс ждёт один update двух валютных путей, затем готовит чат, но не ждёт ChatMessage.create. Это отличается от покупательского caller .035. Ошибка from===to уточнена в issue20; новые 227–229/231 описывают валидацию, свежесть баланса и сообщение.
...
useItem(itemId,options) возвращает castSpell для spell/hex/ritual и пропускает неизвестный ID. В castSpell опции клавиш не передаются getItemAttack; импорт 5/Object.assign445 вводит также calcStaminaMulti. Группы 01–02/13 сверили действительный возврат Roll и оба itemUuid.

[module/actor/mixins/castSpellMixin.js](../../../../../../module/actor/mixins/castSpellMixin.js) — [карточка](mixins/castSpellMixin.js.md); [templates/partials/character/tab-magic.hbs](../../../../../../templates/partials/character/tab-magic.hbs) — [карточка](../../templates/partials/character/tab-magic.hbs.md); [templates/sheets/actor/partials/character/spell-type-list.hbs](../../../../../../templates/sheets/actor/partials/character/spell-type-list.hbs) — [карточка](../../templates/sheets/actor/partials/character/spell-type-list.hbs.md).
...
2026-09-11, `rusbar-main`, `74322e91edac106c82668f4a47eef53ce1889dc1`; исходники прежние. Схемы сообщений отделены от Actor: чтение attackRoll/crit/damage и кнопочные обновления выполняют внешние обработчики. combat.js передаёт prepared crit в Actor.applyCritWound; реальная функция проверена на различии сырого modifier+6 и очищенного DefenseMessageData (группа11). Чат лечения пишет derivedStats.hp.value непосредственно, не вызывает общий Actor.heal; выбор цели зависит от targets/controlled/character.
