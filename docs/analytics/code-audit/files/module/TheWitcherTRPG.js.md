...
2026-09-18, TASK-0010.009. **Назначение:** Инициализация документов и обработки истечения.

**Методы, сущности, действия и зависимости:** init регистрирует registerEffectExpiry после настройки класса ActiveEffect и expiryAction=delete. Сам реестр ядра и его зарегистрированные документы сохраняются; оборачивается его существующий refresh. Остальные регистрации и world ready не менялись.

[Проверки и границы](../../../task-0010-009-checks.md). Реальная игровая приёмка предстоит в .013.

## Актуализация 2026-09-17 — 14.3.1.00049

**Назначение:** Регистрация и ready системы.

**Методы и действия:** В ready preload индекса травм дополнен system.woundTypeId; ошибки загрузки обработаны прежним catch. Получение также перечитывает настроенный pack через selectInitialWound.
...
**Назначение:** Инициализация и регистрация системы.

**Основные методы, сущности и действия:** Добавлены import WitcherItems и CONFIG.Item.collection = WitcherItems рядом с documentClass. Порядок прочих регистраций сохранён.

**Зависимости и потребители:** module/item/collections/WitcherItems.js — мировая коллекция; module/item/witcherItem.js — документ. Остальные ранее описанные зависимости сохранены.
...
М01, М03; [реализация и пределы проверок](../../../../issues/closed/issue-00332.md).

init регистрирует CONFIG.RollTable.documentClass = WitcherRollTable. В ready проверяются выбранная библиотека и тип Item; getIndex ожидается внутри try/catch. Отсутствие, неверный тип или отказ индекса сопровождаются ru/en-уведомлением, после чего продолжаются hotbarDrop, шрифт и сокет. Пустой успешно загруженный индекс не считается ошибкой. Автоматическая замена выбранного pack не добавлена.

Непосредственные зависимости и потребители: [module/rollTable/witcherRollTable.js](../../../../../module/rollTable/witcherRollTable.js), [module/setup/settings.js](../../../../../module/setup/settings.js), [lang/ru.json](../../../../../lang/ru.json), [lang/en.json](../../../../../lang/en.json).
...
2026-09-16, dev, база aeb527c6e6402e8a5f940a2c3ad0020d7e5b35e4. createMacro выводит WITCHER.Macro.ownedItemsOnly. FictionalGameSystemLanguageProvider.i18nInit локализует labels common/dwarven/elder через отдельные WITCHER.languages.* после загрузки словарей и возвращает super.i18nInit(). Конструктор на polyglot.init не вызывает перевод до загрузки словарей. IDs, шрифты, getUserLanguages и условия доступа сохранены; Polyglot локально не установлен.
...
[issue-00002](../../../../issues/closed/issue-00002.md) — отсутствие выбранного компедиума прерывает оставшуюся инициализацию ready.
...
ready:77–87 — фактический потребитель .witcher-style из [входного CSS](../styles/witcher-styles.css.md). C02 повторно исполнил исходный callback с FS/DOM не связанными с миром: useWitcherFont=true и успешный getIndex добавили класс на .game и #chat-log; false — ни одного; отсутствующий pack остановил callback до этого шага (issue-00002). Настройка не подключает CSS динамически: все стили объявлены в system.json независимо от неё.

Общая сверка серии сопоставила регистрации чата с полностью описанными Combat/ApplyDamage/VerbalCombat и реальными получателями SocketMessage. Повторены изолированные проверки .045/.046: замена DOM/jQuery и завершение записей остаются документированными границами. Сокет используется ремонтом/передачей Item; его нельзя автоматически вставлять в каждый боевой маршрут. Это уточнение связей, не запуск init/мира целиком.
