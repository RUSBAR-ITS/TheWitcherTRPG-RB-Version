...
2026-09-16, dev, база aeb527c6e6402e8a5f940a2c3ad0020d7e5b35e4. WITCHER.diagramTypeLabels содержит все 13 типов селекторов, включая potion/decoction/oil. Пустой type по-прежнему отдельная отложенная граница данных. Изменены только три соответствия подписей; типы и механики прежние.
...
Уточнение 2026-09-16: карта diagramTypeLabels покрывает 10 из 13 типов селекторов; отсутствуют potion/decoction/oil. Шаблон инвентаря обращается к карте и для этих формул. [Issue-00178](../../../../../issues/closed/issue-00178.md) остаётся частично исправленной; прежняя проверка десяти подписей не покрывала все типы.
...
2026-09-11, `rusbar-main`, `ce0c7eb7069b215b641d725913b3aae21502e811`. lookup внутри таблиц использует Availability/Concealment/weapon.hands/craftingLevels. Проверены все 4/4/4/5 значения этих словарей с настоящим HBS. Для рецепта отдельно строится TYPES.Item.<type>: десять подтипов не имеют ключей en. WITCHER.Weapon.Availability отсутствует в en/ru; данные самого словаря Availability корректно разрешаются по Everywhere/Common/Poor/Rare.
...
WITCHER.currency624–632 — источник семи options для Rewards.currencyRewardDialog и lookup в rewards/currency.hbs; currencyRates/currencyConverter здесь не читаются. В отличие от обмена, falsecoin доступен. Нативный default первого option — bizant; type в handout повторно не валидируется. Локализация options сверена с раскрытыми EN/RU.
