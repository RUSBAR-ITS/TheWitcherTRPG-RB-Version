...
registerQueries дополнительно регистрирует effectSource → readEffectSource и effectDelivery → receiveEffectDelivery из effectDelivery.js. Получатель источника только читает Item; получатель доставки проверяет локальное право Actor и применяет подготовленный пакет без пересылки. Оба возвращают обычные JSON-данные, не Document. Два прежних query сохранены: общий whitelist ожидает вызванный метод, но возвращает true независимо от его результата; его true нельзя считать подтверждением применения. Потребители нового протокола используют отдельные ключи; issue-00008 и остальные entity-functions не исправлялись.
...
2026-09-18. **Назначение:** Зарегистрированные запросы владельцу/GM.

**Методы, сущности, действия и зависимости:** Общий query ожидает разрешённые функции/методы; специализированный applyTemporaryItemImprovementsToActor передаёт duration и ожидает завершения. true возвращается после успешного завершения, исключение записи передаётся вызывающему. Whitelist/выбор владельца не переустраивались.
...
| --- | --- | --- |
| system | Локальная const, стр. 7 | Namespace TheWitcherTRPG-RB-Version |
| applyTemporaryItemImprovementsToActor | Локальная async function, стр. 9 | Обработчик TheWitcherTRPG-RB-Version.applyTemporaryItemImprovements |
| query | Локальная async function, стр. 15 | Обработчик TheWitcherTRPG-RB-Version.query |
...
2026-09-11, `9f16a7ae4bc942df85a105fd37d9a3cb3ef99f4b`: getActorOwner из helper не является подтверждением запроса: он только возвращает активного не-GM OWNER либо activeGM. Без обоих некоторые потребители обращаются к null.query до входа в этот маршрутизатор ([issue-00185](../../../../../issues/closed/issue-00185.md)). [issue-00008](../../../../../issues/potential/issue-00008.md) о return true без ожидания вызванной функции относится к следующему этапу процесса и остаётся актуальным; сам query в .028 повторно не исполнялся.
