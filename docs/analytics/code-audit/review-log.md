...
Полностью прочитаны 25 JSON Complex / 2058 строк: 24 Item criticalWound и Folder YcLLKtwU75uE8tdC. Проверены все root/system-поля, HTML, 16 ActiveEffect и 61 change, включая _key вложенных эффектов. Восемь Item не имеют effects: шесть Fractured Arm и две исходные формы Ruptured Spleen.

Восемь цепочек Broken Ribs, Fractured Arm Left/Right, Fractured Leg Left/Right, Lost Teeth, Minor Head Wound, Ruptured Spleen. Найдены 16 переходов none → stabilized → treated, восемь конечных null; нет циклов, несуществующих адресатов или выходов из Complex. Location сохраняется в каждой цепочке, в том числе спорный torso у Minor Head Wound. Все корневые _id/_key проверены среди 98 criticalWounds; повторяющиеся effect ID находятся у разных владельцев, _key указывает на своего Item.

### Исполненные методы и фасады

Через stdin выполнены Python-проверки структуры/документации и изолированный `node --input-type=module`; тестовый файл/стенд не создавался. Foundry 14.367.0 из /opt/foundryvtt, Node 24.16.0. Мир, HTTP, браузер, серверный индекс, БД, compile/extract и операции с правами не запускались.

Использованы настоящие BaseItem/BaseActor/BaseFolder/BaseActiveEffect из common/documents, DataModel/TypeDataModel и поля common/data/fields.mjs, ActiveEffectTypeDataModel из common/data/active-effect.mjs. Импортированы реальные [CriticalWoundData](../../../module/data/item/criticalWoundData.js), [WitcherActiveEffectData](../../../module/data/activeEffects/witcherActiveEffectData.js), [CharacterData](../../../module/data/actor/characterData.js) с CommonActorData/вложенными моделями и [WitcherActiveEffect](../../../module/activeEffect/witcherActiveEffect.js).

Клиентский ActiveEffect из /opt/foundryvtt/client/documents/active-effect.mjs загружен целиком с заменой импортов окружения. Исполнены prepareBaseData, active/target, shouldApplyChange, getReplacementData и applyChange через настоящий NumberField. Из client/documents/actor.mjs взяты точные allApplicableEffects/applyActiveEffects; из [WitcherActor](../../../module/actor/witcherActor.js) — calculateStats/calculateStat/calculateFixedDerivedStats/calculateDerivedStats/calculateDerivedStat/calculateWeigthEncumbrance и addItem. Исполнены точный applyCritWound из [damageMixin](../../../module/actor/mixins/damageMixin.js), реальные treat/heal и [modifierMixin.addActiveEffects](../../../module/actor/mixins/modifierMixin.js) с настоящим [CONFIG.WITCHER](../../../module/setup/config.js).

Фасады: ClientDocumentMixin, registry без таймера, Hooks/CONFIG/game/i18n/окружение загрузки. BaseItem с настоящей системной моделью вместо полного WitcherItem; его migrateData/migrateSpells сопоставлены статически, Hexes/Rituals к Complex не относятся. getArmorEcumbrance/getTotalWeight возвращают 0. fromUuid читает Map настоящих Item, индекс — массив их очищенных system/uuid, addItem при выборе и ChatMessage перехватываются. Для treat/heal create/update/delete возвращают незавершаемые Promise, реальной записи нет. В проверке подписи appliedEffects задан из настоящих активных эффектов Actor, displayRollsDetails=true.

Подготовка вызывается явно: CommonActorData.prepareBaseData, подготовка Item и эффекта, initial → calculateStats → calculateFixedDerivedStats → calculateStats → calculateDerivedStats → final. Полный lifecycle клиента, _preCreate/_preUpdate, calculateAttackStats, updateDuration, registry и многоклиентская работа не входят в доказанный результат.

### Результаты — 772 утверждения

| Группа | Проверка | Результат |
| --- | --- | --- |
| 01 | Все 24 Item и Folder, strict=true | Модели создаются; все lesserEffect явно заданы и сохраняются, location не исправляется очисткой, system.htmlFields удалён |
| 02 | 16 эффектов / 61 изменение | Legacy changes → system.changes; mode=2 → add, строковые величины → числа, phase=initial, applyAfterCalculations=false |
| 03 | Приоритеты и срок | Все 61 priority=null получают 20 при prepareBaseData; duration очищается в value=null/units=seconds/expiry=null/expired=false, start=null; prepared value=Infinity |
| 04 | Все целевые поля | NumberField по каждому из 61 ключей; initial даёт соответствующее числовое значение, final не применяет initial changes повторно |
| 05 | Broken Ribs, база 5 | none: BODY3/REF4/DEX4, ENC30, HP.max20; stabilized: BODY4/REF4/DEX5, ENC40, HP.max20; treated: BODY4/REF5/DEX5, ENC40, HP.max20 |
| 06 | Minor Head Wound и Ruptured Spleen | Minor none: INT4/WILL4, STUN3, REC4, HP.max20, RESOLVE.max20/FOCUS.max12. Stabilized STUN4; treated INT5/WILL4, STUN4. Treated Spleen: STUN3 при базовых 5 и модификаторе -2 |
| 07 | Отдельные ноги | Левая SPD:2 → 3 → 4, правая:2 → 3 → 2. Соответствующие dodge/athletics: слева -3/-2/-1, справа -3/-2/-3. Это реальные данные, оценка правильности чисел не выполнялась |
| 08 | Две исходные ноги и исключение источника | Вместе SPD=-1, RUN=-3, LEAP=-1, dodge=-6. Отдельные disabled=true, transfer=false, applySelf=true либо новый Actor без Item дают SPD5. Минимум/правило ограничения не добавлялись |
| 09 | Все 24 treat | 16 непустых followUp загружают настоящий следующий Item: create → delete; восемь null дают только delete. treat завершён при pending вложенных записях |
| 10 | Семь heal на Lost Teeth при healingTime7 | С нуля treated даёт дни1; новая стерилизация дни3/sterilized=true; ранее отмеченная только +1. С дней6 без новой стерилизации и дней4 с новой → delete. None/stabilized с нулём дней → update({}) |
| 11 | Девять выборов из очищенного индекса | Head/4 и head/6 → Lost Teeth; torso/4 → Broken Ribs. Torso/6 в порядке имён файлов → Minor Head Wound, обратном → Ruptured Spleen. Четыре конечности выбирают свои исходные Item |
| 12 | Повторный addItem | Исходный Fractured Leg (Right) при совпадении name/type вызывает update(system.quantity=NaN), create не вызывается. Перехват до записи; сохранение NaN не утверждается |
| 13 | Подписи правой ноги | Реальный addActiveEffects('dodge') возвращает +-3/+-2/+-3[Fractured Leg (Left)] для none/stabilized/treated соответственно |
| 14 | Origin, HTML и границы автоматизации | Все origins — мировые Item.UUID, сохранены; их адресаты не разрешались. У шести рук и двух Spleen effects=[], текст не запускает модификаторы, bleeding или периодические спасброски. Текст Roll 1d10 у Lost Teeth не inline-бросок |

772 — количество assert сценария, таблица также содержит наблюдения сериализованных моделей и отдельные статические выводы. Каждый файл имеет собственные контрольные значения Actor и список всех изменений с исходными строками key. При всех расчётах база восьми характеристик 5, HP.value25, броня/вес0; BODY.max остаётся 5, healingTime Complex=7.

Raw statuscounter flags, ownership, _stats и _key задокументированы отдельно от их очистки. В окружении без регистрации statuscounter его flags удаляются ядром; поведение модуля в действующем мире не проверено. Иконка icons/svg/item-bag.svg — ресурс ядра вне пофайлового анализа.

### Перекрёстные связи и наблюдения

Связаны [манифест](../../../system.json), [packs.mjs](../../../utils/packs.mjs), [extract.mjs](../../../utils/extract.mjs), [package.json](../../../package.json), регистрация моделей, ready/getIndex и criticalWoundsPack. [Ready](../../../module/TheWitcherTRPG.js):62–67 запрашивает criticalLevel/location/lesserEffect/treatment; реальный серверный индекс не заменён утверждением о работе фасада. Формы, _onTreat/_onDropItem и recoverActor сопоставлены с [редактором](../../../templates/sheets/item/criticalWound-sheet.hbs), [списком](../../../templates/partials/crit-wounds-table.hbs), [классом листа](../../../module/item/sheets/WitcherCriticalWoundSheet.js) и [действиями травм](../../../module/actor/sheets/mixins/criticalWoundMixin.js)/[отдыха](../../../module/actor/sheets/mixins/healMixin.js).

При формировании формулы addActiveEffects читает фактические skill activeEffectModifiers и effect.name. [StunSave](../../../module/actor/mixins/defenseMixin.js):433–454 читает derivedStats.stun.value, но полный спасбросок в .059 не запускался. Текстовая [Complex Critical](../../../packsJson/combat/Complex_Critical_p3EAPCnu8RDawpWR.json) не является читаемым источником Item для applyCritWound.

Добавлена [issue-00326](../../issues/potential/issue-00326.md): имя эффекта у правой ноги говорит о левой и попадает в подпись штрафа. [Issue-00324](../../issues/potential/issue-00324.md) уточнена по настоящим очищенным моделям: torso у Minor Head Wound сохраняется, порядок индекса меняет выбор. Дополнены [00121](../../issues/potential/issue-00121.md), [00127](../../issues/potential/issue-00127.md), [00288](../../issues/potential/issue-00288.md) реальными Complex документами. Дубли не создавались, все 326 остаются potential; open/closed пусты.

Числа правой treated-ноги отличаются от левой и усиливают штраф относительно stabilized; это явно отражено в карточке, без объявления нового правила или исправления баланса. Общее сложение двух ног допускает отрицательный SPD в текущем расчёте; это наблюдение, а не согласование минимума. Текстовые ограничения и отсутствие автоматизации не объявлялись ошибками соответствия книгам.

Сверены 50 экспортов Simple/Complex; их совокупный SHA-256 (путь + нулевой разделитель + байты, в отсортированном порядке): `2a04560a4cb3dd9aca450cbd3c3d8c919c23ee93ceb77453a6a2ad5dce671958`. Исправлены ссылки на номера строк корневого _id в 14 прежних карточках Simple: раньше номер относился к вложенному эффекту. Значения ID/UUID и технические выводы не изменены, исходники сохранены; история уточнения добавлена в каждую карточку. Полный повтор всех сценариев Simple не выполнялся, его исходники не изменились.

### Итоговая сверка и сохранность

Проверены 25 новых карточек: все 61 строки changes, исходные значения/mode/priority, подготовленные type/phase, типы полей и номера строк key. Отдельно сверены root/effect-метаданные, 16 followUp, 24 индивидуальных примера Actor и номера строк корневых _id во всех 50 карточках Simple/Complex.

Реестр и карточки взаимно однозначны: 567 проверенных / 48 не начатых из 615. Все 604 файла TASK-0003 распределены без повторов; вместе с 11 TASK-0002 дают 615. .001–.059 done, .060–.061 planned, родитель in-progress, TASK-0004/0005 draft. 326 potential соответствуют реестру, open/closed пусты; 321 прежняя карточка issue не менялась. Остальные 590 строк файлового реестра и историческая часть review-log сохранены.

Локальные ссылки и Markdown-якоря всего docs, таблицы и пробелы изменённых документов проверены, ошибок нет. Scope: 43 изменённых существующих Markdown, 25 новых карточек и один issue. HEAD/ветка/индекс Git не менялись, коммит не создавался.

Все 615 текущих исходников и исторические 621 совпадают с исходным срезом: SHA-256 (отсортированные пути + нулевой разделитель + байты) `384f3c2f6d5f5c50b049bb913ee749f0acab5b1406a87a5d2b2eac1f25a04d5c` и `52701d3d0a5f054319886ac2a9d45b42c26c80098858d02518579c6a1edfaec4`. У всех 1639 исходно отслеживаемых файлов сохранены mode/uid/gid/inode; хеши 1596 файлов вне перечня правок не изменились.

### Следующие этапы

Подготовлены 567/615 карточек, включая 178/226 экспортов и 50/98 criticalWounds. В очереди 48 файлов .060–.061. Эти задачи остаются planned, TASK-0003 in-progress, TASK-0004/TASK-0005 draft. Протокол служит вспомогательным материалом для последующей общей сверки; соответствие контента рулбукам не проверялось.
...
### Состав и сверка внутренних блоков

| Блок | Файлов | Результатов | Проверенная особенность |
| --- | --- | --- | --- |
| Обучение | 3 | 60 | Три варианта ссылок при одинаковой formula=1d10 |
| Испытания | 5 | 40 | Пять formula; диапазоны −3…14 и достижимость каждой строки |
| Школа, важное событие, положение | 3 | 30 | Каждый ID, range, description, ссылка и потребитель |
| Союзники | 8 | 43 | Каждый ID, range, description, ссылка и потребитель |
| Благоприятный исход | 1 | 10 | Каждый ID, range, description, ссылка и потребитель |
| Враги | 8 | 44 | Один переход в lifepath; смерть, причина, inline-десятилетия |
| События, раны, выбор опасности | 3 | 24 | Каждый ID, range, description, ссылка и потребитель |
| Охота | 6 | 42 | Четыре безусловных аспекта и условное осложнение |
| Четыре Outcome | 4 | 28 | Разные диапазоны исходов; опасность выбирает внешний Decade |

Итого: **41 файл, 8708 логических строк, 321 TableResult: 226 text и 95 document**. Строгий Python JSON-разбор отверг бы повторные ключи; проверены все корневые и вложенные поля, _id, _key, flags, _stats, ownership/folder/sort, существование иконок. 362 ключа хранения различны; 73 локальных ID результатов повторяются между разными родителями без конфликта.

У всех root flags={"better-rolltables":{},"core":{}}, img=icons/svg/d20-grey.svg, replacement=true, displayRoll=false, folder=null, sort=0; ownership default=0 и старый пользователь=3. _stats.coreVersion=13.341 — история экспорта; фактическое исполнение проверено на Foundry 14.367.0 / Node.js 24.16.0. В результатах flags пусты, weight=1, drawn=false; text имеет пустое name и описание, document — имя цели, пустое описание и UUID.

Внутренние сверки выполнены для всех девяти блоков: отдельные карточки сопоставлены с полными JSON, частотами диапазонов и таблицами входящих/исходящих ссылок. Похожие описания вариантов не заменяли проверки отдельных ID, formula и адресов. На заключительном проходе дополнительно проверяются фактические строки documentUuid и source description; номера не выводятся из названия или номера результата.

### Формулы, модификаторы и границы

Три Early Training (обычная, +2, −2) имеют formula=1d10 и десять пар text + document. Модификатор реализуется выбором Trials, а не арифметикой названия. Trials имеет пять настоящих формул: 1d10−4, 1d10−2, 1d10, 1d10+2, 1d10+4.

| Формула Trials | Возможные totals | Доли четырёх исходов по диапазонам −3…1 / 2…3 / 4…9 / 10…14 | Недостижимые строки |
| --- | --- | --- | --- |
| 1d10−4 | −3…6 | 5/10, 2/10, 3/10, 0/10 | Extra Mutations и его ссылка |
| 1d10−2 | −1…8 | 3/10, 2/10, 5/10, 0/10 | Extra Mutations и его ссылка |
| 1d10 | 1…10 | 1/10, 2/10, 6/10, 1/10 | Нет |
| 1d10+2 | 3…12 | 0/10, 1/10, 6/10, 3/10 | Nearly Fatal и его ссылка |
| 1d10+4 | 5…14 | 0/10, 0/10, 5/10, 5/10 | Nearly Fatal / Poorly Accepted и две ссылки |

Таким образом штатными formula достижимы **311 из 321 results**. Остальные десять сохранены и описаны; это само по себе не дефект: общий набор диапазонов обслуживает варианты со сдвигами. Реальные модели разрешили отрицательные range. getResultsForRoll(0), (−3), (14) у каждой Trials возвращает пару по диапазону, независимо от собственной formula. Внешний roll=1d1+13 в Trials −4 выбрал пару Extra Mutations с total=14.

По коду roll при отсутствии пересечения диапазонов с внешней формулой выдаёт TABLE.NoPossibleResults; при отдельном промахе повторяет бросок до подходящего значения, не ограничивает его верхней/нижней границей. Реальный контроль с 1d20 дал 20, затем 10 за два обращения к RNG. Контроль 1d1+99 вернул пустой массив и ровно одно уведомление. У штатных формул всех 41 таблиц дыр в покрытии нет.

30 маршрутов возраст (грани 1/3/9 в Witcher Background Generator) × обучение (1–10) подтвердили current-переходы: возраст −2 даёт при обучении 4/7 итоговый модификатор 0/−4, возраст +2 — +4/0; остальные восемь ветвей обоих вариантов ведут к Trials без модификатора. Это 16 мест новой potential [issue-00321](../../issues/potential/issue-00321.md). Ожидаемое сохранение исходного модификатора выведено из текстов того же генератора; правило рулбука не привносилось.

### Граф, внешние входы и процесс вывода

94 documentUuid направлены внутрь witcher-lifepath, один — на [Enemies: Gender](files/packsJson/lifepath/Enemies__Gender_FMondgMHlPLSy3cq.json.md) в Life_Event_Sub-tables. Все 95 целей найдены в экспортном наборе; имена/ID/типы совпали. Из пяти Character-gen приходят 18 прямых ссылок: шесть из Witcher Background Generator и по три из четырёх Decade. Повторный UUID в другом результате учтён отдельно и привязан к собственному _id и строке.
...
Штатные лист/каталог Foundry вызывают draw. Поиск всех 41 ID и имён, полного имени пакета по module/templates/utils и en/ru не нашёл прямого программного потребителя. WitcherItem.checkIfItemHasRollTable:257–315 остаётся общим поиском по совпадению имени; этот метод ожидает Item, а конечные результаты здесь text. Он прочитан вместе с прежними карточками, но не исполнялся заново для всех 41 имён.

backgroundData/lifeEventData и tab-background хранят ручную биографию и события. lifepathData задаёт числовые модификаторы, которые читают отдельные боевые методы. Школа, ранение, награда или бонус из description не создают Item/Actor/ActiveEffect и не записывают эти поля. Возраст, повторение за десятилетия, выбор риска и естественно-языковые условия не выполняются автоматически.

### Исполненные сценарии и границы фасадов

Запуск — Node.js через `node --input-type=module` со скриптом stdin; чтение файлов без создания стенда, сохранения тестов, установки зависимостей или запуска сервера. Подключены реальные common DataModel/Document/EmbeddedCollection, BaseRollTable/BaseTableResult, client RollTable/TableResult, Roll/Die/RollParser, grammar.pegjs и HBS-шаблоны result-details/table-result из /opt/foundryvtt.

Фасады: минимальный ClientDocumentMixin, game/CONFIG/notifications, fromUuid/fromUuidSync и pack lookup по экспортам в памяти; ChatMessage.create и update перехвачены; render основного Roll заменён маркером; текстовые узлы и DOM-element — минимальные объекты. _enrichInlineRolls/_createInlineRoll и Roll.toAnchor выполнялись настоящими методами. Полный DOM enrichHTML, клики, визуальный UI, права игроков, HTTP и DB не выполнялись. Загрузка моделей соседних экспортов обеспечивала lookup и не объявляется их полноценным аудитом.

| Проверка | Фактический результат |
| --- | --- |
| Строгие модели 41 таблицы и всех 321 results | Приняты; _key отсутствует в toObject; текст/тип/иконка сохранены |
| Каждый возможный результат основного кубика | 563 значения; прямые ID независимо сопоставлены с диапазонами raw JSON |
| Рекурсивные roll и повторные draw без чата | Для всех 563 значений, 1126 повторных draw; 311 разных штатно достижимых results |
| Модели без pack / replacement=true | По два draw на таблицу, всего 82; drawn не записан |
| normalize(save:false) | 41 клон с диапазонами по единичным weight; исходные модели не изменены |
| getHTML каждого результата | 226 text и 95 document; все ссылки разрешены, в том числе десять естественно недостижимых строк |
| draw/toMessage | 563 прямых сообщения и восемь внешних Decade-сообщений; displayRoll=false скрывает основной бросок, все результаты попали в HTML |
| Inline | Все 14 вставок в 13 результатах трёх файлов; настоящие парсер/вычисления/сериализация |
| Возраст × обучение | 30 маршрутов, записаны formula/total посещённых Trials |
| Дополнительные составные сценарии | 15: RandomCharacter; четыре жив/мёртв союзника/врага; два случая осложнения охоты; восемь Decade |
| Основной сценарий | 6683 assertions, writes=0; snapshots моделей не изменились |
| Отдельный сценарий внешнего roll / отрицательных range | 23 assertions, запись/чат=0 |
| Всего утверждений двух запусков | 6706 |

Первый основной запуск остановился в проверочном чтении URI-кодированного dataset.roll: потребовался decodeURIComponent перед JSON.parse. После исправления проверки весь сценарий выполнен заново и прошёл. Это ошибка сценария, не исходных JSON. Контролируемые RNG и ожидаемые ID не подменяют методы выбора; минимальные/максимальные количества ветвей дополнительно получены независимым статическим обходом.

Inline без /команды исполняется при getHTML, а не при roll или draw(displayChat:false). В Law of Surprises выполняются обе вставки независимо от текстового условия «7: jewelery». Найдены ещё три места [issue-00319](../../issues/potential/issue-00319.md): Benefit Outcome (Law of Surprises: 1d6x10; Windfall: 1d10x100) и Danger — Events (Debt: 1d10x100). Настоящие формулы валидны, но x — explode. Фактические диапазоны 1–6 и 1–10, контрольные формулы с * дают 10–60 и 100–1000. Ранее найденные Fortune/Misfortune сохранены в той же карточке: всего пять вставок в четырёх JSON.

### Перекрёстная сверка документов и сохранность

Проверены реестр всех 320 прежних issues и совпадения по словам/механизмам; обучение не дублирует прежний issue. Зарегистрирована только issue-00321, issue-00319 дополнена; всего 321 potential, open/closed пусты. Подтверждения пользователя, исправления и новые задачи реализации отсутствуют.

Уточнены восемь прежних карточек: system.json, utils/packs.mjs, utils/extract.mjs, backgroundData.js, lifeEventData.js, lifepathData.js, tab-background.hbs, Enemies: Gender. Прямые и обратные описания межпакетного перехода согласованы. Обновлены указатели, реестр, задачи, CHANGELOG и материалы TASK-0004/TASK-0005 без смены их draft.

Контрольные суммы вычисляются по отсортированным путям: UTF-8 path + NUL + байты файла.

| Срез | SHA-256 |
| --- | --- |
| 41 witcher-lifepath | 9fbee648bd3c71ec6492c88bba164c8e27d3db3658651316f4c119637ee62b99 |
| Текущие 615 исходников | 384f3c2f6d5f5c50b049bb913ee749f0acab5b1406a87a5d2b2eac1f25a04d5c |
| Исторические 621 исходник | 52701d3d0a5f054319886ac2a9d45b42c26c80098858d02518579c6a1edfaec4 |

Итоговая сверка завершена: реестр содержит 615 строк и 493 карточки, 122 файла ожидают шесть задач .056–.061. Все 321 result отражены в новых карточках, проверены 113 прямых связей с точными строками источников и все 563 наблюдаемых основных исхода с независимыми границами графа. 574 строки реестра вне порции и предыдущий текст журнала сохранены. Проверены 24 843 локальные ссылки, 541 таблица Markdown затронутых документов, git diff --check — без ошибок.

Права, владельцы, группы и inode всех 1544 ранее отслеживаемых файлов сохранены; содержимое всех файлов вне разрешённых правок неизменно. Изменены 23 прежних документа, добавлены 41 карточка и один potential issue; других новых файлов нет. Все 615 текущих и 621 исторический исходник совпали с контрольными суммами и Git-срезом. HEAD, ветка и индекс не изменены; коммит не создавался. TASK-0003.055 завершена; общий TASK-0003 остаётся in-progress, TASK-0004/TASK-0005 — draft.
...
| Файл | Строк | Результатов | Особенность описаний |
| --- | --- | --- | --- |
| [packsJson/style/Style__Affectations_4RWDMDzNdnz2kgwU.json](files/packsJson/style/Style__Affectations_4RWDMDzNdnz2kgwU.json.md) | 260 | 10 | 2 HTML-сущности &amp;amp; |
| [packsJson/style/Style__Clothing_BuyEb4FcAyQL2hov.json](files/packsJson/style/Style__Clothing_BuyEb4FcAyQL2hov.json.md) | 260 | 10 | Десять разных текстовых вариантов |
| [packsJson/style/Style__Hair_Style_Ov9xIpAdWEPZCIoH.json](files/packsJson/style/Style__Hair_Style_Ov9xIpAdWEPZCIoH.json.md) | 260 | 10 | 3 HTML-сущности &amp;amp; |
| [packsJson/style/Style__Personality_TOQz3ETDronoeEDt.json](files/packsJson/style/Style__Personality_TOQz3ETDronoeEDt.json.md) | 260 | 10 | Десять разных текстовых вариантов |
| [packsJson/style/Values__Feelings_on_People_4eCXMVEfRx4PivWH.json](files/packsJson/style/Values__Feelings_on_People_4eCXMVEfRx4PivWH.json.md) | 260 | 10 | Два Neutral на исходах 5 и 6 |
| [packsJson/style/Values__Ideals_s5EjP50ddIVoitHT.json](files/packsJson/style/Values__Ideals_s5EjP50ddIVoitHT.json.md) | 260 | 10 | Десять разных текстовых вариантов |
| [packsJson/style/Values__Valued_Person_y1WCi6n2Kpwqb27P.json](files/packsJson/style/Values__Valued_Person_y1WCi6n2Kpwqb27P.json.md) | 260 | 10 | Десять разных текстовых вариантов |

Все файлы прочитаны, включая служебные данные и каждый вложенный результат. Структурная проверка Python с object_pairs_hook не обнаружила дубликатов ключей JSON. В каждом документе 14 корневых ключей и десять результатов по 11 ключей; состав и общие значения совпали. Различия перечислены в индивидуальных карточках, все 70 description и ID приведены в их таблицах.

- formula=1d10, replacement=true, displayRoll=true; каждый диапазон [n,n], n=1…10, weight=1, drawn=false.
- Тип каждого результата text; name пуст, description содержит текст. Нет documentUuid, inline-бросков, URL, @UUID-ссылок и HTML-тегов. Пять &amp;amp; в исходных description — HTML-сущности, а не команды.
- Десять локальных ID повторяются между таблицами, но уникальны внутри каждого родителя. Все 77 ключей хранилища (7 корневых и 70 вложенных) различны; реальные модели построили 70 разных UUID результатов.
- Пустые flags.better-rolltables/core не доказывают наличие модуля или автоматизации. Сохранённые _stats.coreVersion=13.341 и compendiumSource — история экспорта; они не являются ссылками для рекурсивного roll.
- Корневая иконка d20-grey и иконка результата d20-black найдены в public/icons установленного ядра. Ownership и отсутствие папки сохранены в данных; доступность документов реальным игрокам не проверялась.

### Связи с движком и соседними файлами

system.json:38,82–87 объявляет Style_and_Values_Sub-tables, тип RollTable и packs/style.db. utils/packs.mjs рекурсивно читает packsJson/style и передаёт compilePack путь packs/style; нормализация суффикса .db и контракт CLI относятся к ранее проверенной .050. utils/extract.mjs прочитан повторно, сверены _key, служебные поля и корневое расположение JSON. Команды сборки/извлечения не запускались.

В других packsJson найден один прямой потребитель — [Style_and_Values_CjaIcLRWSlzwI6ly.json](../../../packsJson/character-generator/Style_and_Values_CjaIcLRWSlzwI6ly.json). Все семь documentUuid указывают на существующий ID, ожидаемый RollTable и совпадающее имя:

| Порядок результата генератора | Цель | ID результата-ссылки | Строка documentUuid |
| --- | --- | --- | --- |
| 1 | Style: Clothing | q0DM4FW7XCnFdKZu | 30 |
| 2 | Style: Personality | 6eMBIJMQZmhaVyCr | 54 |
| 3 | Style: Hair Style | Kot5qN9Crj60jbwU | 78 |
| 4 | Style: Affectations | YqKoWFzgGZuSuzj0 | 102 |
| 5 | Values: Valued Person | 7LFO8Ez5nQK98SWT | 126 |
| 6 | Values: Ideals | O4kO4Ec9Zjr1F3Qm | 150 |
| 7 | Values: Feelings on People | HkPKK3KrmtmNyo1B | 174 |

У генератора 1d1 и все диапазоны [1,1]. getResultsForRoll(1) возвращает семь результатов, а recursive=true заменяет каждый ссылочный результат результатом соответствующей подтаблицы. При recursive=false остаются семь ссылок. Это допустимое использование пересечения диапазонов. Полный разбор генератора остаётся TASK-0003.056, его карточка в этой порции не создавалась.

В module/templates прямых вызовов по именам/ID/UUID style не найдено. Найден общий динамический поиск таблиц по Item.name: WitcherItem.checkIfItemHasRollTable, вызываемый при экспорте добычи WitcherMonsterSheet. При совпадении имени с любой из семи таблиц настоящий метод получает text и выдаёт exportLootInvalidItemError; он не превращает описание в Item. Для несовпадающего имени возвращён false. Проверялся искусственный Item с таким именем; наличие такого Item в действующем мире не установлено.

detailsData.js задаёт семь тематически соответствующих пар в system.general.details; tab-background.hbs выводит их для ручного заполнения. Обработчик автоматического переноса результатов style в .value в исследованной области не найден. Подобие названия поля и описания результата не считается программной связью. Уточнены пять прежних карточек: system.json, обе утилиты, WitcherItem и detailsData.

### Реальное ядро и пределы подмен

Установлены Foundry 14.367.0, Node 24.16.0, Handlebars 4.7.9 и Peggy 5.1.0. Проверки выполнены через stdin, без создания стенда или установки пакетов.

Использованы настоящие common/primitives, common/abstract/{data,document,embedded-collection}.mjs, common/data/fields.mjs и common/documents/{roll-table,table-result,folder,user}.mjs. Глобальные CONFIG/game задавали минимальные реестры типов text/document и доступов; настоящие RollTable/TableResult создавались со strict:true. Реальные BaseRollTable/BaseTableResult, очистка, валидация, EmbeddedCollection, buildUuid и parseUuid не заменялись.

Клиентские классы из client/documents/{roll-table,table-result}.mjs загружены из исходного текста: удалены только import и изменена форма экспорта для передачи зависимостей через Function. Тела методов сохранены. ClientDocumentMixin заменён оболочкой с пустыми базовыми prepare-методами, isOwner=true и перехватом обновлений; prepareBaseData/prepareDerivedData проверяемых классов вызывались явно. Полный жизненный цикл клиента этим не воспроизведён.

Настоящие Roll, Die/DiceTerm, RollParser и grammar.pegjs разобрали и вычислили формулы. Peggy создала парсер в памяти из установленной грамматики. randomUniform задавался контролируемо для получения каждого исхода 1…10; интерфейс RollResolver имел фасад без пользовательского ввода. Это проверка расчёта, а не статистического качества генератора случайных чисел.

TableResult.getHTML и RollTable.toMessage использовали настоящие Handlebars-шаблоны templates/sheets/roll-table/result-details.hbs и templates/dice/table-result.hbs с такими же allowProto-опциями, как client/applications/handlebars.mjs:102–107. enrichHTML возвращал исходный текст, Roll.render — обозначение числового результата, ChatMessage.create сохранял payload в массив. Вывод description и отсутствие дополнительного экранирования проверены; обработка DOM, секретов, custom enrichers и реальный чат не запускались.

fromUuid разрешал ссылки в карте документов, созданных из экспортов; разбор UUID выполняла настоящая utils.parseUuid. Индекс/загрузка компедиума для Item-потребителя, уведомления и запись документов были фасадами. Не выполнялись HTTP, запуск мира, чтение/миграция/запись packs, сборка/извлечение или изменение прав.

### Результаты изолированного исполнения

| Группа | Проверка | Фактический результат |
| --- | --- | --- |
| S01 | Строгие модели всех 7 JSON / 70 результатов | Все валидны; пустое name разрешено, description/type сохранены, documentUuid отсутствует; _key не входит в toObject |
| S02 | Идентичность вложенных документов | 10 повторяемых локальных ID дают 70 уникальных UUID; конфликтов внутри родителей нет |
| S03 | Все исходы и крайние значения | 70 roll: каждый целый 1…10 выбирает нужную единственную запись; настоящие min/max равны 1/10; прямой getResultsForRoll(0/11) пуст |
| S04 | Повторное получение | 140 draw с чат-фасадом, по два на исход; повтор возвращает ту же запись, drawn не меняется |
| S05 | Данные после вычислений | Сериализации семи исходных моделей после цикла неизменны, записей документов нет |
| S06 | Дополнительные ветви модели | drawn=true исключает запись; normalize(save=false) сохраняет 1d10/интервалы; 14 draw моделей без pack с displayChat=false также не меняют доступность |
| S07 | Формирование сообщения | 140 payload содержат нужное description и обозначение броска; пять HTML-сущностей не экранируются второй раз шаблоном |
| S08 | Общий генератор | recursive=false: 7 ссылок; recursive=true: 7 разрешений и 7 конечных текстов; один payload содержит все результаты без HTML кубика при displayRoll=false |
| S09 | Повтор текста Neutral | Исходы 5/6 сохраняют одинаковый текст при различных ID; технической неоднозначности выбора нет |
| S10 | Item-потребитель | 7 совпадающих имён дают контролируемое exportLootInvalidItemError; несовпадающее имя — false; создания Item/Actor нет |

Проверки модели/бросков/вывода содержали 521 проверочное утверждение; отдельная проверка Item-потребителя — ещё 9. Они завершились успешно. В основном сценарии перехвачено 141 сообщение, реальных записей документов — ноль. Во время подготовки минимального окружения уточнялись недостающие глобальные реестры, интерфейс RollResolver и Handlebars-опции; ошибки неполной подмены не регистрировались как ошибки исходных JSON.

### Issues и сверка документации

Новых технических проблем в этой порции не обнаружено. Выполнен поиск по реестру 318 issues; полностью сопоставлены issue-00002, issue-00039, issue-00313/00314/00315. Issue-00002 относится к настройке criticalWoundsPack при ready, а не к прямому чтению style. Issue-00313 остаётся применимым к процессу извлечения корневых JSON; 00314 про вложенные каталоги не воспроизведён этими файлами. Ограничения генератора добычи из 00039 не превращены в новые issues для каждой текстовой таблицы. Ни одна карточка issue и ни один статус не изменены.

Формальная проверка Python через stdin сопоставила исходники, реестр, карточки и все 61 подзадачу: 615 строк реестра, 396 проверенных карточек, 219 файлов в очереди. TASK-0003 распределяет 604 различных файла; вместе с 11 файлами TASK-0002 это весь текущий объём без пересечений. Все 70 строк результатов новых карточек буквально совпали с ID/description исходных JSON; проверены контрольные суммы и 11 обязательных разделов каждой карточки. Остальные 608 строк реестра сохранены.

Проверены 21 564 локальные ссылки по всей документации и 176 Markdown-таблиц в материалах изменения; ошибок нет. git diff --check завершился без замечаний. Все 318 issues сохранены. Прежний журнал полностью сохранён после нового раздела. Содержимое 621 исходного файла исторического среза не изменилось; контрольные суммы текущих 615 и исторических 621 совпали с предыдущими проверками.

Созданы семь карточек; обновлены 17 существующих документов, включая пять уточнений связей. Для всех 1479 ранее существовавших файлов сохранены mode/uid/gid/inode; изменений содержимого вне согласованного перечня нет. TASK-0003.052 завершена, следующие .053–.061 остаются planned. TASK-0003 продолжается; следующая порция — TASK-0003.053.

Контрольная сумма семи файлов (отсортированный UTF-8 путь, нулевой байт, байты файла): bf80c235455c36140dd223a3b4e8e254a996bf4cca62a074a8f6b935046f0b51. Срез текущих 615 исходников: 384f3c2f6d5f5c50b049bb913ee749f0acab5b1406a87a5d2b2eac1f25a04d5c.

Пределы: описания относятся к экспортным JSON и указанной версии исходников. Не подтверждены содержимое действующей БД, права игроков, сторонние модули/макросы и соответствие контента рулбукам. Прямые и обратные связи с остальными компедиумами подлежат последующей сверке .056/.057/.061. TASK-0004/TASK-0005 этим разбором не завершаются.

## Планирование TASK-0003.052–TASK-0003.061

2026-09-12. Ветка rusbar-main, HEAD 4b9951094106e26d9274bbd5d5e8e7a709cfcf24. По поручению пользователя подготовлены десять задач технического разбора семи компедиумов. На старте уже были 23 незакоммиченных документа по .051 (18 изменённых и пять новых); их изменения сохранены. Этот раздел фиксирует постановку задач, а не выполнение пофайлового анализа.

### Границы и распределение

Все 226 файлов packsJson возвращены в [реестр](registry.md). Текущий объём — 615: 389 проверенных карточек и 226 файлов со статусом «Не начат», без готовых карточек. Другие шесть локализаций de/es/fr/it/pl/ptbr и содержимое packs/ остаются исключёнными. Соответствие текстов и чисел книгам правил не проверяется. Исторические результаты 621 и завершение .051 в границах 389 сохранены.

| Подзадача | Предмет | Файлов | Строк | Статус |
| --- | --- | --- | --- | --- |
| [TASK-0003.052](../../tasks/task-0003.052.md) | Таблицы стиля и ценностей | 7 | 1820 | `planned` |
| [TASK-0003.053](../../tasks/task-0003.053.md) | Таблицы жизненных событий | 21 | 3845 | `planned` |
| [TASK-0003.054](../../tasks/task-0003.054.md) | Подтаблицы создания персонажа | 35 | 8319 | `planned` |
| [TASK-0003.055](../../tasks/task-0003.055.md) | Таблицы биографии ведьмака | 41 | 8708 | `planned` |
| [TASK-0003.056](../../tasks/task-0003.056.md) | Основные генераторы персонажа | 13 | 2435 | `planned` |
| [TASK-0003.057](../../tasks/task-0003.057.md) | Боевые таблицы | 11 | 1915 | `planned` |
| [TASK-0003.058](../../tasks/task-0003.058.md) | Простые критические травмы | 25 | 2068 | `planned` |
| [TASK-0003.059](../../tasks/task-0003.059.md) | Сложные критические травмы | 25 | 2058 | `planned` |
| [TASK-0003.060](../../tasks/task-0003.060.md) | Тяжёлые критические травмы | 25 | 3351 | `planned` |
| [TASK-0003.061](../../tasks/task-0003.061.md) | Смертельные критические травмы и общая сверка компедиумов | 23 | 2131 | `planned` |
| **Всего** | **Технический разбор семи компедиумов** | **226** | **36650** | В очереди |

Шесть компедиумов RollTable содержат 128 документов и 995 вложенных результатов. Седьмой компедиум criticalWounds содержит 94 Item и четыре Folder, 79 эффектов и 360 changes. Группы тяжести выделены отдельно из-за эффектов и переходов followUp. Приведённые количества получены чтением JSON; они не подтверждают техническую исправность данных или их соответствие установленной БД.

В каждой задаче указан полный перечень файлов с именами, экспортными типами и количеством логических строк; определены порядок, зависимости для сверки, проверки, материалы и критерии приёмки. При выполнении каждый JSON получает свою карточку; вложенные сущности описываются в ней. Массовый проход допускается по всем записям с отдельным разбором исключений.

### Контрольные точки плана

- После .056: пять наборов создания персонажа и биографии, 117 файлов и 929 результатов.
- После .057: все шесть наборов RollTable, 128 файлов и 995 результатов.
- После .061: весь criticalWounds (98 файлов), затем все 226 экспортных файлов с манифестом, инструментами и уже описанными потребителями движка; сверка текущего реестра из 615 файлов.
- После каждой порции: исходники ↔ карточки ↔ зависимости ↔ issues, локальные ссылки и применимость версии. Итоговая сверка .061 не заменяет отдельную TASK-0004 и не завершает её автоматически.

TASK-0003 возобновлена в статусе in-progress. Все .052–.061 имеют статус planned; .001–.051 остаются done, TASK-0004/TASK-0005 — draft. Следующей поставлена [TASK-0003.052](../../tasks/task-0003.052.md). Общее распределение TASK-0003 — 604 различных файла; вместе с 11 файлами TASK-0002 это весь текущий реестр.

### Проверка постановки

Проверка выполнена Python-сценариями через stdin из корня репозитория, без создания тестового стенда. Сценарии независимо прочитали все экспортные JSON, сопоставили их с манифестом, таблицами десяти задач, реестром и существующими карточками. Проверка ссылки подтверждает наличие файла/Markdown-якоря, а не исполнение игрового сценария.

- Все 226 файлов распределены ровно один раз; имена, экспортные типы, количество строк и вложенных записей совпали с таблицами задач. Сопоставлены все семь имён/путей/типов пакетов system.json.
- Реестр содержит 615 различных файлов: 389 «Проверено», 226 «Не начат». Все прежние 389 строк реестра сохранены буквально; каждому проверенному файлу соответствует одна из 389 существующих карточек. Для планируемых JSON карточки не созданы и ссылки на будущие файлы не добавлены.
- 61 подзадача TASK-0003 охватывает 604 различных файла, из них 226 в очереди. Вместе с 11 файлами TASK-0002 получен полный текущий состав; пропусков и пересечений нет. Статусы .001–.051 — done, .052–.061 — planned, родительской задачи — in-progress; TASK-0004/TASK-0005 остаются draft.
- Проверены 21 394 локальные ссылки по всей документации, включая якоря: ошибок нет. Проверка 122 Markdown-таблиц в материалах изменения и пробелов в конце строк прошла; git diff --check завершился без замечаний.
- Все 318 issues остались в potential, их документы и реестр не изменены. Существующие карточки и прежние документы подзадач .001–.051 сохранены.
- Содержимое всех 621 исходных файлов совпадает с исходным срезом TASK-0001. Для всех 1469 файлов, существовавших перед этой работой, сохранены mode/uid/gid/inode; сравнение контрольных сумм не обнаружило изменений вне 11 согласованных документов. Новых файлов ровно десять — документы задач.
- Прежний журнал сверок, включая незакоммиченную запись .051, сохранён полностью: восстановленный текст без новой записи совпал с его SHA-256 перед началом работы. Предыдущие незакоммиченные материалы не потеряны.

SHA-256 рассчитан по упорядоченному списку путей: для каждого файла UTF-8 путь, нулевой байт и содержимое. Он фиксирует срез исходников для последующей проверки применимости; пофайловой разбор и запуск Foundry этой суммой не подтверждаются.

| Набор исходников | Файлов | SHA-256 |
| --- | --- | --- |
| Текущие границы | 615 | 384f3c2f6d5f5c50b049bb913ee749f0acab5b1406a87a5d2b2eac1f25a04d5c |
| Возвращённые packsJson | 226 | 23e9769b872532f51910c7890d41efe80a76fd623921cdabf8de2be406054e14 |
| Исторический срез TASK-0001 | 621 | 52701d3d0a5f054319886ac2a9d45b42c26c80098858d02518579c6a1edfaec4 |

Созданы только десять документов задач. Обновлены родительская задача и реестр задач, текущие границы/реестр аудита, указатели README, этот журнал, CHANGELOG и примечания о новых границах в заготовках TASK-0004/TASK-0005. Исходные JSON, код, существующие карточки файлов и issues не правились. Сборка, извлечение, запуск/запись мира и HTTP-проверки не выполнялись: предмет проверки — документация плана.

## TASK-0003.051

2026-09-12. Ветка rusbar-main, HEAD 4b9951094106e26d9274bbd5d5e8e7a709cfcf24; стартовое рабочее дерево чистое. Пользователь поручил разобрать en/ru, ограничив текущий аудит движком. Выполнена [TASK-0003.051](../../tasks/task-0003.051.md). Прочие языки, контент компедиумов и соответствие текстов игровым правилам в порцию не включены.

### Изменение границ и охват

Это изменение состава исследования, а не исходной версии кода. Из прежнего реестра исключены 232 ещё не описанных файла: все 226 packsJson и lang/de.json, es.json, fr.json, it.json, pl.json, ptbr.json. Содержимое packs/ также вне текущих границ; пять LOCK уже были исключены из исторических 621. Файлы не удалялись и не менялись. Программные контракты компедиумов — манифест, выбор/загрузка пакетов, compile/extract — остаются в описаниях движка.

| Группа | Было в реестре | Теперь | Описано |
| --- | --- | --- | --- |
| module | 214 | 214 | 214 |
| templates | 132 | 132 | 132 |
| CSS styles | 36 | 36 | 36 |
| build.json, package.json, system.json | 3 | 3 | 3 |
| utils | 2 | 2 | 2 |
| lang | 8 | 2 | 2 |
| packsJson | 226 | 0 | Не входят |
| Всего | 621 | 389 | 389 |

Прежние 387 карточек сохранены; созданы две новые. TASK-0002 охватила 11 файлов, 51 подзадача TASK-0003 — 378 различных файлов без пересечений. Пофайлового остатка в текущих границах нет. TASK-0003 завершена; TASK-0004 и TASK-0005 сохраняют draft. Возможность будущего обзора однотипных компедиумов крупными группами не означает его запуска.

| Файл | Строк | Строковых листьев | Карточка |
| --- | --- | --- | --- |
| lang/en.json | 1529 | 1153 | [en](files/lang/en.json.md) |
| lang/ru.json | 1503 | 1135 | [ru](files/lang/ru.json.md) |
| Всего | 3032 | 2288 | Два полных структурных разбора |

1133 ключа общие; 20 только en, 2 только ru. Оба словаря имеют три корня TYPES/EFFECT/WITCHER, 105 групп/ключей второго уровня и 63 dotted-записи. Все конечные значения — строки. JSON.parse с object_pairs_hook не обнаружил повторов; полный независимый обход и настоящий expandObject/mergeObject дали одинаковые пути/значения без потерь. По четыре пустые hint-строки являются допустимыми переводами. HTML-тегов нет. Две строки с подстановками используют совпадающие имена currency и target/heal.

### Методы и исполняемая среда

Foundry 14.367.0 из /opt/foundryvtt/package.json, Node 24.16.0, Handlebars 4.7.9. Python/Node запускались из корня репозитория через stdin, без создания стенда и файлов тестов. В Node registerHooks сопоставлял @common/ с установленным common/; загружались реальные primitives, helpers, fields, DataModel/TypeDataModel, ActiveEffectTypeDataModel и client/helpers/localization.mjs. Из client/applications/handlebars.mjs извлечён и выполнен настоящий helper localize; системные helpers зарегистрированы исходным registerHandelbarHelpers.

При проверке setLanguage fetch был заменён функцией, возвращающей исходные en/ru и локальный /opt/foundryvtt/public/lang/en.json через resp.json в памяти. Пакеты/Document DOM были фасадами; никакой HTTP-запрос не выполнялся. Дополнительные module/world переводы — явно созданные тестовые объекты, не данные настоящего мира.

onHeal исполнялся из исходного тела с перехватом fromUuidSync, target.update и ChatMessage; currencyConverter.openCurrencyConverter — из исходного mixin с фасадами DialogV2, renderTemplate и notifications. Wizard использовал настоящий config и схему семи damageTypeModification. Формы с formGroup проверены по полям/контексту/шаблонам; полный DOM формы не рендерился.

При подготовке сценария исправлены ошибки самого проверочного кода: сначала число объявленных типов ошибочно ожидалось равным числу переводов (26 вместо 22), затем произвольный poison в фикстуре заменён настоящей схемой damageTypeModification, исправлен обход вложенных HBS AST-узлов. В итоговую статистику включён полный успешный запуск после этих исправлений; промежуточный неполный счётчик AST не используется. Исходники системы при этом не исправлялись.

### Семнадцать групп локальных проверок

| № | Проверяемый контракт | Полученный результат |
| --- | --- | --- |
| 1 | Все листья после expandObject и mergeObject | Точные пути/значения совпали с независимым полным обходом; 1153 en, 1135 ru |
| 2 | localize/has всех собственных строк и fallback | Все ru/en строки и 20 резервных значений совпали; empty сохраняется |
| 3 | Dotted-имена, регистр, начальный пробел | Actor.Skill.Intelligence разрешается; Water, DangerLow и ключ с пробелом не становятся правильными автоматически |
| 4 | Четыре пустые подсказки attackSkill | Реальные поля дают существующие hint-ключи с пустой строкой в обоих языках |
| 5 | Контракты двух format-строк | Все подстановки совпадают; без data остаются скобки, отсутствующий аргумент в data даёт undefined |
| 6 | Исходный onHeal | При HP 4/9 и запросе 8 переданы фактическое лечение 5 и имя Target в обоих языках; запись/чат перехвачены |
| 7 | Исходный openCurrencyConverter | При нехватке валюты предупреждение содержит переведённое имя валюты, без undefined; update не вызывается |
| 8 | Настоящий HBS localize | Hash-подстановка работает; тестовый target с тегами экранируется обычным HBS-выводом |
| 9 | TYPES по манифесту и дополнительные переводы | Все 22 объявленных Actor/Item/ActiveEffect типа имеют en/ru; ещё четыре подписи относятся к прежнему рассогласованию issue-00005 |
| 10 | Настоящее поле applyAfterCalculations | Ключ получен из schema; ru отсутствует, возвращается en; template использует это поле с localize=true |
| 11 | silver в config и формах | DamageType.silver → Silver, отдельный Damage.silver → Серебро; general/spellGeneral используют config.damageTypes |
| 12 | Исходный мастер эффектов | Динамические группы характеристик, биографии и семи типов урона разрешаются; два неверных skillMap.label остаются issue-00016 |
| 13 | Полный обход AST 132 HBS | 1094 localize: 1013 буквальных и 81 динамический; четыре hash-вызова DOCUMENT.*; буквальные WITCHER-пропуски сопоставлены с прежними issues |
| 14 | Старый WITCHER.Crit | 120 ключей 24 записей отсутствуют в en/ru; наличие данных само по себе не доказывает использование |
| 15 | Разметка и подстановки всех строк | HTML-тегов нет; только два одинаковых набора placeholders |
| 16 | Реальный setLanguage с локальным fetch-фасадом | Загружаются ru системы и en fallback/ядра; EFFECT.TABS.duration разрешается из ядра |
| 17 | Реальный порядок объединения пакетов | Активный модуль переопределяет систему, мир — модуль; выключенный модуль без world оставляет систему |

Все 17 групп прошли. Это изолированное выполнение перечисленных контрактов, а не проверка запуска Foundry, исправности системы или всех игровых ветвей.

### Перекрёстная сверка связей и issues

Полный строковый поиск охватил 214 JS и 132 HBS. Точные упоминания найдены для 998 ключей en и 981 собственного ключа ru. В [инвентаре en](files/lang/en.json.md#полный-инвентарь-строковых-ключей) для каждого из 1153 листьев указаны найденные файлы/строки: 2003 точных упоминания в 197 файлах проверены по исходным строкам; это определения/упоминания ключей, не только вызовы localize. Префиксы, выражения внутри backticks, label/hint моделей, сохранённые имена и ядровые TYPES требуют отдельного разрешения и не считаются автоматически отсутствующими/неиспользуемыми ключами.

| Связь | Сопоставление и вывод |
| --- | --- |
| system.json → Localization → en/ru | Пути зарегистрированы, механизм загрузки и fallback подтверждён настоящим ядром; шесть языков исключены только из аудита |
| TABS.systemSpecific → EFFECT.TABS.systemSpecific | Ядровой labelPrefix наследуется листом; подпись существует в обоих языках |
| WitcherActiveEffectData → systemFields → formGroup | Четыре собственных label имеют ru, пятый applyAfterCalculations использует en |
| config.damageTypes → WitcherConfigurationSheet → general/spellGeneral | Установлен потребитель silver через options и localize=true; путь перевода ru отличается |
| config.statMap/skillMap → wizard/броски | Dynamic label и полные ключи сопоставлены; прежние ошибки регистра не создают новых issues |
| currencyConverter / chat.onHeal → format | Имена аргументов совпали с обоими словарями; исходные обработчики проверены изолированно |
| HBS → ядровой helper | Собственный registerHandelbarHelpers не определяет localize; AST/ядровой helper проверены отдельно |
| WITCHER.Crit → потенциальный потребитель | Прямых чтений CONFIG.WITCHER.Crit/config.Crit в module/templates не найдено; современное применение травм получает Item из пакета |

Уточнены шесть ранее существовавших карточек: system.json, config.js, WitcherActiveEffectSheet.js, witcherActiveEffectData.js, WitcherConfigurationSheet.js и handlebars.js. В карточке модели эффекта исправлено слишком общее прежнее указание «наличие сверено»: все пять подписей существуют в en, только четыре — в ru. Исторические методы/проверки этих карточек не приписаны новому запуску.
...
### Версия, формальная сверка и пределы

Все 621 исходник прежнего реестра по-прежнему совпадают со срезом TASK-0001 15da5b225535e34af4e132c701b5353ef4eb667f. Агрегат SHA256 строится по отсортированным путям: UTF-8(path), нулевой байт, затем байты файла.

- Исторические 621: 52701d3d0a5f054319886ac2a9d45b42c26c80098858d02518579c6a1edfaec4.
- Текущие 389: 0ddc37f317b432277e18fe1de6254f3bd88d025eb22446814bf1a2a193f8108a.

Формальная сверка: 389 строк реестра ↔ 389 карточек; 51 выполненная подзадача ↔ 378 уникальных файлов TASK-0003; 318 issues соответствуют реестру и остаются potential. Проверены 20690 локальных ссылок и якорей во всей документации — ошибок нет. Пять новых документов содержат 28 корректных Markdown-таблиц; полный индекс en содержит ровно 1153 уникальных ключа, таблица русских пропусков — ровно 20. Изменены 18 существующих документов и созданы пять; все изменения внутри docs/. git diff --check прошёл.

Числа прежних записей относятся к историческим границам и не переписаны. Старое содержимое review-log сохранено целиком после новой записи. Проверены mode/uid/gid/inode всех 1464 ранее отслеживаемых файлов: изменений нет. Новые документы создавались без отдельного назначения прав.

Не выполнялись HTTP-доступ к системе, браузер, настоящий мир, БД, сборка/извлечение, установка зависимостей или игровые изменения. Дополнительные переводы модулей/мира, все пользовательские ключи, другие языки и содержание компедиумов не проверены. Отсутствие потребителя в строковом поиске не доказывает ненужность данных. TASK-0004/TASK-0005 ещё предстоит детализировать и выполнять.
